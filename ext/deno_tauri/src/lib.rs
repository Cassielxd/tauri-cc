...
use deno_core::{op2, JsBuffer, OpState, ResourceId};
...
#[op2(fast)]
#[smi]
pub fn op_tauri_broadcast_subscribe(state: &mut OpState) -> Result<ResourceId, AnyError> {
...
#[op2(async)]
#[serde]
pub async fn op_tauri_broadcast_recv(state: Rc<RefCell<OpState>>, #[smi] rid: ResourceId) -> Result<Option<Message>, AnyError> {
...
#[op2(async)]
pub async fn op_tauri_broadcast_send(state: Rc<RefCell<OpState>>, #[smi] rid: ResourceId, #[string] name: String, #[buffer] buf: JsBuffer) -> Result<(), AnyError> {
...
#[op2(fast)]
pub fn op_tauri_broadcast_unsubscribe(state: &mut OpState, #[smi] rid: ResourceId) -> Result<(), AnyError> {
