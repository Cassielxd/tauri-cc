...
mod reader_stream;
lazy_static! {
  pub static ref REQUEST_CHANNEL: (async_channel::Sender<RequestContext>, async_channel::Receiver<RequestContext>) = async_channel::unbounded::<RequestContext>();
}
...
async fn op_http_accept(state: Rc<RefCell<OpState>>, #[smi] _rid: ResourceId) -> Result<Option<NextRequestResponse>, AnyError> {
  let receiver = REQUEST_CHANNEL.1.clone();
...
/// 添加默认路由的封装  主要用作第三方server 接受请求转发
