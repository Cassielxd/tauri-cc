...
mod ts {
  ...
  fn op_build_info(state: &mut OpState) -> BuildInfoResponse {
    ...
    let build_libs = state.borrow::<Vec<&str>>().iter().map(|s| s.to_string()).collect();
    let node_built_in_module_names = SUPPORTED_BUILTIN_NODE_MODULES.iter().map(|s| s.to_string()).collect();
    BuildInfoResponse {
      build_specifier,
      libs: build_libs,
  ...
  #[op2]
  #[string]
  fn op_script_version(_state: &mut OpState, #[string] _arg: &str) -> Result<Option<String>, AnyError> {
    Ok(Some("1".to_string()))
  }
  ...
  fn op_load(state: &mut OpState, #[string] load_specifier: &str) -> Result<LoadResponse, AnyError> {
    ...
    if load_specifier == build_specifier {
      ...
      if let Some(lib) = caps.get(1).map(|m| m.as_str()) {
        ...
        Err(custom_error("InvalidSpecifier", format!("An invalid specifier was requested: {}", load_specifier)))
      }
    } else {
      Err(custom_error("InvalidSpecifier", format!("An invalid specifier was requested: {}", load_specifier)))
    }
  ...
  pub fn create_compiler_snapshot(snapshot_path: PathBuf, cwd: &Path) {
    ...
    for name in libs.iter() {
      println!("cargo:rerun-if-changed={}", path_dts.join(format!("lib.{name}.d.ts")).display());
    }
    ...
    std::fs::write(PathBuf::from(env::var_os("OUT_DIR").unwrap()).join("lib_file_names.json"), serde_json::to_string(&build_libs).unwrap()).unwrap();

    let output = create_snapshot(CreateSnapshotOptions {
      cargo_manifest_dir: env!("CARGO_MANIFEST_DIR"),
      snapshot_path,
      startup_snapshot: None,
      extensions: vec![deno_tsc::init_ops_and_esm(op_crate_libs, build_libs, path_dts)],
...
fn create_cli_snapshot(snapshot_path: PathBuf) -> CreateSnapshotOutput {
  ...
  fn deno_version() -> String {
    if env::var("DENO_CANARY").is_ok() {
      format!("{}+{}", env!("CARGO_PKG_VERSION"), git_commit_hash()[..7].to_string())
    } else {
  ...
  let extensions: Vec<Extension> = vec![
    ...
    deno_web::deno_web::init_ops::<PermissionsContainer>(Default::default(), Default::default()),
    deno_fetch::deno_fetch::init_ops::<PermissionsContainer>(Default::default()),
    deno_cache::deno_cache::init_ops::<SqliteBackedCache>(None),
    deno_websocket::deno_websocket::init_ops::<PermissionsContainer>("".to_owned(), None, None),
    deno_webstorage::deno_webstorage::init_ops(None),
    deno_crypto::deno_crypto::init_ops(None),
    deno_broadcast_channel::deno_broadcast_channel::init_ops(deno_broadcast_channel::InMemoryBroadcastChannel::default()),
    deno_ffi::deno_ffi::init_ops::<PermissionsContainer>(),
    deno_net::deno_net::init_ops::<PermissionsContainer>(None, None),
    deno_tls::deno_tls::init_ops(),
    deno_kv::deno_kv::init_ops(SqliteDbHandler::<PermissionsContainer>::new(None, None)),
    ...
    deno_runtime::ops::runtime::deno_runtime::init_ops("deno:runtime".parse().unwrap()),
    deno_runtime::ops::worker_host::deno_worker_host::init_ops(Arc::new(|_| unreachable!("not used in snapshot.")), None),
...
fn main() {
  ...
  let symbols_path = std::path::Path::new("src/napi")
    .join(format!("generated_symbol_exports_list_{}.def", env::consts::OS).as_str())
    .canonicalize()
    .expect("Missing symbols list! Generate using tools/napi/generate_symbols_lists.js");

  #[cfg(target_os = "macos")]
  println!("cargo:rustc-link-arg-bin=deno=-Wl,-exported_symbols_list,{}", symbols_path.display());
  ...
  #[cfg(target_os = "linux")]
  {
    ...
    if env::var("CC").is_err() && glibc_version::get_version().map(|ver| ver.major <= 2 && ver.minor < 35).unwrap_or(false) {
      ...
      println!("cargo:rustc-link-arg-bin=deno=-Wl,--export-dynamic-symbol-list={}", symbols_path.display());
    }
  }
