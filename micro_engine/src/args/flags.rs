...
impl CompileFlags {
  pub fn resolve_target(&self) -> String {
    self.target.clone().unwrap_or_else(|| env!("TARGET").to_string())
  }
}
...
fn join_paths(allowlist: &[PathBuf], d: &str) -> String {
  allowlist.iter().map(|path| path.to_str().unwrap().to_string()).collect::<Vec<String>>().join(d)
}
...
impl Flags {
  ...
  pub fn package_json_search_dir(&self, current_dir: &Path) -> Option<PathBuf> {
    ...
    match &self.subcommand {
      ...
      Bundle(_) | Completions(_) | Doc(_) | Fmt(_) | Init(_) | Install(_) | Uninstall(_) | Jupyter(_) | Lsp | Lint(_) | Types | Upgrade(_) | Vendor(_) | Publish(_) => None,
    }
  }
...
pub fn flags_from_vec(args: Vec<String>) -> clap::error::Result<Flags> {
  ...
  flags.cache_path= Some("./.deno".into());
  flags.unstable = true;
  flags.unstable_features.push(deno_runtime::deno_broadcast_channel::UNSTABLE_FEATURE_NAME.to_string());
  flags.unstable_features.push(deno_runtime::deno_ffi::UNSTABLE_FEATURE_NAME.to_string());
  flags.unstable_features.push(deno_runtime::deno_fs::UNSTABLE_FEATURE_NAME.to_string());
  flags.unstable_features.push(deno_runtime::ops::http::UNSTABLE_FEATURE_NAME.to_string());
  flags.unstable_features.push(deno_runtime::deno_kv::UNSTABLE_FEATURE_NAME.to_string());
  flags.unstable_features.push(deno_runtime::deno_net::UNSTABLE_FEATURE_NAME.to_string());
  flags.unstable_features.push(deno_runtime::ops::worker_host::UNSTABLE_FEATURE_NAME.to_string());
  flags.unstable_features.push(deno_runtime::deno_cron::UNSTABLE_FEATURE_NAME.to_string());
  flags.unstable_bare_node_builtins = true;
  //默认使用deno 自带npm 管理工具进行包依赖下载  如果不想使用客设置true  自行 使用npm install
  flags.unstable_byonm = false;
  flags.unstable_workspaces = true;
...
fn clap_root() -> Command {
  ...
  let mut cmd = Command::new("deno")
    ...
    .arg(Arg::new("unstable").long("unstable").help("Enable unstable features and APIs").action(ArgAction::SetTrue).global(true))
    .arg(
  ...
  for (flag_name, help, _) in crate::UNSTABLE_GRANULAR_FLAGS {
    cmd = cmd.arg(Arg::new(format!("unstable-{}", flag_name)).long(format!("unstable-{}", flag_name)).help(help).action(ArgAction::SetTrue).global(true));
  }

  cmd
    .arg(Arg::new("log-level").short('L').long("log-level").help("Set log level").hide(true).value_parser(["trace", "debug", "info"]).global(true))
    .arg(Arg::new("quiet").short('q').long("quiet").help("Suppress diagnostic output").action(ArgAction::SetTrue).global(true))
  ...
  deno bench src/",
    ...
    .defer(|cmd| {
      runtime_args(cmd, true, false)
        .arg(check_arg(true))
        .arg(Arg::new("json").long("json").action(ArgAction::SetTrue).help("UNSTABLE: Output benchmark result in JSON format"))
        .arg(
          Arg::new("ignore")
            .long("ignore")
            .num_args(1..)
        ...
        .arg(Arg::new("filter").long("filter").allow_hyphen_values(true).help("Run benchmarks with this string or pattern in the bench name"))
        .arg(Arg::new("files").help("List of file names to run").num_args(..).value_parser(value_parser!(PathBuf)).action(ArgAction::Append))
        .arg(Arg::new("no-run").long("no-run").help("Cache bench modules, but don't run benchmarks").action(ArgAction::SetTrue))
        .arg(watch_arg(false))
...
--reload is specified.",
    ...
    .defer(|cmd| compile_args(cmd).arg(check_arg(false)).arg(Arg::new("file").num_args(1..).required(true).value_hint(ValueHint::FilePath)))
}
...
Unless --reload is specified, this command will not re-download already cached dependencies.",
    ...
    .defer(|cmd| {
      compile_args_without_check_args(cmd)
        ...
        .arg(
          // past alias for --all
          Arg::new("remote").long("remote").help("Type-check all modules, including remote").action(ArgAction::SetTrue).conflicts_with("no-remote").hide(true),
        )
    ...
    times, to include multiple additional modules.",
        ...
        .arg(
          Arg::new("target")
            ...
            .value_parser(["x86_64-unknown-linux-gnu", "x86_64-pc-windows-msvc", "x86_64-apple-darwin", "aarch64-apple-darwin"]),
        )
        .arg(Arg::new("no-terminal").long("no-terminal").help("Hide terminal on Windows").action(ArgAction::SetTrue))
        .arg(executable_ext_arg())
        .arg(env_file_arg())
...
fn completions_subcommand() -> Command {
  ...
  source /usr/local/etc/bash_completion.d/deno.bash",
    ...
    .defer(|cmd| cmd.disable_help_subcommand(true).arg(Arg::new("shell").value_parser(["bash", "fish", "powershell", "zsh", "fig"]).required(true)))
}
...
",
    ...
    .defer(|cmd| {
      cmd
        ...
        .arg(Arg::new("lcov").long("lcov").help("Output coverage report in lcov format").action(ArgAction::SetTrue))
        .arg(
          Arg::new("output")
            .requires("lcov")
            .long("output")
    ...
    If no --output arg is specified then the report is written to stdout.",
        ...
        .arg(Arg::new("files").num_args(1..).value_parser(value_parser!(PathBuf)).action(ArgAction::Append).required(true).value_hint(ValueHint::AnyPath))
    })
}
...
Show documentation for runtime built-ins:
    ...
    .defer(|cmd| {
      cmd
        ...
        .arg(no_npm_arg())
        .arg(no_remote_arg())
        .arg(Arg::new("json").long("json").help("Output documentation in JSON format").action(ArgAction::SetTrue))
        .arg(Arg::new("html").long("html").help("Output documentation in HTML format").action(ArgAction::SetTrue).conflicts_with("json"))
        .arg(
          Arg::new("name")
            .long("name")
        ...
        .arg(Arg::new("private").long("private").help("Output private documentation").action(ArgAction::SetTrue))
        .arg(
          Arg::new("filter")
            .long("filter")
        ...
        .arg(Arg::new("lint").long("lint").help("Output documentation diagnostics.").action(ArgAction::SetTrue))
    ...
    .defer(|cmd| {
      runtime_args(cmd, false, true)
        ...
        .arg(
          ...
          Arg::new("ts").conflicts_with("ext").long("ts").short('T').help("deprecated: Treat eval input as TypeScript").action(ArgAction::SetTrue).hide(true),
        )
        .arg(executable_ext_arg())
        .arg(Arg::new("print").long("print").short('p').help("print result to stdout").action(ArgAction::SetTrue))
        .arg(Arg::new("code_arg").num_args(1..).action(ArgAction::Append).help("Code arg").value_name("CODE_ARG").required(true))
        .arg(env_file_arg())
    })
}
  ...
  // deno-fmt-ignore-file",
    ...
    .defer(|cmd| {
      cmd
        .arg(config_arg())
        .arg(no_config_arg())
        .arg(Arg::new("check").long("check").help("Check if the source files are formatted").num_args(0))
        .arg(
          Arg::new("ext")
            .long("ext")
        ...
        .arg(Arg::new("files").value_parser(value_parser!(PathBuf)).num_args(1..).action(ArgAction::Append).required(false).value_hint(ValueHint::AnyPath))
        .arg(watch_arg(false))
        .arg(no_clear_screen_arg())
        .arg(
          Arg::new("use-tabs")
...
fn init_subcommand() -> Command {
  Command::new("init").about("Initialize a new project").defer(|cmd| cmd.arg(Arg::new("dir").required(false).value_hint(ValueHint::DirPath)))
}
...
TypeScript compiler cache: Subdirectory containing TS compiler output.",
    ...
    .defer(|cmd| {
      cmd
        ...
        .arg(vendor_arg())
        .arg(Arg::new("json").long("json").help("UNSTABLE: Outputs the information in JSON format").action(ArgAction::SetTrue))
    })
}
...
These must be added to the path manually if required.",
    ...
    .defer(|cmd| {
      runtime_args(cmd, true, true)
        ...
        .arg(Arg::new("root").long("root").help("Installation root").value_hint(ValueHint::DirPath))
        .arg(Arg::new("force").long("force").short('f').help("Forcefully overwrite existing installation").action(ArgAction::SetTrue))
    })
    .arg(env_file_arg())
}
...
fn jupyter_subcommand() -> Command {
  Command::new("jupyter")
    ...
    .arg(Arg::new("kernel").long("kernel").help("Start the kernel").conflicts_with("install").requires("conn").action(ArgAction::SetTrue))
    .arg(
      Arg::new("conn")
        .long("conn")
...
The installation root is determined, in order of precedence:
  ...
  - $HOME/.deno",
    ...
    .defer(|cmd| cmd.arg(Arg::new("name").required(true)).arg(Arg::new("root").long("root").help("Installation root").value_hint(ValueHint::DirPath)))
}
...
fn lsp_subcommand() -> Command {
  Command::new("lsp").about("Start the language server").long_about(LSP_HELP)
}
...
",
    ...
    .defer(|cmd| {
      cmd
        ...
        .arg(Arg::new("json").long("json").help("Output lint result in JSON format").action(ArgAction::SetTrue))
        .arg(Arg::new("compact").long("compact").help("Output lint result in compact format").action(ArgAction::SetTrue).conflicts_with("json"))
        .arg(Arg::new("files").value_parser(value_parser!(PathBuf)).num_args(1..).action(ArgAction::Append).required(false).value_hint(ValueHint::AnyPath))
        .arg(watch_arg(false))
        .arg(no_clear_screen_arg())
    })
  ...
  Command::new("repl")
    ...
    .defer(|cmd| {
      runtime_args(cmd, true, true)
        ...
        .arg(Arg::new("eval").long("eval").help("Evaluates the provided code when the REPL starts.").value_name("code"))
    })
    .arg(env_file_arg())
}
  ...
  deno task build",
    ...
    .defer(|cmd| {
      cmd
        ...
        .arg(config_arg())
        .arg(Arg::new("cwd").long("cwd").value_name("DIR").help("Specify the directory to run the task in").value_hint(ValueHint::DirPath))
    })
}
  ...
  deno test src/",
    ...
    .defer(|cmd| {
      runtime_args(cmd, true, true)
        ...
        .arg(Arg::new("no-run").long("no-run").help("Cache test modules, but don't run tests").action(ArgAction::SetTrue))
        .arg(
          Arg::new("trace-ops")
            .long("trace-ops")
        ...
        .arg(Arg::new("doc").long("doc").help("Type-check code blocks in JSDoc and Markdown").action(ArgAction::SetTrue))
        .arg(
          Arg::new("fail-fast")
            .long("fail-fast")
            .alias("failfast")
        ...
        .arg(Arg::new("allow-none").long("allow-none").help("Don't return error code if no test files are found").action(ArgAction::SetTrue))
        .arg(Arg::new("filter").allow_hyphen_values(true).long("filter").help("Run tests with this string or pattern in the test name"))
        .arg(
          Arg::new("shuffle")
            .long("shuffle")
            .value_name("NUMBER")
        ...
        .arg(watch_arg(false).conflicts_with("no-run").conflicts_with("coverage"))
        .arg(no_clear_screen_arg())
        .arg(script_arg().last(true))
        .arg(
          Arg::new("junit-path")
        ...
        .arg(Arg::new("reporter").long("reporter").help("Select reporter to use. Default to 'pretty'.").value_parser(["pretty", "dot", "junit", "tap"]))
        .arg(env_file_arg())
    })
}
  ...
  deno upgrade --output $HOME/my_deno",
    ...
    .defer(|cmd| {
      cmd
        ...
        .arg(Arg::new("dry-run").long("dry-run").help("Perform all checks without replacing old exe").action(ArgAction::SetTrue))
        .arg(Arg::new("force").long("force").short('f').help("Replace current exe even if not out-of-date").action(ArgAction::SetTrue))
        .arg(Arg::new("canary").long("canary").help("Upgrade to canary builds").action(ArgAction::SetTrue))
        .arg(ca_file_arg())
    })
}
  ...
  deno vendor main.ts test.deps.ts https://deno.land/std/path/mod.ts",
    ...
    .defer(|cmd| {
      cmd
        ...
        .arg(Arg::new("force").long("force").short('f').help("Forcefully overwrite conflicting files in existing output directory").action(ArgAction::SetTrue))
        .arg(no_config_arg())
        .arg(config_arg())
        .arg(import_map_arg())
        .arg(lock_arg())
...
fn publish_subcommand() -> Command {
  Command::new("do-not-use-publish")
    ...
    .defer(|cmd| {
      cmd
        .arg(Arg::new("directory").help("The directory to the package, or workspace of packages to publish").value_hint(ValueHint::DirPath).required(true))
        .arg(Arg::new("token").help("The API token to use when publishing. If unset, interactive authentication will be used."))
    })
}
...
fn permission_args(app: Command) -> Command {
  app
    ...
    .arg(Arg::new("allow-run").long("allow-run").num_args(0..).use_value_delimiter(true).require_equals(true).value_name("PROGRAM_NAME").help(ALLOW_RUN_HELP))
    .arg(Arg::new("deny-run").long("deny-run").num_args(0..).use_value_delimiter(true).require_equals(true).value_name("PROGRAM_NAME").help(DENY_RUN_HELP))
    .arg(
      Arg::new("allow-ffi")
        .long("allow-ffi")
        .num_args(0..)
    ...
    .arg(Arg::new("allow-hrtime").long("allow-hrtime").action(ArgAction::SetTrue).help(ALLOW_HRTIME_HELP))
    .arg(Arg::new("deny-hrtime").long("deny-hrtime").action(ArgAction::SetTrue).help(DENY_HRTIME_HELP))
    .arg(Arg::new("allow-all").short('A').long("allow-all").action(ArgAction::SetTrue).help(ALLOW_ALL_HELP))
    .arg(Arg::new("prompt").long("prompt").action(ArgAction::SetTrue).hide(true).help("deprecated: Fallback to prompt if required permission wasn't passed"))
    .arg(Arg::new("no-prompt").long("no-prompt").action(ArgAction::SetTrue).help("Always throw if required permission wasn't passed"))
}
...
fn runtime_args(app: Command, include_perms: bool, include_inspector: bool) -> Command {
  ...
  app.arg(cached_only_arg()).arg(location_arg()).arg(v8_flags_arg()).arg(seed_arg()).arg(enable_testing_features_arg()).arg(strace_ops_arg())
}
...
fn ca_file_arg() -> Arg {
  Arg::new("cert").long("cert").value_name("FILE").help("Load certificate authority from PEM encoded file").value_hint(ValueHint::FilePath)
}

fn cached_only_arg() -> Arg {
  Arg::new("cached-only").long("cached-only").action(ArgAction::SetTrue).help("Require that remote dependencies are already cached")
}
...
fn executable_ext_arg() -> Arg {
  Arg::new("ext").long("ext").help("Set content type of the supplied file").value_parser(["ts", "tsx", "js", "jsx"])
}
...
fn v8_flags_arg() -> Arg {
  Arg::new("v8-flags").long("v8-flags").num_args(..).use_value_delimiter(true).require_equals(true).help("Set V8 command line options").long_help(
...
fn seed_arg() -> Arg {
  Arg::new("seed").long("seed").value_name("NUMBER").help("Set the random number generator seed").value_parser(value_parser!(u64))
}

fn hmr_arg(takes_files: bool) -> Arg {
  let arg = Arg::new("hmr").long("unstable-hmr").help("UNSTABLE: Watch for file changes and hot replace modules").conflicts_with("watch");
...
fn no_check_arg() -> Arg {
  Arg::new("no-check").num_args(0..=1).require_equals(true).value_name("NO_CHECK_TYPE").long("no-check").help("Skip type-checking modules").long_help(
...
fn check_arg(checks_local_by_default: bool) -> Arg {
  let arg = Arg::new("check").conflicts_with("no-check").long("check").num_args(0..=1).require_equals(true).value_name("CHECK_TYPE").help("Type-check modules");
...
fn lock_write_arg() -> Arg {
  Arg::new("lock-write").action(ArgAction::SetTrue).long("lock-write").help("Force overwriting the lock file.").conflicts_with("no-lock")
}

fn no_lock_arg() -> Arg {
  Arg::new("no-lock").long("no-lock").action(ArgAction::SetTrue).help("Disable auto discovery of the lock file.").conflicts_with("lock")
}
...
fn no_remote_arg() -> Arg {
  Arg::new("no-remote").long("no-remote").action(ArgAction::SetTrue).help("Do not resolve remote modules")
}

fn no_npm_arg() -> Arg {
  Arg::new("no-npm").long("no-npm").action(ArgAction::SetTrue).help("Do not resolve npm modules")
}
...
fn bench_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  ...
  if matches.contains_id("script_arg") {
    flags.argv.extend(matches.remove_many::<String>("script_arg").unwrap());
  }

  let include = if let Some(files) = matches.remove_many::<PathBuf>("files") { files.collect() } else { Vec::new() };
...
fn bundle_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  ...
  let out_file = if let Some(out_file) = matches.remove_one::<PathBuf>("out_file") {
    flags.allow_write = Some(vec![]);
    Some(out_file)
  } else {
    None
...
fn coverage_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  ...
  flags.subcommand = DenoSubcommand::Coverage(CoverageFlags {
    files: FileFlags { include: files, ignore },
    output,
    include,
    exclude,
    lcov,
...
fn doc_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  ...
  let html = if matches.get_flag("html") {
    ...
    let output = matches.remove_one::<PathBuf>("output").unwrap_or(PathBuf::from("./docs/"));
...
fn eval_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  ...
  if as_typescript {
    eprintln!("{}", crate::colors::yellow("Warning: --ts/-T flag is deprecated. Use --ext=ts instead."),);
...
fn install_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  ...
  flags.subcommand = DenoSubcommand::Install(InstallFlags { name, module_url, args, root, force });
}
...
fn run_parse(flags: &mut Flags, matches: &mut ArgMatches, app: Command) -> clap::error::Result<()> {
  runtime_args_parse(flags, matches, true, true);

  let mut script_arg = matches.remove_many::<String>("script_arg").ok_or_else(|| {
    let mut app = app;
    let subcommand = &mut app.find_subcommand_mut("run").unwrap();
    subcommand.error(clap::error::ErrorKind::MissingRequiredArgument, "[SCRIPT_ARG] may only be omitted with --v8-flags=--help")
  })?;
...
fn task_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  flags.config_flag = matches.remove_one::<String>("config").map(ConfigFlag::Path).unwrap_or(ConfigFlag::Discover);
  ...
  if let Some((task, mut matches)) = matches.remove_subcommand() {
    ...
    flags.argv.extend(matches.remove_many::<std::ffi::OsString>("").into_iter().flatten().filter_map(|arg| arg.into_string().ok()));
  }
...
fn test_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  ...
  let shuffle = if matches.contains_id("shuffle") { Some(matches.remove_one::<u64>("shuffle").unwrap_or_else(rand::random)) } else { None };
  ...
  let concurrent_jobs = if matches.get_flag("parallel") {
    ...
    eprintln!("{}", crate::colors::yellow("Warning: --jobs flag is deprecated. Use the --parallel flag with possibly the 'DENO_JOBS' environment variable."),);
...
fn vendor_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  ...
  flags.subcommand = DenoSubcommand::Vendor(VendorFlags {
    specifiers: matches.remove_many::<String>("specifiers").map(|p| p.collect()).unwrap_or_default(),
...
fn inspect_arg_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  ...
  flags.inspect = if matches.contains_id("inspect") { Some(matches.remove_one::<SocketAddr>("inspect").unwrap_or_else(default)) } else { None };
...
fn watch_arg_parse_with_paths(matches: &mut ArgMatches) -> Option<WatchFlagsWithPaths> {
  ...
  matches.remove_many::<PathBuf>("hmr").map(|paths| WatchFlagsWithPaths {
    paths: paths.collect(),
    hmr: true,
    no_clear_screen: matches.get_flag("no-clear-screen"),
  })
