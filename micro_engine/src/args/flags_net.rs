...
pub fn validator(host_and_port: &str) -> Result<String, String> {
  if Url::parse(&format!("internal://{host_and_port}")).is_ok() || host_and_port.parse::<IpAddr>().is_ok() || host_and_port.parse::<BarePort>().is_ok() {
...
pub fn parse(paths: Vec<String>) -> clap::error::Result<Vec<String>> {
  ...
  for host_and_port in paths.iter() {
    if Url::parse(&format!("internal://{host_and_port}")).is_ok() || host_and_port.parse::<IpAddr>().is_ok() {
      ...
      return Err(clap::Error::raw(clap::error::ErrorKind::InvalidValue, format!("Bad host:port pair: {host_and_port}")));
    }
  }
  Ok(out)
}
