...
pub async fn resolve_import_map_from_specifier(specifier: &Url, maybe_config_file: Option<&ConfigFile>, file_fetcher: &FileFetcher) -> Result<ImportMap, AnyError> {
  let value: serde_json::Value = if specifier.scheme() == "data" {
    serde_json::from_str(&get_source_from_data_url(specifier)?.0)?
  } else {
    let import_map_config = maybe_config_file.as_ref().filter(|c| c.specifier == *specifier);
...
pub fn import_map_from_value(specifier: &Url, json_value: serde_json::Value) -> Result<ImportMap, AnyError> {
  debug_assert!(!specifier.as_str().contains("../"), "Import map specifier incorrectly contained ../: {}", specifier.as_str());
