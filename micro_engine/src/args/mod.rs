...
impl BenchOptions {
  pub fn resolve(maybe_bench_config: Option<BenchConfig>, maybe_bench_flags: Option<BenchFlags>) -> Result<Self, AnyError> {
...
impl FmtOptions {
  pub fn resolve(maybe_fmt_config: Option<FmtConfig>, maybe_fmt_flags: Option<FmtFlags>) -> Result<Self, AnyError> {
    let (maybe_config_options, maybe_config_files) = maybe_fmt_config.map(|c| (c.options, c.files)).unzip();
    ...
    Ok(Self {
      ...
      files: resolve_files(maybe_config_files, maybe_fmt_flags.map(|f| f.files))?,
    })
  }
}

fn resolve_fmt_options(fmt_flags: Option<&FmtFlags>, options: Option<FmtOptionsConfig>) -> FmtOptionsConfig {
...
impl TestOptions {
  pub fn resolve(maybe_test_config: Option<TestConfig>, maybe_test_flags: Option<TestFlags>) -> Result<Self, AnyError> {
    ...
    Ok(Self {
      ...
      concurrent_jobs: test_flags.concurrent_jobs.unwrap_or_else(|| NonZeroUsize::new(1).unwrap()),
      doc: test_flags.doc,
...
impl LintOptions {
  pub fn resolve(maybe_lint_config: Option<LintConfig>, maybe_lint_flags: Option<LintFlags>) -> Result<Self, AnyError> {
    ...
    let (maybe_file_flags, maybe_rules_tags, maybe_rules_include, maybe_rules_exclude) = maybe_lint_flags.map(|f| (f.files, f.maybe_rules_tags, f.maybe_rules_include, f.maybe_rules_exclude)).unwrap_or_default();

    let (maybe_config_files, maybe_config_rules) = maybe_lint_config.map(|c| (c.files, c.rules)).unzip();
    Ok(Self {
      reporter_kind: maybe_reporter_kind.unwrap_or_default(),
      files: resolve_files(maybe_config_files, Some(maybe_file_flags))?,
      rules: resolve_lint_rules_options(maybe_config_rules, maybe_rules_tags, maybe_rules_include, maybe_rules_exclude),
    })
  }
}

fn resolve_lint_rules_options(maybe_lint_rules_config: Option<LintRulesConfig>, mut maybe_rules_tags: Option<Vec<String>>, mut maybe_rules_include: Option<Vec<String>>, mut maybe_rules_exclude: Option<Vec<String>>) -> LintRulesConfig {
...
fn discover_package_json(flags: &Flags, maybe_stop_at: Option<PathBuf>, current_dir: &Path) -> Result<Option<PackageJson>, AnyError> {
...
impl CliRootCertStoreProvider {
  pub fn new(maybe_root_path: Option<PathBuf>, maybe_ca_stores: Option<Vec<String>>, maybe_ca_data: Option<CaData>) -> Self {
    Self {
...
impl RootCertStoreProvider for CliRootCertStoreProvider {
  fn get_or_try_init(&self) -> Result<&RootCertStore, AnyError> {
    self
      .cell
      .get_or_try_init(|| get_root_cert_store(self.maybe_root_path.clone(), self.maybe_ca_stores.clone(), self.maybe_ca_data.clone()))
      .map_err(|e| e.into())
  }
}
...
/// environment.
pub fn get_root_cert_store(maybe_root_path: Option<PathBuf>, maybe_ca_stores: Option<Vec<String>>, maybe_ca_data: Option<CaData>) -> Result<RootCertStore, RootCertStoreLoadError> {
  let mut root_cert_store = RootCertStore::empty();
  let ca_stores: Vec<String> = maybe_ca_stores
    .or_else(|| {
      let env_ca_store = env::var("DENO_TLS_CA_STORE").ok()?;
      Some(env_ca_store.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect())
    })
  ...
  for store in ca_stores.iter() {
    match store.as_str() {
      "mozilla" => {
        root_cert_store.add_trust_anchors(webpki_roots::TLS_SERVER_ROOTS.iter().map(|ta| rustls::OwnedTrustAnchor::from_subject_spki_name_constraints(ta.subject, ta.spki, ta.name_constraints)));
      }
      "system" => {
        let roots = load_native_certs().expect("could not load platform certs");
        for root in roots {
          root_cert_store.add(&rustls::Certificate(root.0)).expect("Failed to add platform cert to root cert store");
        }
      }
      _ => {
  ...
  if let Some(ca_data) = ca_data {
    let result = match ca_data {
      CaData::File(ca_file) => {
        let ca_file = if let Some(root) = &maybe_root_path { root.join(&ca_file) } else { PathBuf::from(ca_file) };
        let certfile = std::fs::File::open(ca_file).map_err(|err| RootCertStoreLoadError::CaFileOpenError(err.to_string()))?;
...
impl CliOptions {
  pub fn new(flags: Flags, initial_cwd: PathBuf, maybe_config_file: Option<ConfigFile>, maybe_lockfile: Option<Arc<Mutex<Lockfile>>>, maybe_package_json: Option<PackageJson>) -> Result<Self, AnyError> {
    ...
    let maybe_node_modules_folder = resolve_node_modules_folder(&initial_cwd, &flags, maybe_config_file.as_ref(), maybe_package_json.as_ref()).with_context(|| "Resolving node_modules folder.")?;
    let maybe_vendor_folder = resolve_vendor_folder(&initial_cwd, &flags, maybe_config_file.as_ref());
    let maybe_workspace_config = if let Some(config_file) = maybe_config_file.as_ref() { config_file.to_workspace_config()? } else { None };
  ...
  pub fn from_flags(flags: Flags) -> Result<Self, AnyError> {
    ...
    let maybe_config_file = ConfigFile::discover(&flags.config_flag, flags.config_path_args(&initial_cwd), &initial_cwd)?;

    let mut maybe_package_json = None;
    if flags.config_flag == deno_config::ConfigFlag::Disabled || flags.no_npm || has_flag_env_var("DENO_NO_PACKAGE_JSON") {
      log::debug!("package.json auto-discovery is disabled")
    } else if let Some(config_file) = &maybe_config_file {
      let specifier = config_file.specifier.clone();
      if specifier.scheme() == "file" {
        let maybe_stop_at = specifier.to_file_path().unwrap().parent().map(|p| p.to_path_buf());
    ...
    Self::new(flags, initial_cwd, maybe_config_file, maybe_lock_file.map(|l| Arc::new(Mutex::new(l))), maybe_package_json)
  }
  ...
  pub fn ts_type_lib_window(&self) -> TsTypeLib {
    if self.flags.unstable || !self.flags.unstable_features.is_empty() || self.maybe_config_file.as_ref().map(|f| !f.json.unstable.is_empty()).unwrap_or(false) {
  ...
  pub fn npm_system_info(&self) -> NpmSystemInfo {
    match self.sub_command() {
      DenoSubcommand::Compile(CompileFlags { target: Some(target), .. }) => {
  ...
  pub fn resolve_import_map_specifier(&self) -> Result<Option<ModuleSpecifier>, AnyError> {
    match self.overrides.import_map_specifier.clone() {
      ...
      None => resolve_import_map_specifier(self.flags.import_map_path.as_deref(), self.maybe_config_file.as_ref(), &self.initial_cwd),
    }
  }

  pub async fn resolve_import_map(&self, file_fetcher: &FileFetcher) -> Result<Option<ImportMap>, AnyError> {
    if let Some(workspace_config) = self.maybe_workspace_config.as_ref() {
      ...
      let maybe_import_map = ::import_map::ext::create_synthetic_import_map(base_import_map_config, children_configs);
      if let Some((_import_map_url, import_map)) = maybe_import_map {
        log::debug!("Workspace config generated this import map {}", serde_json::to_string_pretty(&import_map).unwrap());
    ...
    resolve_import_map_from_specifier(&import_map_specifier, self.maybe_config_file().as_ref(), file_fetcher)
      .await
  ...
  pub fn resolve_main_module(&self) -> Result<ModuleSpecifier, AnyError> {
    match &self.flags.subcommand {
      DenoSubcommand::Bundle(bundle_flags) => resolve_url_or_path(&bundle_flags.source_file, self.initial_cwd()).map_err(AnyError::from),
      DenoSubcommand::Compile(compile_flags) => resolve_url_or_path(&compile_flags.source_file, self.initial_cwd()).map_err(AnyError::from),
      DenoSubcommand::Eval(_) => resolve_url_or_path("./$deno$eval", self.initial_cwd()).map_err(AnyError::from),
      DenoSubcommand::Repl(_) => resolve_url_or_path("./$deno$repl.ts", self.initial_cwd()).map_err(AnyError::from),
      DenoSubcommand::Run(run_flags) => {
        if run_flags.is_stdin() {
          std::env::current_dir().context("Unable to get CWD").and_then(|cwd| resolve_url_or_path("./$deno$stdin.ts", &cwd).map_err(AnyError::from))
  ...
  pub fn resolve_file_header_overrides(&self) -> HashMap<ModuleSpecifier, HashMap<String, String>> {
    ...
    if let (Some(main_specifier), Some(content_type)) = (maybe_main_specifier, maybe_content_type) {
      HashMap::from([(main_specifier, HashMap::from([("content-type".to_string(), content_type.to_string())]))])
    } else {
      HashMap::default()
    }
  }

  pub fn resolve_npm_resolution_snapshot(&self) -> Result<Option<ValidSerializedNpmResolutionSnapshot>, AnyError> {
    if let Some(NpmProcessStateKind::Snapshot(snapshot)) = NPM_PROCESS_STATE.as_ref().map(|s| &s.kind) {
  ...
  pub fn node_modules_dir_enablement(&self) -> Option<bool> {
    self.flags.node_modules_dir.or_else(|| self.maybe_config_file.as_ref().and_then(|c| c.node_modules_dir_flag()))
  }
  ...
  pub fn resolve_root_cert_store_provider(&self) -> Arc<dyn RootCertStoreProvider> {
    Arc::new(CliRootCertStoreProvider::new(None, self.flags.ca_stores.clone(), self.flags.ca_data.clone()))
  }

  pub fn resolve_ts_config_for_emit(&self, config_type: TsConfigType) -> Result<TsConfigForEmit, AnyError> {
  ...
  pub fn resolve_inspector_server(&self) -> Option<InspectorServer> {
    let maybe_inspect_host = self.flags.inspect.or(self.flags.inspect_brk).or(self.flags.inspect_wait);
  ...
  pub fn to_maybe_imports(&self) -> Result<Vec<deno_graph::ReferrerImports>, AnyError> {
    if let Some(config_file) = &self.maybe_config_file {
      config_file
        .to_maybe_imports()
        .map(|maybe_imports| maybe_imports.into_iter().map(|(referrer, imports)| deno_graph::ReferrerImports { referrer, imports }).collect())
    } else {
      Ok(Vec::new())
    }
  }
  ...
  pub fn maybe_package_json_deps(&self) -> Option<PackageJsonDeps> {
    if matches!(self.flags.subcommand, DenoSubcommand::Task(TaskFlags { task: None, .. })) {
      // don't have any package json dependencies for deno task with no args
      None
    } else {
      self.maybe_package_json().as_ref().map(package_json::get_local_package_json_version_reqs)
    }
  }

  pub fn resolve_fmt_options(&self, fmt_flags: FmtFlags) -> Result<FmtOptions, AnyError> {
    let maybe_fmt_config = if let Some(config_file) = &self.maybe_config_file { config_file.to_fmt_config()? } else { None };
    FmtOptions::resolve(maybe_fmt_config, Some(fmt_flags))
  }

  pub fn resolve_lint_options(&self, lint_flags: LintFlags) -> Result<LintOptions, AnyError> {
    let maybe_lint_config = if let Some(config_file) = &self.maybe_config_file { config_file.to_lint_config()? } else { None };
    LintOptions::resolve(maybe_lint_config, Some(lint_flags))
  }

  pub fn resolve_test_options(&self, test_flags: TestFlags) -> Result<TestOptions, AnyError> {
    let maybe_test_config = if let Some(config_file) = &self.maybe_config_file { config_file.to_test_config()? } else { None };
  ...
  pub fn resolve_bench_options(&self, bench_flags: BenchFlags) -> Result<BenchOptions, AnyError> {
    let maybe_bench_config = if let Some(config_file) = &self.maybe_config_file { config_file.to_bench_config()? } else { None };
  ...
  pub fn check_js(&self) -> bool {
    self.maybe_config_file.as_ref().map(|cf| cf.get_check_js()).unwrap_or(false)
  }

  pub fn coverage_dir(&self) -> Option<String> {
    match &self.flags.subcommand {
      DenoSubcommand::Test(test) => test.coverage_dir.as_ref().map(ToOwned::to_owned).or_else(|| env::var("DENO_UNSTABLE_COVERAGE_DIR").ok()),
      _ => None,
    }
  }

  pub fn enable_op_summary_metrics(&self) -> bool {
    self.flags.enable_op_summary_metrics || matches!(self.flags.subcommand, DenoSubcommand::Test(_) | DenoSubcommand::Repl(_))
  }
  ...
  pub fn is_inspecting(&self) -> bool {
    self.flags.inspect.is_some() || self.flags.inspect_brk.is_some() || self.flags.inspect_wait.is_some()
  }
  ...
  pub fn is_quiet(&self) -> bool {
    self.log_level().map(|l| l == log::Level::Error).unwrap_or(false)
  }
  ...
  pub fn unstable_bare_node_builtins(&self) -> bool {
    self.flags.unstable_bare_node_builtins || self.maybe_config_file().as_ref().map(|c| c.json.unstable.contains(&"bare-node-builtins".to_string())).unwrap_or(false)
  }

  pub fn unstable_byonm(&self) -> bool {
    self.flags.unstable_byonm || NPM_PROCESS_STATE.as_ref().map(|s| matches!(s.kind, NpmProcessStateKind::Byonm)).unwrap_or(false) || self.maybe_config_file().as_ref().map(|c| c.json.unstable.iter().any(|c| c == "byonm")).unwrap_or(false)
  }

  pub fn unstable_features(&self) -> Vec<String> {
    let mut from_config_file = self.maybe_config_file().as_ref().map(|c| c.json.unstable.clone()).unwrap_or_default();
  ...
  pub fn watch_paths(&self) -> Vec<PathBuf> {
    ...
    if let Ok(Some(import_map_path)) = self.resolve_import_map_specifier().map(|ms| ms.and_then(|ref s| s.to_file_path().ok())) {
...
fn resolve_node_modules_folder(cwd: &Path, flags: &Flags, maybe_config_file: Option<&ConfigFile>, maybe_package_json: Option<&PackageJson>) -> Result<Option<PathBuf>, AnyError> {
  ...
  let path = if use_node_modules_dir == Some(false) {
    ...
    return Ok(None);
  } else if let Some(config_path) = maybe_config_file.as_ref().and_then(|c| c.specifier.to_file_path().ok()) {
...
fn resolve_vendor_folder(cwd: &Path, flags: &Flags, maybe_config_file: Option<&ConfigFile>) -> Option<PathBuf> {
  let use_vendor_dir = flags.vendor.or_else(|| maybe_config_file.and_then(|c| c.vendor_dir_flag())).unwrap_or(false);
...
fn resolve_import_map_specifier(maybe_import_map_path: Option<&str>, maybe_config_file: Option<&ConfigFile>, current_dir: &Path) -> Result<Option<ModuleSpecifier>, AnyError> {
  if let Some(import_map_path) = maybe_import_map_path {
    ...
    let specifier = deno_core::resolve_url_or_path(import_map_path, current_dir).with_context(|| format!("Bad URL (\"{import_map_path}\") for import map."))?;
...
impl StorageKeyResolver {
  pub fn from_options(options: &CliOptions) -> Self {
    Self(if let Some(location) = &options.flags.location {
      ...
      options.maybe_config_file.as_ref().map(|config_file| Some(config_file.specifier.to_string()))
    })
  }
...
fn resolve_files(maybe_files_config: Option<FilesConfig>, maybe_file_flags: Option<FileFlags>) -> Result<FilesConfig, AnyError> {
