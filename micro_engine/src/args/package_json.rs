...
impl PackageJsonDepsProvider {
  ...
  pub fn reqs(&self) -> Vec<&PackageReq> {
    match &self.0 {
      Some(deps) => {
        let mut package_reqs = deps.values().filter_map(|r| r.as_ref().ok()).collect::<Vec<_>>();
...
pub fn get_local_package_json_version_reqs(package_json: &PackageJson) -> PackageJsonDeps {
  fn parse_entry(key: &str, value: &str) -> Result<PackageReq, PackageJsonDepValueParseError> {
    if value.starts_with("workspace:") || value.starts_with("file:") || value.starts_with("git:") || value.starts_with("http:") || value.starts_with("https:") {
      return Err(PackageJsonDepValueParseError::Unsupported {
        scheme: value.split(':').next().unwrap().to_string(),
      });
    }
    let (name, version_req) = parse_dep_entry_name_and_raw_version(key, value).map_err(PackageJsonDepValueParseError::SchemeValue)?;

    let result = VersionReq::parse_from_specifier(version_req);
    match result {
      Ok(version_req) => Ok(PackageReq { name: name.to_string(), version_req }),
  ...
  fn insert_deps(deps: Option<&IndexMap<String, String>>, result: &mut PackageJsonDeps) {
    if let Some(deps) = deps {
      for (key, value) in deps {
        result.entry(key.to_string()).or_insert_with(|| parse_entry(key, value));
      }
    }
  }
...
pub fn discover_from(start: &Path, maybe_stop_at: Option<PathBuf>) -> Result<Option<PackageJson>, AnyError> {
  ...
  for ancestor in start.ancestors() {
    ...
    let source = match std::fs::read_to_string(&path) {
      ...
      Err(err) => bail!("Error loading package.json at {}. {:#}", path.display(), err),
    };
