...
impl CacheDB {
  ...
  pub fn from_path(config: &'static CacheDBConfiguration, path: PathBuf, version: &'static str) -> Self {
  ...
  fn actually_open_connection(&self, path: &Option<PathBuf>) -> Result<Connection, rusqlite::Error> {
    match path {
  ...
  fn initialize_connection(config: &CacheDBConfiguration, conn: &Connection, version: &str) -> Result<(), AnyError> {
    let sql = config.create_combined_sql();
    conn.execute_batch(&sql)?;

    // Check the version
    let existing_version = conn.query_row("SELECT value FROM info WHERE key='CLI_VERSION' LIMIT 1", [], |row| row.get::<_, String>(0)).optional()?.unwrap_or_default();
  ...
  fn open_connection(&self) -> Result<ConnectionState, AnyError> {
    ...
    log::trace!("Could not initialize cache database '{}', retrying... ({err:?})", path.to_string_lossy(),);
    ...
    match self.config.on_failure {
      CacheFailure::InMemory => {
        log::log!(if is_tty { log::Level::Error } else { log::Level::Trace }, "Failed to open cache file '{}', opening in-memory cache.", path.to_string_lossy());
        Ok(ConnectionState::Connected(self.open_connection_and_init(&None)?))
      }
      CacheFailure::Blackhole => {
        log::log!(if is_tty { log::Level::Error } else { log::Level::Trace }, "Failed to open cache file '{}', performance may be degraded.", path.to_string_lossy());
        Ok(ConnectionState::Blackhole)
      }
      CacheFailure::Error => {
        log::error!("Failed to open cache file '{}', expect further errors.", path.to_string_lossy());
        Err(err)
      }
    }
  }
  ...
  pub fn with_connection<T: Default>(&self, f: impl FnOnce(&Connection) -> Result<T, AnyError>) -> Result<T, AnyError> {
  ...
  pub fn query_row<T, F>(&self, sql: &'static str, params: impl Params, f: F) -> Result<Option<T>, AnyError>
  where
