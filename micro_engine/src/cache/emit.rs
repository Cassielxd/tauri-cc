...
impl EmitCache {
  ...
  pub fn get_emit_code(&self, specifier: &ModuleSpecifier, expected_source_hash: u64) -> Option<String> {
  ...
  pub fn get_emit_filepath(&self, specifier: &ModuleSpecifier) -> Option<PathBuf> {
    Some(self.disk_cache.location.join(self.get_emit_filename(specifier)?))
  }
  ...
  fn set_emit_code_result(&self, specifier: &ModuleSpecifier, source_hash: u64, code: &str) -> Result<(), AnyError> {
    let meta_filename = self.get_meta_filename(specifier).ok_or_else(|| anyhow!("Could not get meta filename."))?;
    let emit_filename = self.get_emit_filename(specifier).ok_or_else(|| anyhow!("Could not get emit filename."))?;
    ...
    self.disk_cache.set(&meta_filename, &serde_json::to_vec(&metadata)?)?;
  ...
  fn get_meta_filename(&self, specifier: &ModuleSpecifier) -> Option<PathBuf> {
    self.disk_cache.get_cache_filename_with_extension(specifier, "meta")
  }

  fn get_emit_filename(&self, specifier: &ModuleSpecifier) -> Option<PathBuf> {
    self.disk_cache.get_cache_filename_with_extension(specifier, "js")
  }
}
