...
impl IncrementalCache {
  pub fn new<TState: Serialize>(db: CacheDB, state: &TState, initial_file_paths: &[PathBuf]) -> Self {
...
impl IncrementalCacheInner {
  ...
  fn from_sql_incremental_cache(cache: SqlIncrementalCache, initial_file_paths: &[PathBuf]) -> Self {
  ...
  pub fn update_file(&self, file_path: &Path, file_text: &str) {
    ...
    let _ = self.sender.send(ReceiverMessage::Update(file_path.to_path_buf(), hash));
  }
...
impl SqlIncrementalCache {
  ...
  fn get_source_hash_result(&self, path: &Path) -> Result<Option<u64>, AnyError> {
    let query = "
      ...
      WHERE
        ...
        AND state_hash=?2
      LIMIT 1";
    let res = self.conn.query_row(query, params![path.to_string_lossy(), self.state_hash.to_string()], |row| {
  ...
  pub fn set_source_hash(&self, path: &Path, source_hash: u64) -> Result<(), AnyError> {
    let sql = "
      ...
      VALUES
        (?1, ?2, ?3)";
    self.conn.execute(sql, params![path.to_string_lossy(), &self.state_hash.to_string(), &source_hash,])?;
    Ok(())
  }
}
