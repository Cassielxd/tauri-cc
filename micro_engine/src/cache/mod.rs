...
impl deno_cache_dir::DenoCacheEnv for RealDenoCacheEnv {
  ...
  fn modified(&self, path: &Path) -> std::io::Result<Option<SystemTime>> {
    match std::fs::metadata(path) {
      Ok(metadata) => Ok(Some(metadata.modified().unwrap_or_else(|_| SystemTime::now()))),
      Err(err) if err.kind() == std::io::ErrorKind::NotFound => Ok(None),
      Err(err) => Err(err),
    }
  }
...
impl FetchCacher {
  ...
  #[deprecated(note = "There should not be a way to do this because the file may not be cached at a local path in the future.")]
  fn get_local_path(&self, specifier: &ModuleSpecifier) -> Option<PathBuf> {
    ...
    if specifier.scheme() == "flags" {
      ...
      #[allow(deprecated)]
      self.global_http_cache.get_global_cache_filepath(specifier).ok()
    }
  }
}
...
impl Loader for FetchCacher {
  ...
  fn get_cache_info(&self, specifier: &ModuleSpecifier) -> Option<CacheInfo> {
    ...
    if local.is_file() {
      let emit = self.emit_cache.get_emit_filepath(specifier).filter(|p| p.is_file());
      Some(CacheInfo { local: Some(local), emit, map: None })
    } else {
      None
    }
  }

  fn load(&mut self, specifier: &ModuleSpecifier, _is_dynamic: bool, cache_setting: deno_graph::source::CacheSetting) -> LoadFuture {
      ...
      let maybe_cache_setting = match cache_setting {
        ...
        LoaderCacheSetting::Reload => {
          if matches!(file_fetcher.cache_setting(), CacheSetting::Only) {
            return Err(deno_core::anyhow::anyhow!("Failed to resolve version constraint. Try running again without --cached-only"));
          }
  ...
  fn cache_module_info(&mut self, specifier: &ModuleSpecifier, source: &str, module_info: &deno_graph::ModuleInfo) {
    let result = self.module_info_cache.set_module_info(specifier, MediaType::from_specifier(specifier), source, module_info);
