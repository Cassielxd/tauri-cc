...
impl NodeAnalysisCache {
  pub fn new(db: CacheDB) -> Self {
    Self { inner: NodeAnalysisCacheInner::new(db) }
  }
  ...
  pub fn get_cjs_analysis(&self, specifier: &str, expected_source_hash: &str) -> Option<CjsAnalysis> {
  ...
  pub fn set_cjs_analysis(&self, specifier: &str, source_hash: &str, cjs_analysis: &CjsAnalysis) {
    Self::ensure_ok(self.inner.set_cjs_analysis(specifier, source_hash, cjs_analysis));
  }
}
...
impl NodeAnalysisCacheInner {
  ...
  pub fn get_cjs_analysis(&self, specifier: &str, expected_source_hash: &str) -> Result<Option<CjsAnalysis>, AnyError> {
    let query = "
      SELECT
        data
      FROM
      ...
      WHERE
        ...
        AND source_hash=?2
      LIMIT 1";
    let res = self.conn.query_row(query, params![specifier, &expected_source_hash], |row| {
  ...
  pub fn set_cjs_analysis(&self, specifier: &str, source_hash: &str, cjs_analysis: &CjsAnalysis) -> Result<(), AnyError> {
    let sql = "
      INSERT OR REPLACE INTO
        cjsanalysiscache (specifier, source_hash, data)
      VALUES
        (?1, ?2, ?3)";
    self.conn.execute(sql, params![specifier, &source_hash.to_string(), &serde_json::to_string(&cjs_analysis)?,])?;
    Ok(())
  }
}
