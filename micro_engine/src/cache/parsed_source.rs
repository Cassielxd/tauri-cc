...
impl ParsedSourceCache {
  pub fn get_parsed_source_from_esm_module(&self, module: &deno_graph::EsmModule) -> Result<ParsedSource, deno_ast::Diagnostic> {
  ...
  pub fn get_or_parse_module(&self, specifier: &deno_graph::ModuleSpecifier, source: Arc<str>, media_type: MediaType) -> deno_core::anyhow::Result<ParsedSource, deno_ast::Diagnostic> {
...
impl deno_graph::ParsedSourceStore for ParsedSourceCache {
  fn set_parsed_source(&self, specifier: deno_graph::ModuleSpecifier, parsed_source: ParsedSource) -> Option<ParsedSource> {
