...
impl Emitter {
  pub fn new(emit_cache: EmitCache, parsed_source_cache: Arc<ParsedSourceCache>, emit_options: deno_ast::EmitOptions) -> Self {
  ...
  pub fn cache_module_emits(&self, graph: &ModuleGraph) -> Result<(), AnyError> {
    for module in graph.modules() {
      if let Module::Esm(module) = module {
        let is_emittable = matches!(module.media_type, MediaType::TypeScript | MediaType::Mts | MediaType::Cts | MediaType::Jsx | MediaType::Tsx);
        if is_emittable {
  ...
  pub fn emit_parsed_source(&self, specifier: &ModuleSpecifier, media_type: MediaType, source: &Arc<str>) -> Result<ModuleCode, AnyError> {
    let source_hash = self.get_source_hash(source);
    ...
    if let Some(emit_code) = self.emit_cache.get_emit_code(specifier, source_hash) {
      ...
      let parsed_source = self.parsed_source_cache.get_or_parse_module(specifier, source.clone(), media_type)?;
      let transpiled_source = parsed_source.transpile(&self.emit_options)?;
      debug_assert!(transpiled_source.source_map.is_none());
      self.emit_cache.set_emit_code(specifier, source_hash, &transpiled_source.text);
      Ok(transpiled_source.text.into())
    }
  }

  /// Expects a file URL, panics otherwise.
  pub async fn load_and_emit_for_hmr(&self, specifier: &ModuleSpecifier) -> Result<String, AnyError> {
    let media_type = MediaType::from_specifier(specifier);
    let source_code = tokio::fs::read_to_string(ModuleSpecifier::to_file_path(specifier).unwrap()).await?;
    let source_arc: Arc<str> = source_code.into();
    let parsed_source = self.parsed_source_cache.get_or_parse_module(specifier, source_arc, media_type)?;
  ...
  fn get_source_hash(&self, source_text: &str) -> u64 {
    FastInsecureHasher::new().write_str(source_text).write_u64(self.emit_options_hash).finish()
  }
}
