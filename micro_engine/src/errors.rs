...
fn get_module_graph_error_class(err: &ModuleGraphError) -> &'static str {
  match err {
    ModuleGraphError::ModuleError(err) => match err {
      ...
      ModuleError::UnsupportedMediaType { .. } | ModuleError::UnsupportedImportAttributeType { .. } => "TypeError",
      ModuleError::Missing(_, _) | ModuleError::MissingDynamic(_, _) | ModuleError::MissingWorkspaceMemberExports { .. } | ModuleError::UnknownExport { .. } | ModuleError::UnknownPackage { .. } | ModuleError::UnknownPackageReq { .. } => "NotFound",
    },
    ModuleGraphError::ResolutionError(err) | ModuleGraphError::TypesResolutionError(err) => get_resolution_error_class(err),
  }
}
...
pub fn get_error_class_name(e: &AnyError) -> &'static str {
  deno_runtime::errors::get_error_class_name(e)
    ...
    .or_else(|| e.downcast_ref::<ModuleGraphError>().map(get_module_graph_error_class))
    .or_else(|| e.downcast_ref::<ResolutionError>().map(get_resolution_error_class))
    .unwrap_or_else(|| {
      if cfg!(debug) {
        log::warn!(
