...
fn fetch_local(specifier: &ModuleSpecifier) -> Result<File, AnyError> {
  let local = specifier.to_file_path().map_err(|_| uri_error(format!("Invalid file path.\n  Specifier: {specifier}")))?;
...
pub fn get_source_from_data_url(specifier: &ModuleSpecifier) -> Result<(String, String), AnyError> {
  ...
  let (bytes, _) = data_url.decode_to_vec().map_err(|e| uri_error(format!("{e:?}")))?;
  Ok((get_source_from_bytes(bytes, charset)?, format!("{mime}")))
}

/// Given a vector of bytes and optionally a charset, decode the bytes to a
/// string.
pub fn get_source_from_bytes(bytes: Vec<u8>, maybe_charset: Option<String>) -> Result<String, AnyError> {
...
fn get_validated_scheme(specifier: &ModuleSpecifier) -> Result<String, AnyError> {
  ...
  if !SUPPORTED_SCHEMES.contains(&scheme) {
    Err(generic_error(format!("Unsupported scheme \"{scheme}\" for module \"{specifier}\". Supported schemes: {SUPPORTED_SCHEMES:#?}")))
  } else {
    Ok(scheme.to_string())
  }
}
...
pub fn map_content_type(specifier: &ModuleSpecifier, maybe_content_type: Option<&String>) -> (MediaType, Option<String>) {
  if let Some(content_type) = maybe_content_type {
    ...
    let charset = content_types.map(str::trim).find_map(|s| s.strip_prefix("charset=")).map(String::from);
...
impl FileFetcher {
  pub fn new(http_cache: Arc<dyn HttpCache>, cache_setting: CacheSetting, allow_remote: bool, http_client: Arc<HttpClient>, blob_store: Arc<BlobStore>, progress_bar: Option<ProgressBar>) -> Self {
    Self {
  ...
  fn build_remote_file(&self, specifier: &ModuleSpecifier, bytes: Vec<u8>, headers: &HashMap<String, String>) -> Result<File, AnyError> {
    ...
    let maybe_types = match media_type {
      MediaType::JavaScript | MediaType::Cjs | MediaType::Mjs | MediaType::Jsx => headers.get("x-typescript-types").cloned(),
      _ => None,
    };
  ...
  pub fn fetch_cached(&self, specifier: &ModuleSpecifier, redirect_limit: i64) -> Result<Option<File>, AnyError> {
  ...
  async fn fetch_blob_url(&self, specifier: &ModuleSpecifier) -> Result<File, AnyError> {
    ...
    let blob = self.blob_store.get_object_url(specifier.clone()).ok_or_else(|| custom_error("NotFound", format!("Blob URL not found: \"{specifier}\".")))?;
  ...
  /// return a `Pin<Box<..>>`.
  fn fetch_remote(&self, specifier: &ModuleSpecifier, permissions: PermissionsContainer, redirect_limit: i64, maybe_accept: Option<String>, cache_setting: &CacheSetting) -> Pin<Box<dyn Future<Output = Result<File, AnyError>> + Send>> {
    ...
    if *cache_setting == CacheSetting::Only {
      return futures::future::err(custom_error("NotCached", format!("Specifier not found in cache: \"{specifier}\", --cached-only is specified."))).boxed();
    }
    ...
    if let Some(pb) = self.progress_bar.as_ref() {
      ...
      log::log!(self.download_log_level, "{} {}", colors::green("Download"), specifier);
    }
    ...
    async fn handle_request_or_server_error(retried: &mut bool, specifier: &Url, err_str: String) -> Result<(), AnyError> {
      ...
      if !*retried {
        *retried = true;
        ...
        Err(generic_error(format!("Import '{}' failed: {}", specifier, err_str)))
      }
    }
      ...
      let result = loop {
        ...
        .await?
        {
          ...
          FetchOnceResult::Redirect(redirect_url, headers) => {
            ...
            file_fetcher.fetch_remote(&redirect_url, permissions, redirect_limit - 1, maybe_accept, &cache_setting).await
          }
          FetchOnceResult::Code(bytes, headers) => {
            file_fetcher.http_cache.set(&specifier, headers.clone(), &bytes)?;
  ...
  fn should_use_cache(&self, specifier: &ModuleSpecifier, cache_setting: &CacheSetting) -> bool {
    match cache_setting {
      ...
      CacheSetting::RespectHeaders => {
        ...
        let cache_semantics = CacheSemantics::new(metadata.headers, metadata.time, SystemTime::now());
  ...
  pub async fn fetch(&self, specifier: &ModuleSpecifier, permissions: PermissionsContainer) -> Result<File, AnyError> {
    self
  ...
  pub async fn fetch_with_options(&self, options: FetchOptions<'_>) -> Result<File, AnyError> {
    ...
    if let Some(file) = self.cache.get(specifier) {
      ...
      Err(custom_error("NoRemote", format!("A remote specifier was requested: \"{specifier}\", but --no-remote is specified.")))
    } else {
      let result = self
        .fetch_remote(specifier, options.permissions, 10, options.maybe_accept.map(String::from), options.maybe_cache_setting.unwrap_or(&self.cache_setting))
        .await;
...
/// yields Redirect(url).
async fn fetch_once<'a>(http_client: &HttpClient, args: FetchOnceArgs<'a>) -> Result<FetchOnceResult, AnyError> {
  ...
  if let Some(warning) = response_headers.get("X-Deno-Warning") {
    log::warn!("{} {}", crate::colors::yellow("Warning"), warning.to_str().unwrap());
  }
  ...
  let body = http_util::get_response_body_with_progress(response, args.maybe_progress_guard).await?;
