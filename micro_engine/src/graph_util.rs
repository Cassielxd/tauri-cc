...
pub fn graph_valid_with_cli_options(graph: &ModuleGraph, roots: &[ModuleSpecifier], options: &CliOptions) -> Result<(), AnyError> {
  graph_valid(
    graph,
    roots,
    GraphValidOptions {
...
/// for the CLI.
pub fn graph_valid(graph: &ModuleGraph, roots: &[ModuleSpecifier], options: GraphValidOptions) -> Result<(), AnyError> {
  let mut errors = graph
    ...
    .flat_map(|error| {
      ...
      let mut message = match &error {
        ModuleGraphError::ResolutionError(resolution_error) => enhanced_resolution_error_message(resolution_error),
        ModuleGraphError::TypesResolutionError(resolution_error) => {
          format!("Failed resolving types. {}", enhanced_resolution_error_message(resolution_error,))
        }
      ...
      if options.is_vendoring {
        ...
        if matches!(error, ModuleGraphError::ModuleError(ModuleError::MissingDynamic(_, _))) {
        ...
        match &error {
          ModuleGraphError::ResolutionError(err) | ModuleGraphError::TypesResolutionError(err) => {
            if matches!(err, ResolutionError::InvalidDowngrade { .. } | ResolutionError::InvalidLocalImport { .. }) {
              return None;
            }
          }
...
pub fn graph_lock_or_exit(graph: &ModuleGraph, lockfile: &mut Lockfile) {
  for module in graph.modules() {
    ...
    if !lockfile.check_or_insert_remote(module.specifier().as_str(), source) {
      let err = format!(
        concat!("The source code is invalid, as it does not match the expected hash in the lock file.\n", "  Specifier: {}\n", "  Lock file: {}",),
        module.specifier(),
...
impl ModuleGraphBuilder {
  ...
  pub async fn create_graph(&self, graph_kind: GraphKind, roots: Vec<ModuleSpecifier>) -> Result<deno_graph::ModuleGraph, AnyError> {
    let mut cache = self.create_graph_loader();
    self.create_graph_with_loader(graph_kind, roots, &mut cache).await
  }

  pub async fn create_graph_with_loader(&self, graph_kind: GraphKind, roots: Vec<ModuleSpecifier>, loader: &mut dyn Loader) -> Result<deno_graph::ModuleGraph, AnyError> {
  ...
  pub async fn create_graph_with_options(&self, options: CreateGraphOptions<'_>) -> Result<deno_graph::ModuleGraph, AnyError> {
  ...
  pub async fn create_graph_and_maybe_check(&self, roots: Vec<ModuleSpecifier>) -> Result<Arc<deno_graph::ModuleGraph>, AnyError> {
    let mut cache = self.create_graph_loader();
    let maybe_imports = self.options.to_maybe_imports()?;
    let maybe_workspace_config = self.options.maybe_workspace_config();
    let workspace_members = if let Some(wc) = maybe_workspace_config { workspace_config_to_workspace_members(wc)? } else { vec![] };
  ...
  pub async fn build_graph_with_npm_resolution<'a>(&self, graph: &mut ModuleGraph, roots: Vec<ModuleSpecifier>, loader: &mut dyn deno_graph::source::Loader, options: deno_graph::BuildOptions<'a>) -> Result<(), AnyError> {
    ...
    if graph.packages.is_empty() {
      if let Some(lockfile) = &self.lockfile {
        ...
        for (key, value) in &lockfile.content.packages.specifiers {
          if let Some(key) = key.strip_prefix("jsr:").and_then(|key| PackageReq::from_str(key).ok()) {
            if let Some(value) = value.strip_prefix("jsr:").and_then(|value| PackageNv::from_str(value).ok()) {
    ...
    if !graph.redirects.is_empty() {
      if let Some(lockfile) = &self.lockfile {
        let graph_redirects = graph.redirects.iter().filter(|(from, _)| !matches!(from.scheme(), "npm" | "file" | "deno"));
...
pub fn error_for_any_npm_specifier(graph: &ModuleGraph) -> Result<(), AnyError> {
  for module in graph.modules() {
    match module {
      ...
      Module::Node(module) => {
        bail!(
          "Node specifiers have not yet been implemented for this subcommand (https://github.com/denoland/deno/issues/15960). Found: node:{}",
          module.module_name
        )
      }
...
pub fn get_resolution_error_bare_node_specifier(error: &ResolutionError) -> Option<&str> {
  get_resolution_error_bare_specifier(error).filter(|specifier| deno_node::is_builtin_node_module(specifier))
}
...
fn get_resolution_error_bare_specifier(error: &ResolutionError) -> Option<&str> {
  ...
  } = error
  {
    ...
    if let ResolveError::Other(error) = (*error).as_ref() {
      if let Some(ImportMapError::UnmappedBareSpecifier(specifier, _)) = error.downcast_ref::<ImportMapError>() {
        Some(specifier.as_str())
      } else {
        None
      }
...
pub fn has_graph_root_local_dependent_changed(graph: &ModuleGraph, root: &ModuleSpecifier, changed_specifiers: &HashSet<ModuleSpecifier>) -> bool {
...
impl deno_graph::source::Reporter for FileWatcherReporter {
  fn on_load(&self, specifier: &ModuleSpecifier, modules_done: usize, modules_total: usize) {
    ...
    if modules_done == modules_total {
      self.watcher_communicator.watch_paths(file_paths.drain(..).collect()).unwrap();
    }
  }
}

pub fn workspace_config_to_workspace_members(workspace_config: &deno_config::WorkspaceConfig) -> Result<Vec<deno_graph::WorkspaceMember>, AnyError> {
  workspace_config
    .members
    .iter()
    .map(|member| workspace_member_config_try_into_workspace_member(member).with_context(|| format!("Failed to resolve configuration for '{}' workspace member at '{}'", member.member_name, member.config_file.specifier.as_str())))
    .collect()
}

fn workspace_member_config_try_into_workspace_member(config: &deno_config::WorkspaceMemberConfig) -> Result<deno_graph::WorkspaceMember, AnyError> {
