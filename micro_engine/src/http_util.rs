...
pub fn resolve_redirect_from_response(request_url: &Url, response: &Response) -> Result<Url, AnyError> {
  ...
  if let Some(location) = response.headers().get(LOCATION) {
    ...
    Err(generic_error(format!("Redirection from '{request_url}' did not provide location header")))
  }
}
...
impl CacheSemantics {
  pub fn new(headers: HashMap<String, String>, cached: SystemTime, now: SystemTime) -> Self {
    let cache_control = headers.get("cache-control").map(|v| CacheControl::from_value(v).unwrap_or_default()).unwrap_or_default();
    Self { cache_control, cached, headers, now }
  }
  ...
  fn age_header_value(&self) -> Duration {
    Duration::from_secs(self.headers.get("age").and_then(|v| v.parse().ok()).unwrap_or(0))
  }
  ...
  fn max_age(&self) -> Duration {
    ...
    if let Some(expires) = self.headers.get("expires") {
      return match DateTime::parse_from_rfc2822(expires) {
        ...
        Ok(expires) => {
          let expires = SystemTime::UNIX_EPOCH + Duration::from_secs(expires.timestamp().max(0) as _);
  ...
  pub fn should_use(&self) -> bool {
    ...
    if self.is_stale() {
      ...
      let allows_stale = has_max_stale && self.cache_control.max_stale.map(|val| val > self.age() - self.max_age()).unwrap_or(true);
      if !allows_stale {
        return false;
      }
    }
...
impl HttpClient {
  pub fn new(root_cert_store_provider: Option<Arc<dyn RootCertStoreProvider>>, unsafely_ignore_certificate_errors: Option<Vec<String>>) -> Self {
    Self {
      options: CreateHttpClientOptions {
  ...
  pub fn get_no_redirect<U: reqwest::IntoUrl>(&self, url: U) -> Result<reqwest::RequestBuilder, AnyError> {
  ...
  async fn inner_download<U: reqwest::IntoUrl>(&self, url: U, progress_guard: Option<&UpdateGuard>) -> Result<Option<Vec<u8>>, AnyError> {
    ...
    get_response_body_with_progress(response, progress_guard).await.map(Some)
  }
