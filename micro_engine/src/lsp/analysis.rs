...
impl<'a> TsResponseImportMapper<'a> {
  pub fn new(documents: &'a Documents, maybe_import_map: Option<&'a ImportMap>, node_resolver: Option<&'a NodeResolver>, npm_resolver: Option<&'a dyn CliNpmResolver>) -> Self {
    Self {
      documents,
      maybe_import_map,
      node_resolver,
  ...
  pub fn check_specifier(&self, specifier: &ModuleSpecifier, referrer: &ModuleSpecifier) -> Option<String> {
          ...
          if !pkg_reqs.is_empty() {
            ...
            if let Some(import_map) = self.maybe_import_map {
              for pkg_req in &pkg_reqs {
                let paths = vec![concat_npm_specifier("npm:", pkg_req, sub_path.as_deref()), concat_npm_specifier("npm:/", pkg_req, sub_path.as_deref())];
                for path in paths {
                  if let Some(mapped_path) = ModuleSpecifier::parse(&path).ok().and_then(|s| import_map.lookup(&s, referrer)) {
  ...
  fn resolve_package_path(&self, specifier: &ModuleSpecifier) -> Option<String> {
    let node_resolver = self.node_resolver?;
    let package_json = node_resolver.get_closest_package_json(specifier, &PermissionsContainer::allow_all()).ok().flatten()?;
  ...
  /// None if none match.
  pub fn check_unresolved_specifier(&self, specifier: &str, referrer: &ModuleSpecifier) -> Option<String> {
    ...
    for ext in SUPPORTED_EXTENSIONS {
      ...
      if self.documents.contains_import(&specifier_with_ext, referrer) {
...
fn try_reverse_map_package_json_exports(root_path: &Path, target_path: &Path, exports: &serde_json::Map<String, serde_json::Value>) -> Option<String> {
  use deno_core::serde_json::Value;

  fn try_reverse_map_package_json_exports_inner(root_path: &Path, target_path: &Path, exports: &serde_json::Map<String, Value>) -> Option<String> {
    for (key, value) in exports {
      match value {
        ...
        Value::Object(obj) => {
          if let Some(result) = try_reverse_map_package_json_exports_inner(root_path, target_path, obj) {
            return Some(if let Some(suffix) = key.strip_prefix("./") {
              if result.is_empty() {
                suffix.to_string()
              } else {
...
pub fn fix_ts_import_changes(referrer: &ModuleSpecifier, changes: &[tsc::FileTextChanges], import_mapper: &TsResponseImportMapper) -> Result<Vec<tsc::FileTextChanges>, AnyError> {
  let mut r = Vec::new();
...
fn fix_ts_import_action(referrer: &ModuleSpecifier, action: &tsc::CodeFixAction, import_mapper: &TsResponseImportMapper) -> Result<tsc::CodeFixAction, AnyError> {
...
pub fn ts_changes_to_edit(changes: &[tsc::FileTextChanges], language_server: &language_server::Inner) -> Result<Option<lsp::WorkspaceEdit>, AnyError> {
...
impl CodeActionCollection {
  ...
  pub fn add_deno_lint_ignore_action(&mut self, specifier: &ModuleSpecifier, diagnostic: &lsp::Diagnostic, maybe_text_info: Option<SourceTextInfo>, maybe_parsed_source: Option<deno_ast::ParsedSource>) -> Result<(), AnyError> {
    let code = diagnostic
      .code
      .as_ref()
      .map(|v| match v {
    ...
    let maybe_ignore_comment = maybe_parsed_source.clone().and_then(|ps| {
      ...
      ps.comments().get_vec().iter().find_map(|c| {
        ...
        comment_text.split_whitespace().next().and_then(|prefix| if prefix == "deno-lint-ignore-file" { Some(c.clone()) } else { None })
      })
    });
  ...
  pub fn add_ts_fix_action(&mut self, specifier: &ModuleSpecifier, action: &tsc::CodeFixAction, diagnostic: &lsp::Diagnostic, language_server: &language_server::Inner) -> Result<(), AnyError> {
    ...
    let action = fix_ts_import_action(specifier, action, &language_server.get_ts_response_import_mapper())?;
  ...
  pub fn add_ts_fix_all_action(&mut self, action: &tsc::CodeFixAction, specifier: &ModuleSpecifier, diagnostic: &lsp::Diagnostic) {
    ...
    self.fix_all_actions.insert(FixAllKind::Tsc(action.fix_id.clone().unwrap()), CodeActionKind::Tsc(code_action, action.clone()));
  }
  ...
  pub fn get_response(self) -> lsp::CodeActionResponse {
    ...
    let (tsc, rest): (Vec<_>, Vec<_>) = self.actions.into_iter().partition(|a| matches!(a, CodeActionKind::Tsc(..)));
    let (deno, deno_lint): (Vec<_>, Vec<_>) = rest.into_iter().partition(|a| matches!(a, CodeActionKind::Deno(_)));
  ...
  pub fn add_cache_all_action(&mut self, specifier: &ModuleSpecifier, diagnostics: Vec<lsp::Diagnostic>) {
