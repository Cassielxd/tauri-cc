...
pub fn calculate_fs_version(cache: &Arc<dyn HttpCache>, specifier: &ModuleSpecifier) -> Option<String> {
  match specifier.scheme() {
    "npm" | "node" | "data" | "blob" => None,
    "file" => specifier_to_file_path(specifier).ok().and_then(|path| calculate_fs_version_at_path(&path)),
...
fn calculate_fs_version_in_cache(cache: &Arc<dyn HttpCache>, specifier: &ModuleSpecifier) -> Option<String> {
...
impl CacheMetadata {
  pub fn new(cache: Arc<dyn HttpCache>) -> Self {
    Self { cache, metadata: Default::default() }
  }
  ...
  pub fn get(&self, specifier: &ModuleSpecifier) -> Option<Arc<HashMap<MetadataKey, String>>> {
    if matches!(specifier.scheme(), "file" | "npm" | "node" | "data" | "blob") {
      return None;
    }
