...
impl Client {
  ...
  pub fn send_did_change_deno_configuration_notification(&self, params: lsp_custom::DidChangeDenoConfigurationNotificationParams) {
    // do on a task in case the caller currently is in the lsp lock
    let client = self.0.clone();
    spawn(async move {
      client.send_did_change_deno_configuration_notification(params).await;
    });
  }

  pub fn send_did_upgrade_check_notification(&self, params: lsp_custom::DidUpgradeCheckNotificationParams) {
    // do on a task in case the caller currently is in the lsp lock
    let client = self.0.clone();
    spawn(async move {
...
impl OutsideLockClient {
  ...
  pub async fn workspace_configuration(&self, scopes: Vec<Option<lsp::Url>>) -> Result<Vec<WorkspaceSettings>, AnyError> {
    self.0.workspace_configuration(scopes).await
  }

  pub async fn publish_diagnostics(&self, uri: LspClientUrl, diags: Vec<lsp::Diagnostic>, version: Option<i32>) {
    self.0.publish_diagnostics(uri.into_url(), diags, version).await;
  }
}
...
trait ClientTrait: Send + Sync {
  ...
  async fn send_did_change_deno_configuration_notification(&self, params: lsp_custom::DidChangeDenoConfigurationNotificationParams);
  async fn send_did_upgrade_check_notification(&self, params: lsp_custom::DidUpgradeCheckNotificationParams);
  async fn workspace_configuration(&self, scopes: Vec<Option<lsp::Url>>) -> Result<Vec<WorkspaceSettings>, AnyError>;
...
impl ClientTrait for TowerClient {
  ...
  async fn send_did_change_deno_configuration_notification(&self, params: lsp_custom::DidChangeDenoConfigurationNotificationParams) {
    self.0.send_notification::<lsp_custom::DidChangeDenoConfigurationNotification>(params).await
  }

  async fn send_did_upgrade_check_notification(&self, params: lsp_custom::DidUpgradeCheckNotificationParams) {
    self.0.send_notification::<lsp_custom::DidUpgradeCheckNotification>(params).await
  }
    ...
    match config_response {
      Ok(configs) => {
        ...
        for _ in 0..scopes.len() {
          ...
          result.push(WorkspaceSettings::from_raw_settings(deno, javascript, typescript));
        }
        Ok(result)
      }
      Err(err) => {
...
impl ClientTrait for ReplClient {
  ...
  async fn send_did_change_deno_configuration_notification(&self, _params: lsp_custom::DidChangeDenoConfigurationNotificationParams) {}

  async fn send_did_upgrade_check_notification(&self, _params: lsp_custom::DidUpgradeCheckNotificationParams) {}
