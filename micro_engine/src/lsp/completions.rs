...
async fn check_auto_config_registry(url_str: &str, workspace_settings: &WorkspaceSettings, client: &Client, module_registries: &ModuleRegistry) {
  ...
  if workspace_settings.suggest.imports.auto_discover {
    if let Ok(specifier) = resolve_url(url_str) {
      ...
      if scheme.starts_with("http") && !path.is_empty() && url_str.ends_with(path) {
        ...
        let in_config = workspace_settings.suggest.imports.hosts.iter().any(|(h, _)| resolve_url(h).map(|u| u.origin()) == Ok(specifier.origin()));
        ...
        if !in_config {
          ...
          if suggestions {
            client.send_registry_state_notification(lsp_custom::RegistryStateNotificationParams { origin, suggestions });
          }
        }
      }
    }
...
fn to_narrow_lsp_range(text_info: &SourceTextInfo, range: &deno_graph::Range) -> lsp::Range {
...
pub async fn get_import_completions(
  ...
  if let Some(completion_list) = get_import_map_completions(specifier, &text, &range, maybe_import_map.clone(), documents) {
    ...
    Some(lsp::CompletionResponse::List(lsp::CompletionList { is_incomplete: !items.is_empty(), items }))
  } else if !text.is_empty() {
    // completion of modules from a module registry or cache
    check_auto_config_registry(&text, config.workspace_settings_for_specifier(specifier), client, module_registries).await;
    let offset = if position.character > range.start.character { (position.character - range.start.character) as usize } else { 0 };
    let maybe_list = module_registries.get_completions(&text, offset, &range, |specifier| documents.exists(specifier)).await;
    ...
    Some(lsp::CompletionResponse::List(lsp::CompletionList { is_incomplete, items }))
  }
}
...
fn get_base_import_map_completions(import_map: &ImportMap) -> Vec<lsp::CompletionItem> {
  import_map
    ...
    .map(|key| {
      ...
      // change the behavior
      let mut label = if key.starts_with("file://") { FILE_PROTO_RE.replace(key, "").to_string() } else { key.to_string() };
...
fn get_import_map_completions(specifier: &ModuleSpecifier, text: &str, range: &lsp::Range, maybe_import_map: Option<Arc<ImportMap>>, documents: &Documents) -> Option<lsp::CompletionList> {
  if !text.is_empty() {
    if let Some(import_map) = maybe_import_map {
      ...
      for key in import_map.imports().keys() {
        ...
        let key = if key.starts_with("file://") { FILE_PROTO_RE.replace(key, "").to_string() } else { key.to_string() };
        if text.starts_with(&key) && key.ends_with('/') {
            ...
            let workspace_items: Vec<lsp::CompletionItem> = documents
              ...
              .filter_map(|d| {
                ...
                if specifier_str.starts_with(&resolved) {
                  ...
                  let text_edit = Some(lsp::CompletionTextEdit::Edit(lsp::TextEdit { range: *range, new_text: new_text.clone() }));
          ...
          let text_edit = Some(lsp::CompletionTextEdit::Edit(lsp::TextEdit { range: *range, new_text: label.clone() }));
        ...
        if !items.is_empty() {
          return Some(lsp::CompletionList { items, is_incomplete: false });
        }
      }
    }
  }
...
fn get_local_completions(base: &ModuleSpecifier, current: &str, range: &lsp::Range) -> Option<Vec<lsp::CompletionItem>> {
    ...
    Some(
      items
        .filter_map(|de| {
          ...
          let filter_text = if full_text.starts_with(current) { Some(full_text) } else { Some(format!("{current}{label}")) };
          match de.file_type() {
...
fn get_relative_specifiers(base: &ModuleSpecifier, specifiers: Vec<ModuleSpecifier>) -> Vec<String> {
  specifiers.iter().filter_map(|s| if s != base { Some(relative_specifier(base, s).unwrap_or_else(|| s.to_string())) } else { None }).collect()
}

/// Get completions for `npm:` specifiers.
async fn get_npm_completions(referrer: &ModuleSpecifier, specifier: &str, range: &lsp::Range, npm_search_api: &impl NpmSearchApi) -> Option<Vec<lsp::CompletionItem>> {
  ...
  let v_index = if bare_specifier.starts_with('@') {
    bare_specifier.find('/').filter(|idx| !bare_specifier[1..*idx].is_empty()).and_then(|idx| {
      bare_specifier[idx..]
        .find('@')
    ...
    bare_specifier.find('@').filter(|idx| !bare_specifier[..*idx].is_empty()).filter(|idx| !bare_specifier[..*idx].contains('/'))
  };
  ...
  if let Some(v_index) = v_index {
    ...
    let versions = &npm_search_api.package_info(package_name).await.ok()?.versions;
    ...
    let items = versions
      ...
      .filter_map(|(idx, version)| {
        ...
        let text_edit = Some(lsp::CompletionTextEdit::Edit(lsp::TextEdit { range: *range, new_text: specifier.clone() }));
        Some(lsp::CompletionItem {
          label: specifier,
          kind: Some(lsp::CompletionItemKind::FILE),
...
fn get_workspace_completions(specifier: &ModuleSpecifier, current: &str, range: &lsp::Range, documents: &Documents) -> Vec<lsp::CompletionItem> {
  let workspace_specifiers = documents.documents(DocumentsFilter::AllDiagnosable).into_iter().map(|d| d.specifier().clone()).collect();
  let specifier_strings = get_relative_specifiers(specifier, workspace_specifiers);
  specifier_strings
    .into_iter()
    .filter_map(|label| {
      if label.starts_with(current) {
        let detail = Some(if label.starts_with("http:") || label.starts_with("https:") {
          "(remote)".to_string()
