...
impl DiagnosticsPublisher {
  ...
  pub async fn publish(&self, source: DiagnosticSource, diagnostics: DiagnosticVec, url_map: &LspUrlMap, token: &CancellationToken) -> usize {
    ...
    for record in diagnostics {
      ...
      let diagnostics_by_source = diagnostics_by_specifier.entry(record.specifier.clone()).or_default();
      ...
      *source_diagnostics = record.versioned;
      ...
      let all_specifier_diagnostics = diagnostics_by_source.values().flat_map(|d| &d.diagnostics).cloned().collect::<Vec<_>>();

      self.state.update(&record.specifier, version, &all_specifier_diagnostics);
      self
        .client
        .when_outside_lsp_lock()
        .publish_diagnostics(url_map.normalize_specifier(&record.specifier).unwrap_or(LspClientUrl::new(record.specifier)), all_specifier_diagnostics, version)
        .await;
      messages_sent += 1;
    }
      ...
      if diagnostics_by_source.is_empty() {
        ...
        if let Some(removed_value) = maybe_removed_value {
          ...
          self
            .client
            .when_outside_lsp_lock()
            .publish_diagnostics(url_map.normalize_specifier(specifier).unwrap_or_else(|_| LspClientUrl::new(specifier.clone())), Vec::new(), removed_value.version)
            .await;
          messages_sent += 1;
        }
      }
...
impl TsDiagnosticsStore {
  ...
  fn update(&self, diagnostics: &DiagnosticVec) {
    ...
    *stored_ts_diagnostics = diagnostics.iter().map(|record| (record.specifier.clone(), record.versioned.clone())).collect();
  }
}
...
impl DiagnosticsState {
  fn update(&self, specifier: &ModuleSpecifier, version: Option<i32>, diagnostics: &[lsp::Diagnostic]) {
    ...
    for diagnostic in diagnostics {
      if diagnostic.code == Some(lsp::NumberOrString::String("no-cache".to_string())) || diagnostic.code == Some(lsp::NumberOrString::String("no-cache-npm".to_string())) {
        no_cache_diagnostics.push(diagnostic.clone());
      }
    }
    specifiers.insert(specifier.clone(), SpecifierState { version, no_cache_diagnostics });
  }
  ...
  pub fn has_no_cache_diagnostics(&self, specifier: &ModuleSpecifier) -> bool {
    self.specifiers.read().get(specifier).map(|s| !s.no_cache_diagnostics.is_empty()).unwrap_or(false)
  }

  pub fn no_cache_diagnostics(&self, specifier: &ModuleSpecifier) -> Vec<lsp::Diagnostic> {
    self.specifiers.read().get(specifier).map(|s| s.no_cache_diagnostics.clone()).unwrap_or_default()
  }
}
...
impl DiagnosticsServer {
  pub fn new(client: Client, performance: Arc<Performance>, ts_server: Arc<TsServer>, state: Arc<DiagnosticsState>) -> Self {
    DiagnosticsServer {
  ...
  pub fn start(&mut self) {
    ...
    let _join_handle = thread::spawn(move || {
      ...
      runtime.block_on(async {
        ...
        let diagnostics_publisher = Arc::new(DiagnosticsPublisher::new(client.clone(), state.clone()));
          ...
          match rx.recv().await {
            ...
            Some(message) => {
              ...
              let ChannelUpdateMessage {
                message: DiagnosticServerUpdateMessage { snapshot, config, lint_options, url_map },
                batch_index,
              } = message;
              ...
              ts_handle = Some(spawn({
                ...
                async move {
                  ...
                  if !token.is_cancelled() {
                    ...
                    messages_len = diagnostics_publisher.publish(DiagnosticSource::Ts, diagnostics, &url_map, &token).await;
                  ...
                  if let Some(batch_index) = batch_index {
                    diagnostics_publisher.client.send_diagnostic_batch_notification(DiagnosticBatchNotificationParams { batch_index, messages_len });
                  }
                }
              }));
              ...
              deps_handle = Some(spawn({
                ...
                async move {
                  ...
                  let mut messages_len = 0;
                  if !token.is_cancelled() {
                    messages_len = diagnostics_publisher.publish(DiagnosticSource::Deno, diagnostics, &url_map, &token).await;
              ...
              lint_handle = Some(spawn({
                ...
                async move {
                  ...
                  let mut messages_len = 0;
                  if !token.is_cancelled() {
                    messages_len = diagnostics_publisher.publish(DiagnosticSource::Lint, diagnostics, &url_map, &token).await;
...
fn to_lsp_range(start: &crate::tsc::Position, end: &crate::tsc::Position) -> lsp::Range {
  lsp::Range { start: start.into(), end: end.into() }
}
      ...
      .filter_map(|ri| {
        if let (Some(file_name), Some(start), Some(end)) = (&ri.file_name, &ri.start, &ri.end) {
          ...
          Some(lsp::DiagnosticRelatedInformation {
            location: lsp::Location { uri, range: to_lsp_range(start, end) },
  ...
  for document in documents {
    ...
    diagnostics_vec.push(DiagnosticRecord {
      ...
      versioned: VersionedDiagnostics {
        version,
        diagnostics: generate_document_lint_diagnostics(config, lint_options, lint_rules.clone(), &document),
      },
    });
  }
  diagnostics_vec
...
fn generate_document_lint_diagnostics(config: &ConfigSnapshot, lint_options: &LintOptions, lint_rules: Vec<&'static dyn LintRule>, document: &Document) -> Vec<lsp::Diagnostic> {
...
async fn generate_ts_diagnostics(snapshot: Arc<language_server::StateSnapshot>, config: &ConfigSnapshot, ts_server: &tsc::TsServer, token: CancellationToken) -> Result<DiagnosticVec, AnyError> {
  let mut diagnostics_vec = Vec::new();
  let specifiers = snapshot.documents.documents(DocumentsFilter::OpenDiagnosable).into_iter().map(|d| d.specifier().clone());
  ...
  for (specifier_str, ts_json_diagnostics) in ts_diagnostics_map {
    ...
    let ts_diagnostics = if config.specifier_enabled(&specifier) { ts_json_to_diagnostics(ts_json_diagnostics) } else { Vec::new() };
    diagnostics_vec.push(DiagnosticRecord {
      specifier,
      versioned: VersionedDiagnostics { version, diagnostics: ts_diagnostics },
    });
  }
  ...
  for specifier in disabled_specifiers {
    ...
    diagnostics_vec.push(DiagnosticRecord {
      specifier,
      versioned: VersionedDiagnostics { version, diagnostics: Vec::new() },
    });
  }
  Ok(diagnostics_vec)
}
...
impl DenoDiagnostic {
  ...
  pub fn is_fixable(diagnostic: &lsp_types::Diagnostic) -> bool {
    if let Some(lsp::NumberOrString::String(code)) = &diagnostic.code {
      matches!(code.as_str(), "import-map-remap" | "no-cache" | "no-cache-npm" | "no-attribute-type" | "redirect" | "import-node-prefix-missing")
    } else {
      false
    }
  }
  ...
  pub fn to_lsp_diagnostic(&self, range: &lsp::Range) -> lsp::Diagnostic {
    let (severity, message, data) = match self {
      ...
      Self::ImportMapRemap { from, to } => (
        ...
        format!("The import specifier can be remapped to \"{to}\" which will resolve it via the active import map."),
        Some(json!({ "from": from, "to": to })),
      ),
      Self::InvalidAttributeType(assert_type) => (
        lsp::DiagnosticSeverity::ERROR,
        format!("The module is a JSON module and expected an attribute type of \"json\". Instead got \"{assert_type}\"."),
        None,
      ),
      Self::NoAttributeType => (
        lsp::DiagnosticSeverity::ERROR,
        "The module is a JSON module and not being imported with an import attribute. Consider adding `with { type: \"json\" }` to the import statement.".to_string(),
        None,
      ),
      ...
      Self::Redirect { from, to } => (
        ...
        format!("The import of \"{from}\" was redirected to \"{to}\"."),
        Some(json!({ "specifier": from, "redirect": to })),
      ),
      Self::ResolutionError(err) => (
        lsp::DiagnosticSeverity::ERROR,
        enhanced_resolution_error_message(err),
        graph_util::get_resolution_error_bare_node_specifier(err).map(|specifier| json!({ "specifier": specifier })),
      ),
      ...
      Self::BareNodeSpecifier(specifier) => (
        ...
        format!("\"{}\" is resolved to \"node:{}\". If you want to use a built-in Node module, add a \"node:\" prefix.", specifier, specifier),
...
fn diagnose_resolution(snapshot: &language_server::StateSnapshot, dependency_key: &str, resolution: &Resolution, is_dynamic: bool, maybe_assert_type: Option<&str>) -> Vec<DenoDiagnostic> {
      ...
      if let Some(doc) = snapshot.documents.get(specifier) {
        ...
        if doc.media_type() == MediaType::Json {
          match maybe_assert_type {
            ...
            Some(assert_type) => diagnostics.push(DenoDiagnostic::InvalidAttributeType(assert_type.to_string())),
        ...
        if let Some(npm_resolver) = snapshot.npm.as_ref().and_then(|n| n.npm_resolver.as_managed()) {
        ...
        if !deno_node::is_builtin_node_module(module_name) {
        ...
        } else if let Some(npm_resolver) = snapshot.npm.as_ref().and_then(|n| n.npm_resolver.as_managed()) {
          ...
          if !npm_resolver.is_pkg_req_folder_cached(&types_node_req) {
            diagnostics.push(DenoDiagnostic::NoCacheNpm(types_node_req, ModuleSpecifier::parse("npm:@types/node").unwrap()));
          }
        }
      } else {
...
fn diagnose_dependency(diagnostics: &mut Vec<lsp::Diagnostic>, snapshot: &language_server::StateSnapshot, referrer: &ModuleSpecifier, dependency_key: &str, dependency: &deno_graph::Dependency) {
  if let Some(npm) = &snapshot.npm {
    ...
    if let Resolution::Ok(resolved) = &dependency.maybe_code {
      if let Some(to) = import_map.lookup(&resolved.specifier, referrer) {
        if dependency_key != to {
          diagnostics.push(DenoDiagnostic::ImportMapRemap { from: dependency_key.to_string(), to }.to_lsp_diagnostic(&documents::to_lsp_range(&resolved.range)));
        }
      }
    }
  }

  let import_ranges: Vec<_> = dependency.imports.iter().map(|i| documents::to_lsp_range(&i.range)).collect();

  diagnostics.extend(
    diagnose_resolution(
      snapshot,
      dependency_key,
      if dependency.maybe_code.is_none() { &dependency.maybe_type } else { &dependency.maybe_code },
      dependency.is_dynamic,
      dependency.maybe_attribute_type.as_deref(),
    )
    .iter()
    .flat_map(|diag| import_ranges.iter().map(|range| diag.to_lsp_diagnostic(range))),
  );
  ...
  if !dependency.maybe_type.is_none() && !dependency.imports.iter().any(|i| dependency.maybe_type.includes(&i.range.start).is_some()) {
    ...
    diagnostics.extend(
      diagnose_resolution(snapshot, dependency_key, &dependency.maybe_type, dependency.is_dynamic, dependency.maybe_attribute_type.as_deref())
        .iter()
...
fn generate_deno_diagnostics(snapshot: &language_server::StateSnapshot, config: &ConfigSnapshot, token: CancellationToken) -> DiagnosticVec {
  let mut diagnostics_vec = Vec::new();
