...
static JS_HEADERS: Lazy<HashMap<String, String>> = Lazy::new(|| ([("content-type".to_string(), "application/javascript".to_string())]).into_iter().collect());

static JSX_HEADERS: Lazy<HashMap<String, String>> = Lazy::new(|| ([("content-type".to_string(), "text/jsx".to_string())]).into_iter().collect());

static TS_HEADERS: Lazy<HashMap<String, String>> = Lazy::new(|| ([("content-type".to_string(), "application/typescript".to_string())]).into_iter().collect());

static TSX_HEADERS: Lazy<HashMap<String, String>> = Lazy::new(|| ([("content-type".to_string(), "text/tsx".to_string())]).into_iter().collect());

pub const DOCUMENT_SCHEMES: [&str; 7] = ["data", "blob", "file", "http", "https", "untitled", "deno-notebook-cell"];
...
pub fn file_like_to_file_specifier(specifier: &Url) -> Option<Url> {
  if matches!(specifier.scheme(), "untitled" | "deno-notebook-cell") {
    if let Ok(mut s) = ModuleSpecifier::parse(&format!("file://{}", &specifier.as_str()[url::quirks::internal_components(specifier).host_end as usize..],)) {
      s.query_pairs_mut().append_pair("scheme", specifier.scheme());
      return Some(s);
    }
  }
  None
    ...
    if file_like_to_file_specifier(&module.specifier).is_some() {
      for (_, dep) in &mut deps.deps {
        if let Resolution::Ok(resolved) = &mut dep.maybe_code {
          if let Some(specifier) = file_like_to_file_specifier(&resolved.specifier) {
...
impl Document {
  fn new(specifier: ModuleSpecifier, fs_version: String, maybe_headers: Option<HashMap<String, String>>, text_info: SourceTextInfo, resolver: &dyn deno_graph::source::Resolver, npm_resolver: &dyn deno_graph::source::NpmResolver) -> Self {
    ...
    // parse the module.
    let (maybe_parsed_source, maybe_module) = parse_and_analyze_module(&specifier, text_info.clone(), maybe_headers.as_ref(), resolver, npm_resolver);
  ...
  fn maybe_with_new_resolver(&self, resolver: &dyn deno_graph::source::Resolver, npm_resolver: &dyn deno_graph::source::NpmResolver) -> Option<Self> {
    ...
    let maybe_module = Some(analyze_module(&self.0.specifier, &parsed_source_result, self.0.maybe_headers.as_ref(), resolver, npm_resolver));
    let dependencies = Arc::new(DocumentDependencies::from_maybe_module(&maybe_module));
  ...
  fn open(specifier: ModuleSpecifier, version: i32, language_id: LanguageId, content: Arc<str>, cache: &Arc<dyn HttpCache>, resolver: &dyn deno_graph::source::Resolver, npm_resolver: &dyn deno_graph::source::NpmResolver) -> Self {
    let maybe_headers = language_id.as_headers();
    let text_info = SourceTextInfo::new(content);
    let (maybe_parsed_source, maybe_module) = if language_id.is_diagnosable() {
      parse_and_analyze_module(&specifier, text_info.clone(), maybe_headers, resolver, npm_resolver)
    } else {
      (None, None)
    };
    let dependencies = Arc::new(DocumentDependencies::from_maybe_module(&maybe_module));
  ...
  fn with_change(&self, version: i32, changes: Vec<lsp::TextDocumentContentChangeEvent>, resolver: &dyn deno_graph::source::Resolver, npm_resolver: &dyn deno_graph::source::NpmResolver) -> Result<Document, AnyError> {
    ...
    let (maybe_parsed_source, maybe_module) = if self.0.maybe_language_id.as_ref().map(|li| li.is_diagnosable()).unwrap_or(false) {
      ...
      parse_and_analyze_module(&self.0.specifier, text_info.clone(), maybe_headers, resolver, npm_resolver)
    } else {
      (None, None)
    };
    ...
    let line_index = if index_valid == IndexValid::All { line_index } else { Arc::new(LineIndex::new(text_info.text_str())) };
  ...
  pub fn script_version(&self) -> String {
    self.maybe_lsp_version().map(|v| format!("{}+{v}", self.fs_version())).unwrap_or_else(|| self.fs_version().to_string())
  }
  ...
  pub fn get_maybe_dependency(&self, position: &lsp::Position) -> Option<(String, deno_graph::Dependency, deno_graph::Range)> {
    ...
    module.dependencies.iter().find_map(|(s, dep)| dep.includes(&position).map(|r| (s.clone(), dep.clone(), r.clone())))
  }
}
  ...
  match result {
    Resolution::Ok(resolved) => {
      ...
      match specifier.scheme() {
        ...
        _ => format!("{}&#8203;{}", &specifier[..url::Position::AfterScheme], &specifier[url::Position::AfterScheme..],).replace('@', "&#8203;@"),
      }
    }
...
fn recurse_dependents(specifier: &ModuleSpecifier, map: &HashMap<ModuleSpecifier, HashSet<ModuleSpecifier>>, dependents: &mut HashSet<ModuleSpecifier>) {
...
impl SpecifierResolver {
  pub fn new(cache: Arc<dyn HttpCache>) -> Self {
    Self { cache, redirects: Mutex::new(HashMap::new()) }
  }
  ...
  fn resolve_remote(&self, specifier: &ModuleSpecifier, redirect_limit: usize) -> Option<ModuleSpecifier> {
    if redirect_limit > 0 {
      ...
      let headers = self.cache.read_metadata(&cache_key).ok().flatten().map(|m| m.headers)?;
...
impl FileSystemDocuments {
  pub fn get(&mut self, cache: &Arc<dyn HttpCache>, resolver: &dyn deno_graph::source::Resolver, specifier: &ModuleSpecifier, npm_resolver: &dyn deno_graph::source::NpmResolver) -> Option<Document> {
    let fs_version = if specifier.scheme() == "data" { Some("1".to_string()) } else { calculate_fs_version(cache, specifier) };
  ...
  fn refresh_document(&mut self, cache: &Arc<dyn HttpCache>, resolver: &dyn deno_graph::source::Resolver, specifier: &ModuleSpecifier, npm_resolver: &dyn deno_graph::source::NpmResolver) -> Option<Document> {
    let doc = if specifier.scheme() == "file" {
      ...
      Document::new(specifier.clone(), fs_version, None, SourceTextInfo::from_string(content), resolver, npm_resolver)
    } else if specifier.scheme() == "data" {
      let (source, _) = get_source_from_data_url(specifier).ok()?;
      Document::new(specifier.clone(), "1".to_string(), None, SourceTextInfo::from_string(source), resolver, npm_resolver)
    } else {
      ...
      let content = get_source_from_bytes(bytes, maybe_charset).ok()?;
      Document::new(specifier.clone(), fs_version, maybe_headers, SourceTextInfo::from_string(content), resolver, npm_resolver)
    };
    self.dirty = true;
...
impl Documents {
  ...
  pub fn module_graph_imports(&self) -> impl Iterator<Item = &ModuleSpecifier> {
    self.imports.values().flat_map(|i| i.dependencies.values()).flat_map(|value| value.get_type().or_else(|| value.get_code()))
  }
  ...
  pub fn open(&mut self, specifier: ModuleSpecifier, version: i32, language_id: LanguageId, content: Arc<str>) -> Document {
    ...
    let document = Document::open(specifier.clone(), version, language_id, content, &self.cache, resolver, npm_resolver);
  ...
  pub fn change(&mut self, specifier: &ModuleSpecifier, version: i32, changes: Vec<lsp::TextDocumentContentChangeEvent>) -> Result<Document, AnyError> {
    let doc = self
      .open_docs
      .get(specifier)
      .cloned()
    ...
    self.dirty = true;
    let doc = doc.with_change(version, changes, self.get_resolver(), self.get_npm_resolver())?;
  ...
  pub fn resolve_redirected(&self, specifier: &ModuleSpecifier) -> Option<ModuleSpecifier> {
  ...
  pub fn exists(&self, specifier: &ModuleSpecifier) -> bool {
    ...
    if let Some(specifier) = specifier {
      ...
      if specifier.scheme() == "file" {
        return specifier_to_file_path(&specifier).map(|p| p.is_file()).unwrap_or(false);
      }
  ...
  pub fn get(&self, original_specifier: &ModuleSpecifier) -> Option<Document> {
    ...
    if let Some(document) = self.open_docs.get(&specifier) {
      ...
      let mut file_system_docs = self.file_system_docs.lock();
      file_system_docs.get(&self.cache, self.get_resolver(), &specifier, self.get_npm_resolver())
    }
  }
  ...
  pub fn documents(&self, filter: DocumentsFilter) -> Vec<Document> {
    match filter {
      DocumentsFilter::OpenDiagnosable => self.open_docs.values().filter_map(|doc| if doc.is_diagnosable() { Some(doc.clone()) } else { None }).collect(),
  ...
  pub fn resolve(&self, specifiers: Vec<String>, referrer_doc: &AssetOrDocument, maybe_npm: Option<&StateNpmSnapshot>) -> Vec<Option<(ModuleSpecifier, MediaType)>> {
      ...
      if let Some(npm) = maybe_npm {
        if npm.node_resolver.in_npm_package(referrer) {
          ...
          results.push(Some(NodeResolution::into_specifier_and_media_type(
            npm.node_resolver.resolve(&specifier, referrer, NodeResolutionMode::Types, &PermissionsContainer::allow_all()).ok().flatten(),
          )));
          continue;
        }
      }
  ...
  pub fn try_cache_navigation_tree(&self, specifier: &ModuleSpecifier, script_version: &str, navigation_tree: Arc<tsc::NavigationTree>) -> Result<(), AnyError> {
  ...
  pub fn update_config(&mut self, options: UpdateDocumentConfigOptions) {
    ...
    let maybe_package_json_deps = options.maybe_package_json.map(|package_json| package_json::get_local_package_json_version_reqs(package_json));
    let maybe_jsx_config = options.maybe_config_file.and_then(|cf| cf.to_maybe_jsx_import_source_config().ok().flatten());
    ...
    self.resolver = Arc::new(CliGraphResolver::new(CliGraphResolverOptions {
      ...
      maybe_vendor_dir: options.maybe_config_file.and_then(|c| c.vendor_dir_path()).as_ref(),
      bare_node_builtins_enabled: options.maybe_config_file.map(|config| config.json.unstable.contains(&"bare-node-builtins".to_string())).unwrap_or(false),
    }));
    self.imports = Arc::new(if let Some(Ok(imports)) = options.maybe_config_file.map(|cf| cf.to_maybe_imports()) {
      imports
        .into_iter()
        .map(|(referrer, imports)| {
          let graph_import = GraphImport::new(&referrer, imports, Some(self.get_resolver()), Some(self.get_npm_resolver()));
    ...
    if self.resolver_config_hash != new_resolver_config_hash {
      self.refresh_dependencies(options.enabled_paths, options.disabled_paths, options.document_preload_limit);
  ...
  fn refresh_dependencies(&mut self, enabled_paths: Vec<PathBuf>, disabled_paths: Vec<PathBuf>, document_preload_limit: usize) {
  ...
  fn calculate_dependents_if_dirty(&mut self) {
    ...
    while let Some(specifier) = doc_analyzer.pending_specifiers.pop_front() {
      if let Some(doc) = self.open_docs.get(&specifier) {
        doc_analyzer.analyze_doc(&specifier, doc);
      } else if let Some(doc) = file_system_docs.get(&self.cache, resolver, &specifier, npm_resolver) {
        doc_analyzer.analyze_doc(&specifier, &doc);
      }
    }
  ...
  fn resolve_dependency(&self, specifier: &ModuleSpecifier, maybe_npm: Option<&StateNpmSnapshot>, referrer: &ModuleSpecifier) -> Option<(ModuleSpecifier, MediaType)> {
...
fn node_resolve_npm_req_ref(npm_req_ref: NpmPackageReqReference, maybe_npm: Option<&StateNpmSnapshot>, referrer: &ModuleSpecifier) -> Option<(ModuleSpecifier, MediaType)> {
  maybe_npm.map(|npm| {
    NodeResolution::into_specifier_and_media_type(npm.npm_resolver.resolve_pkg_folder_from_deno_module_req(npm_req_ref.req(), referrer).ok().and_then(|package_folder| {
      npm
        .node_resolver
        .resolve_package_subpath_from_deno_module(&package_folder, npm_req_ref.sub_path(), referrer, NodeResolutionMode::Types, &PermissionsContainer::allow_all())
        .ok()
        .flatten()
    }))
  })
...
impl<'a> deno_graph::source::Loader for OpenDocumentsGraphLoader<'a> {
  ...
  fn load(&mut self, specifier: &ModuleSpecifier, is_dynamic: bool, cache_setting: deno_graph::source::CacheSetting) -> deno_graph::source::LoadFuture {
  ...
  fn cache_module_info(&mut self, specifier: &deno_ast::ModuleSpecifier, source: &str, module_info: &deno_graph::ModuleInfo) {
    self.inner_loader.cache_module_info(specifier, source, module_info)
  }
}
...
fn parse_and_analyze_module(
  ...
  let module_result = analyze_module(specifier, &parsed_source_result, maybe_headers, resolver, npm_resolver);
...
fn analyze_module(
  ...
  match parsed_source_result {
    Ok(parsed_source) => Ok(deno_graph::parse_module_from_ast(deno_graph::GraphKind::All, specifier, maybe_headers, parsed_source, Some(resolver), Some(npm_resolver))),
    Err(err) => Err(deno_graph::ModuleGraphError::ModuleError(deno_graph::ModuleError::ParseErr(specifier.clone(), err.clone()))),
  }
}
...
impl PreloadDocumentFinder {
  pub fn new(options: PreloadDocumentFinderOptions) -> Self {
    fn paths_into_globs_and_paths(input_paths: Vec<PathBuf>) -> (glob::GlobSet, HashSet<PathBuf>) {
      let mut globs = Vec::with_capacity(input_paths.len());
      let mut paths = HashSet::with_capacity(input_paths.len());
      for path in input_paths {
        if let Ok(Some(glob)) = glob::GlobPattern::new_if_pattern(&path.to_string_lossy()) {
          globs.push(glob);
        } else {
          paths.insert(path);
        }
    ...
    let (disabled_globs, disabled_paths) = paths_into_globs_and_paths(options.disabled_paths);
    ...
    for path in options.enabled_paths {
      if !finder.disabled_paths.contains(&path) && !finder.disabled_globs.matches_path(&path) {
        if path.is_dir() {
          ...
          while let Some(entry) = entries.next() {
            ...
            if let Ok(entry) = entry {
              ...
              if let Ok(file_type) = entry.file_type() {
                if !self.disabled_paths.contains(&path) && !self.disabled_globs.matches_path(&path) {
