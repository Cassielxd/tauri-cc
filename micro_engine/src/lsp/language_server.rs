...
impl LanguageServer {
  pub fn new(client: Client, token: CancellationToken) -> Self {
    Self(Arc::new(tokio::sync::RwLock::new(Inner::new(client))), token)
  }
  ...
  pub async fn cache_request(&self, params: Option<Value>) -> LspResult<Option<Value>> {
    async fn create_graph_for_caching(cli_options: CliOptions, roots: Vec<ModuleSpecifier>, open_docs: Vec<Document>) -> Result<(), AnyError> {
      ...
      let graph = module_graph_builder.create_graph_with_loader(GraphKind::All, roots.clone(), &mut loader).await?;
  ...
  pub async fn virtual_text_document(&self, params: Option<Value>) -> LspResult<Option<Value>> {
    match params.map(serde_json::from_value) {
      Some(Ok(params)) => Ok(Some(serde_json::to_value(self.0.read().await.virtual_text_document(params)?).map_err(|err| {
  ...
  pub async fn refresh_configuration(&self) {
    let (client, folders, capable) = {
      ...
      (ls.client.clone(), ls.config.workspace_folders.clone(), ls.config.client_capabilities.workspace_configuration)
    };
    if capable {
      ...
      let configs = client.when_outside_lsp_lock().workspace_configuration(scopes).await;
      if let Ok(configs) = configs {
        ...
        ls.config.set_workspace_settings(unscoped, Some(by_workspace_folder));
      }
    }
  }
}
...
impl Inner {
  fn new(client: Client) -> Self {
    ...
    let module_registries = ModuleRegistry::new(module_registries_location.clone(), http_client.clone());
    let npm_search_api = CliNpmSearchApi::new(module_registries.file_fetcher.clone(), None);
    let location = dir.deps_folder_path();
    let deps_http_cache = Arc::new(GlobalHttpCache::new(location, crate::cache::RealDenoCacheEnv));
    ...
    let diagnostics_server = DiagnosticsServer::new(client.clone(), performance.clone(), ts_server.clone(), diagnostics_state.clone());
  ...
  pub async fn get_navigation_tree(&self, specifier: &ModuleSpecifier) -> Result<Arc<tsc::NavigationTree>, AnyError> {
    let mark = self.performance.mark_with_args("lsp.get_navigation_tree", json!({ "specifier": specifier }));
    ...
    let navigation_tree = if let Some(navigation_tree) = asset_or_doc.maybe_navigation_tree() {
      ...
      match asset_or_doc {
        ...
        AssetOrDocument::Document(doc) => self.documents.try_cache_navigation_tree(specifier, &doc.script_version(), navigation_tree.clone())?,
      }
      navigation_tree
    };
    ...
    if let Some(config_str) = maybe_config {
      if !config_str.is_empty() {
        ...
        let config_url = if let Ok(url) = Url::from_file_path(config_str) {
          ...
          root_uri.join(config_str).map_err(|_| anyhow!("Bad file path for configuration file: \"{}\"", config_str))
        } else {
  ...
  fn is_diagnosable(&self, specifier: &ModuleSpecifier) -> bool {
    if specifier.scheme() == "asset" {
      matches!(
        ...
        MediaType::JavaScript | MediaType::Jsx | MediaType::Mjs | MediaType::Cjs | MediaType::TypeScript | MediaType::Tsx | MediaType::Mts | MediaType::Cts | MediaType::Dts | MediaType::Dmts | MediaType::Dcts
      )
    } else {
  ...
  pub fn snapshot(&self) -> Arc<StateSnapshot> {
    let maybe_state_npm_snapshot = self.npm.resolver.as_ref().map(|resolver| resolver.clone_snapshotted()).map(|resolver| {
      let fs = Arc::new(deno_fs::RealFs);
      let node_resolver = Arc::new(NodeResolver::new(fs, resolver.clone().into_npm_resolver()));
      StateNpmSnapshot { node_resolver, npm_resolver: resolver }
    });
  ...
  pub async fn update_cache(&mut self) -> Result<(), AnyError> {
    ...
    let maybe_global_cache_path = if let Some(cache_str) = maybe_cache {
      ...
      lsp_log!("  Resolved global cache path: \"{}\"", cache_path.to_string_lossy());
      Some(cache_path)
    } else {
      None
    };
    if self.maybe_global_cache_path != maybe_global_cache_path {
      self.set_new_global_cache_path(maybe_global_cache_path).await?;
    }
    Ok(())
  }

  async fn recreate_http_client_and_dependents(&mut self) -> Result<(), AnyError> {
    self.set_new_global_cache_path(self.maybe_global_cache_path.clone()).await
  }

  /// Recreates the http client and all dependent structs.
  async fn set_new_global_cache_path(&mut self, new_cache_path: Option<PathBuf>) -> Result<(), AnyError> {
    let dir = DenoDir::new(new_cache_path.clone())?;
    let workspace_settings = self.config.workspace_settings();
    let maybe_root_path = self.config.root_uri().and_then(|uri| specifier_to_file_path(uri).ok());
    let root_cert_store = get_root_cert_store(maybe_root_path, workspace_settings.certificate_stores.clone(), workspace_settings.tls_certificate.clone().map(CaData::File))?;
    let root_cert_store_provider = Arc::new(LspRootCertStoreProvider(root_cert_store));
    let module_registries_location = dir.registries_folder_path();
    self.http_client = Arc::new(HttpClient::new(Some(root_cert_store_provider), workspace_settings.unsafely_ignore_certificate_errors.clone()));
    self.module_registries = ModuleRegistry::new(module_registries_location.clone(), self.http_client.clone());
    self.npm.search_api = CliNpmSearchApi::new(self.module_registries.file_fetcher.clone(), None);
    self.module_registries_location = module_registries_location;
    // update the cache path
    let global_cache = Arc::new(GlobalHttpCache::new(dir.deps_folder_path(), crate::cache::RealDenoCacheEnv));
    let maybe_local_cache = self.config.maybe_vendor_dir_path().map(|local_path| Arc::new(LocalLspHttpCache::new(local_path, global_cache.clone())));
    let cache: Arc<dyn HttpCache> = maybe_local_cache.clone().map(|c| c as Arc<dyn HttpCache>).unwrap_or(global_cache);
  ...
  async fn recreate_npm_services_if_necessary(&mut self) {
    ...
    let npm_resolver = create_npm_resolver(&deno_dir, &self.http_client, self.config.maybe_config_file(), self.config.maybe_lockfile(), self.config.maybe_node_modules_dir_path().cloned()).await;
    self.npm.node_resolver = Some(Arc::new(NodeResolver::new(Arc::new(deno_fs::RealFs), npm_resolver.clone().into_npm_resolver())));
  ...
  async fn fetch_import_map(&self, import_map_url: &ModuleSpecifier, cache_setting: CacheSetting) -> Result<ImportMap, AnyError> {
    resolve_import_map_from_specifier(import_map_url, self.config.maybe_config_file(), &self.create_file_fetcher(cache_setting))
      .await
  ...
  fn create_file_fetcher(&self, cache_setting: CacheSetting) -> FileFetcher {
    let mut file_fetcher = FileFetcher::new(self.deps_http_cache.clone(), cache_setting, true, self.http_client.clone(), Default::default(), None);
    file_fetcher.set_download_log_level(super::logging::lsp_log_level());
    file_fetcher
  }

  fn resolve_import_map_specifier(&self) -> Result<Option<ModuleSpecifier>, AnyError> {
    Ok(if let Some(import_map_str) = self.config.workspace_settings().import_map.clone().and_then(|s| if s.is_empty() { None } else { Some(s) }) {
      ...
      if config_file.is_an_import_map() {
        ...
        let specifier = if let Ok(config_file_path) = config_file.specifier.to_file_path() {
          let import_map_file_path = config_file_path.parent().ok_or_else(|| anyhow!("Bad config file specifier: {}", config_file.specifier))?.join(&import_map_path);
...
async fn create_npm_resolver(deno_dir: &DenoDir, http_client: &Arc<HttpClient>, maybe_config_file: Option<&ConfigFile>, maybe_lockfile: Option<&Arc<Mutex<Lockfile>>>, maybe_node_modules_dir_path: Option<PathBuf>) -> Arc<dyn CliNpmResolver> {
  let is_byonm = std::env::var("DENO_UNSTABLE_BYONM").as_deref() == Ok("1") || maybe_config_file.as_ref().map(|c| c.json.unstable.iter().any(|c| c == "byonm")).unwrap_or(false);
  create_cli_npm_resolver_for_lsp(if is_byonm {
    CliNpmResolverCreateOptions::Byonm(CliNpmResolverByonmCreateOptions {
      fs: Arc::new(deno_fs::RealFs),
      root_node_modules_dir: std::env::current_dir().unwrap().join("node_modules"),
    })
  } else {
...
impl Inner {
  async fn initialize(&mut self, params: InitializeParams) -> LspResult<InitializeResult> {
    ...
    let capabilities = capabilities::server_capabilities(&params.capabilities, enable_builtin_commands);
...

    {
      if let Some(options) = params.initialization_options {
        self.config.set_workspace_settings(WorkspaceSettings::from_initialization_options(options), None);
      }
      if let Some(folders) = params.workspace_folders {
        self.config.workspace_folders = folders.into_iter().map(|folder| (self.url_map.normalize_url(&folder.uri, LspUrlKind::Folder), folder)).collect();
      }
      // rootUri is deprecated by the LSP spec. If it's specified, merge it into
      // workspace_folders.
      if let Some(root_uri) = params.root_uri {
        if !self.config.workspace_folders.iter().any(|(_, f)| f.uri == root_uri) {
          let name = root_uri.path_segments().and_then(|s| s.last());
          let name = name.unwrap_or_default().to_string();
          self.config.workspace_folders.insert(0, (self.url_map.normalize_url(&root_uri, LspUrlKind::Folder), WorkspaceFolder { uri: root_uri, name }));
        }
      }
  ...
  async fn did_open(&mut self, specifier: &ModuleSpecifier, params: DidOpenTextDocumentParams) -> Document {
    ...
    if language_id == LanguageId::Unknown {
      lsp_warn!("Unsupported language id \"{}\" received for document \"{}\".", params.text_document.language_id, params.text_document.uri);
    }
    let document = self
      .documents
      .open(specifier.clone(), params.text_document.version, params.text_document.language_id.parse().unwrap(), params.text_document.text.into());
  ...
  async fn did_change_watched_files(&mut self, params: DidChangeWatchedFilesParams) {
    ...
    fn has_config_changed(config: &Config, changes: &IndexSet<Url>) -> bool {
      ...
      if config.maybe_config_file_canonicalized_specifier().map(|s| changes.contains(s)).unwrap_or(false) {
        return true;
      }
      match config.maybe_config_file() {
        ...
        None => {
          ...
          if changes.iter().any(|url| url.path().ends_with("/deno.json") || url.path().ends_with("/deno.jsonc")) {
            return true;
          }
        }
      }
      ...
      if let Some(lockfile) = config.maybe_lockfile() {
        let lockfile_matches = config
          ...
          .or_else(|| ModuleSpecifier::from_file_path(&lockfile.lock().filename).ok().map(|s| changes.contains(&s)))
          .unwrap_or(false);
    ...
    let mark = self.performance.mark_with_args("lsp.did_change_watched_files", &params);
    let mut touched = false;
    let changes: IndexSet<Url> = params.changes.iter().map(|f| self.url_map.normalize_url(&f.uri, LspUrlKind::File)).collect();
    ...
    if has_config_changed(&self.config, &changes) {
      ...
      config_changes.extend(params.changes.iter().filter(|e| files_to_check.contains(&e.uri)).map(|e| lsp_custom::DenoConfigurationChangeEvent {
        file_event: e.clone(),
    ...
    let has_package_json_changed = changes.iter().any(|e| e.as_str().ends_with("/package.json"));
    ...
    if !config_changes.is_empty() {
      self
        .client
        .send_did_change_deno_configuration_notification(lsp_custom::DidChangeDenoConfigurationNotificationParams { changes: config_changes.into_iter().collect() });
    }

    // if the current import map, or config file has changed, we need to
    // reload the import map
    let import_map_changed = self.maybe_import_map_uri.as_ref().map(|uri| changes.contains(uri)).unwrap_or(false);
  ...
  async fn document_symbol(&self, params: DocumentSymbolParams) -> LspResult<Option<DocumentSymbolResponse>> {
    ...
    let mark = self.performance.mark_with_args("lsp.document_symbol", &params);
  ...
  async fn formatting(&self, params: DocumentFormattingParams) -> LspResult<Option<Vec<TextEdit>>> {
    let mut specifier = self.url_map.normalize_url(&params.text_document.uri, LspUrlKind::File);
      ...
      move || {
        let format_result = match document.maybe_parsed_source() {
          ...
          None => {
            ...
            let file_path = document.maybe_language_id().and_then(|id| id.as_extension()).map(|ext| file_path.with_extension(ext)).unwrap_or(file_path);
  ...
  async fn hover(&self, params: HoverParams) -> LspResult<Option<Hover>> {
    let specifier = self.url_map.normalize_url(&params.text_document_position_params.text_document.uri, LspUrlKind::File);
    if !self.is_diagnosable(&specifier) || !self.config.specifier_enabled(&specifier) {
      return Ok(None);
    }
    ...
    let hover = if let Some((_, dep, range)) = asset_or_doc.get_maybe_dependency(&params.text_document_position_params.position) {
      ...
      let value = match (dep.maybe_code.is_none(), dep.maybe_type.is_none(), &dep_maybe_types_dependency) {
        (false, false, None) => format!("**Resolved Dependency**\n\n**Code**: {}\n\n**Types**: {}\n", to_hover_text(&dep.maybe_code), to_hover_text(&dep.maybe_type)),
        ...
        (false, false, Some(_)) => format!("**Resolved Dependency**\n\n**Code**: {}\n\n**Types**: {}\n", to_hover_text(&dep.maybe_code), to_hover_text(&dep.maybe_type)),
        (false, true, Some(types_dep)) if !types_dep.is_none() => format!("**Resolved Dependency**\n\n**Code**: {}\n\n**Types**: {}\n", to_hover_text(&dep.maybe_code), to_hover_text(types_dep)),
        (false, true, _) => format!("**Resolved Dependency**\n\n**Code**: {}\n", to_hover_text(&dep.maybe_code)),
        (true, false, _) => format!("**Resolved Dependency**\n\n**Types**: {}\n", to_hover_text(&dep.maybe_type)),
        (true, true, _) => unreachable!("{}", json!(params)),
      };
      let value = if let Some(docs) = self.module_registries.get_hover(&dep).await { format!("{value}\n\n---\n\n{docs}") } else { value };
      Some(Hover {
  ...
  async fn code_action(&self, params: CodeActionParams) -> LspResult<Option<CodeActionResponse>> {
    ...
    if !fixable_diagnostics.is_empty() {
      ...
      let file_diagnostics = self.diagnostics_server.get_ts_diagnostics(&specifier, asset_or_doc.document_lsp_version());
      ...
      for diagnostic in &fixable_diagnostics {
        match diagnostic.source.as_deref() {
          Some("deno-ts") => {
            ...
            let actions = self
              ...
              .get_code_fixes(
                ...
                tsc::UserPreferences::from_config_for_specifier(&self.config, &self.fmt_options.options, &specifier),
              )
              .await;
          ...
          Some("deno") => {
            if diagnostic.code == Some(NumberOrString::String("no-cache".to_string())) || diagnostic.code == Some(NumberOrString::String("no-cache-npm".to_string())) {
              includes_no_cache = true;
            }
            code_actions.add_deno_fix_action(&specifier, diagnostic).map_err(|err| {
              error!("{}", err);
              LspError::internal_error()
            })?
          }
          Some("deno-lint") => code_actions
            .add_deno_lint_ignore_action(&specifier, diagnostic, asset_or_doc.document().map(|d| d.text_info()), asset_or_doc.maybe_parsed_source().and_then(|r| r.ok()))
            .map_err(|err| {
      ...
      if includes_no_cache {
        ...
        let uncached_deps = no_cache_diagnostics
          .iter()
          .filter_map(|d| {
            let data = serde_json::from_value::<DiagnosticDataSpecifier>(d.data.clone().into()).ok()?;
            Some(data.specifier)
          })
          .collect::<HashSet<_>>();
    ...
    // Refactor
    let only = params.context.only.as_ref().and_then(|values| values.first().map(|v| v.as_str().to_owned())).unwrap_or_default();
    let refactor_infos = self
      ...
      .get_applicable_refactors(
        ...
        Some(tsc::UserPreferences::from_config_for_specifier(&self.config, &self.fmt_options.options, &specifier)),
        only,
      )
      .await?;
    ...
    all_actions.extend(refactor::prune_invalid_actions(refactor_actions, 5).into_iter().map(CodeActionOrCommand::CodeAction));
  ...
  async fn code_action_resolve(&self, params: CodeAction) -> LspResult<CodeAction> {
    ...
    let mark = self.performance.mark_with_args("lsp.code_action_resolve", &params);
    ...
    let result = if kind.as_str().starts_with(CodeActionKind::QUICKFIX.as_str()) {
      ...
      let combined_code_actions = self
        ...
        .get_combined_code_fix(
          ...
          &code_action_data,
          (&self.fmt_options.options).into(),
          tsc::UserPreferences::from_config_for_specifier(&self.config, &self.fmt_options.options, &code_action_data.specifier),
        )
        .await?;
      ...
      let changes = if code_action_data.fix_id == "fixMissingImport" {
        fix_ts_import_changes(&code_action_data.specifier, &combined_code_actions.changes, &self.get_ts_response_import_mapper()).map_err(|err| {
      ...
      let refactor_edit_info = self
        ...
        .get_edits_for_refactor(
          ...
          Some(tsc::UserPreferences::from_config_for_specifier(&self.config, &self.fmt_options.options, &action_data.specifier)),
        )
        .await?;
  ...
  pub fn get_ts_response_import_mapper(&self) -> TsResponseImportMapper {
    TsResponseImportMapper::new(&self.documents, self.maybe_import_map.as_deref(), self.npm.node_resolver.as_deref(), self.npm.resolver.as_deref())
  }
  ...
  async fn code_lens(&self, params: CodeLensParams) -> LspResult<Option<Vec<CodeLens>>> {
    ...
    if settings.code_lens.test && self.config.specifier_enabled_for_test(&specifier) {
      if let Some(Ok(parsed_source)) = asset_or_doc.maybe_parsed_source() {
        code_lenses.extend(code_lens::collect_test(&specifier, parsed_source).map_err(|err| {
          error!("Error getting test code lenses for \"{}\": {}", &specifier, err);
          LspError::internal_error()
        })?);
      }
    }
    if settings.code_lens.implementations || settings.code_lens.references {
      ...
      let line_index = asset_or_doc.line_index();
      code_lenses.extend(code_lens::collect_tsc(&specifier, &settings.code_lens, line_index, &navigation_tree).await.map_err(|err| {
        error!("Error getting ts code lenses for \"{}\": {}", &specifier, err);
        LspError::internal_error()
      })?);
    }
    self.performance.measure(mark);
  ...
  async fn code_lens_resolve(&self, code_lens: CodeLens) -> LspResult<CodeLens> {
    let mark = self.performance.mark_with_args("lsp.code_lens_resolve", &code_lens);
  ...
  async fn document_highlight(&self, params: DocumentHighlightParams) -> LspResult<Option<Vec<DocumentHighlight>>> {
    ...
    let mark = self.performance.mark_with_args("lsp.document_highlight", &params);
    let asset_or_doc = self.get_asset_or_document(&specifier)?;
    let line_index = asset_or_doc.line_index();
    ...
    if let Some(document_highlights) = maybe_document_highlights {
      let result = document_highlights.into_iter().flat_map(|dh| dh.to_highlight(line_index.clone())).collect();
      self.performance.measure(mark);
      Ok(Some(result))
    } else {
      self.performance.measure(mark);
  ...
  async fn references(&self, params: ReferenceParams) -> LspResult<Option<Vec<Location>>> {
    let specifier = self.url_map.normalize_url(&params.text_document_position.text_document.uri, LspUrlKind::File);
    if !self.is_diagnosable(&specifier) || !self.config.specifier_enabled(&specifier) {
      return Ok(None);
    }
    ...
    let asset_or_doc = self.get_asset_or_document(&specifier)?;
    let line_index = asset_or_doc.line_index();
    let maybe_referenced_symbols = self.ts_server.find_references(self.snapshot(), specifier.clone(), line_index.offset_tsc(params.text_document_position.position)?).await?;
  ...
  async fn goto_definition(&self, params: GotoDefinitionParams) -> LspResult<Option<GotoDefinitionResponse>> {
    ...
    let mark = self.performance.mark_with_args("lsp.goto_definition", &params);
    let asset_or_doc = self.get_asset_or_document(&specifier)?;
    let line_index = asset_or_doc.line_index();
    let maybe_definition = self.ts_server.get_definition(self.snapshot(), specifier, line_index.offset_tsc(params.text_document_position_params.position)?).await?;
  ...
  async fn goto_type_definition(&self, params: GotoTypeDefinitionParams) -> LspResult<Option<GotoTypeDefinitionResponse>> {
    ...
    let asset_or_doc = self.get_asset_or_document(&specifier)?;
    let line_index = asset_or_doc.line_index();
    let maybe_definition_info = self.ts_server.get_type_definition(self.snapshot(), specifier, line_index.offset_tsc(params.text_document_position_params.position)?).await?;
  ...
  async fn completion(&self, params: CompletionParams) -> LspResult<Option<CompletionResponse>> {
    ...
    if !self.is_diagnosable(&specifier) || !self.config.specifier_enabled(&specifier) || !language_settings.map(|s| s.suggest.enabled).unwrap_or(true) {
      return Ok(None);
    }
    ...
    if language_settings.map(|s| s.suggest.include_completions_for_import_statements).unwrap_or(true) {
    ...
    if response.is_none() {
      ...
      let maybe_completion_info = self
        ...
        .get_completions(
          ...
          tsc::GetCompletionsAtPositionOptions {
            user_preferences: tsc::UserPreferences::from_config_for_specifier(&self.config, &self.fmt_options.options, &specifier),
            trigger_character,
            trigger_kind,
          },
          (&self.fmt_options.options).into(),
      ...
      if let Some(completions) = maybe_completion_info {
        response = Some(completions.as_completion_response(line_index, &self.config.language_settings_for_specifier(&specifier).cloned().unwrap_or_default().suggest, &specifier, position, self));
      }
    };
    self.performance.measure(mark);
    Ok(response)
  ...
  async fn completion_resolve(&self, params: CompletionItem) -> LspResult<CompletionItem> {
    let mark = self.performance.mark_with_args("lsp.completion_resolve", &params);
        ...
        let result = self
          ...
          .get_completion_details(
            ...
            GetCompletionDetailsArgs {
              ...
              preferences: Some(tsc::UserPreferences::from_config_for_specifier(&self.config, &self.fmt_options.options, specifier)),
              ..data.into()
            },
          )
          .await;
  ...
  async fn goto_implementation(&self, params: GotoImplementationParams) -> LspResult<Option<GotoImplementationResponse>> {
    ...
    let mark = self.performance.mark_with_args("lsp.goto_implementation", &params);
    let asset_or_doc = self.get_asset_or_document(&specifier)?;
    let line_index = asset_or_doc.line_index();

    let maybe_implementations = self.ts_server.get_implementations(self.snapshot(), specifier, line_index.offset_tsc(params.text_document_position_params.position)?).await?;
  ...
  async fn folding_range(&self, params: FoldingRangeParams) -> LspResult<Option<Vec<FoldingRange>>> {
    ...
    let mark = self.performance.mark_with_args("lsp.folding_range", &params);
    let asset_or_doc = self.get_asset_or_document(&specifier)?;
    ...
    let response = if !outlining_spans.is_empty() {
      Some(
        outlining_spans
          .iter()
          .map(|span| span.to_folding_range(asset_or_doc.line_index(), asset_or_doc.text().as_bytes(), self.config.client_capabilities.line_folding_only))
  ...
  async fn incoming_calls(&self, params: CallHierarchyIncomingCallsParams) -> LspResult<Option<Vec<CallHierarchyIncomingCall>>> {
    ...
    let mark = self.performance.mark_with_args("lsp.incoming_calls", &params);
    let asset_or_doc = self.get_asset_or_document(&specifier)?;
    let line_index = asset_or_doc.line_index();

    let incoming_calls: Vec<tsc::CallHierarchyIncomingCall> = self.ts_server.provide_call_hierarchy_incoming_calls(self.snapshot(), specifier, line_index.offset_tsc(params.item.selection_range.start)?).await?;

    let maybe_root_path_owned = self.config.root_uri().and_then(|uri| specifier_to_file_path(uri).ok());
  ...
  async fn outgoing_calls(&self, params: CallHierarchyOutgoingCallsParams) -> LspResult<Option<Vec<CallHierarchyOutgoingCall>>> {
    ...
    let mark = self.performance.mark_with_args("lsp.outgoing_calls", &params);
    let asset_or_doc = self.get_asset_or_document(&specifier)?;
    let line_index = asset_or_doc.line_index();

    let outgoing_calls: Vec<tsc::CallHierarchyOutgoingCall> = self.ts_server.provide_call_hierarchy_outgoing_calls(self.snapshot(), specifier, line_index.offset_tsc(params.item.selection_range.start)?).await?;
  ...
  async fn prepare_call_hierarchy(&self, params: CallHierarchyPrepareParams) -> LspResult<Option<Vec<CallHierarchyItem>>> {
    ...
    let mark = self.performance.mark_with_args("lsp.prepare_call_hierarchy", &params);
    let asset_or_doc = self.get_asset_or_document(&specifier)?;
    let line_index = asset_or_doc.line_index();

    let maybe_one_or_many = self.ts_server.prepare_call_hierarchy(self.snapshot(), specifier, line_index.offset_tsc(params.text_document_position_params.position)?).await?;
  ...
  async fn rename(&self, params: RenameParams) -> LspResult<Option<WorkspaceEdit>> {
    ...
    let maybe_locations = self.ts_server.find_rename_locations(self.snapshot(), specifier, line_index.offset_tsc(params.text_document_position.position)?).await?;
  ...
  async fn selection_range(&self, params: SelectionRangeParams) -> LspResult<Option<Vec<SelectionRange>>> {
    ...
    let mark = self.performance.mark_with_args("lsp.selection_range", &params);
    let asset_or_doc = self.get_asset_or_document(&specifier)?;
    let line_index = asset_or_doc.line_index();

    let mut selection_ranges = Vec::<SelectionRange>::new();
    for position in params.positions {
      let selection_range: tsc::SelectionRange = self.ts_server.get_smart_selection_range(self.snapshot(), specifier.clone(), line_index.offset_tsc(position)?).await?;
  ...
  async fn semantic_tokens_full(&self, params: SemanticTokensParams) -> LspResult<Option<SemanticTokensResult>> {
    ...
    let mark = self.performance.mark_with_args("lsp.semantic_tokens_full", &params);
    let asset_or_doc = self.get_asset_or_document(&specifier)?;
    let line_index = asset_or_doc.line_index();

    let semantic_classification = self.ts_server.get_encoded_semantic_classifications(self.snapshot(), specifier, 0..line_index.text_content_length_utf16().into()).await?;

    let semantic_tokens = semantic_classification.to_semantic_tokens(&asset_or_doc, line_index)?;
    let response = if !semantic_tokens.data.is_empty() { Some(SemanticTokensResult::Tokens(semantic_tokens)) } else { None };
    self.performance.measure(mark);
    Ok(response)
  }
  ...
  async fn semantic_tokens_range(&self, params: SemanticTokensRangeParams) -> LspResult<Option<SemanticTokensRangeResult>> {
    ...
    let mark = self.performance.mark_with_args("lsp.semantic_tokens_range", &params);
    let asset_or_doc = self.get_asset_or_document(&specifier)?;
    let line_index = asset_or_doc.line_index();

    let semantic_classification = self
      .ts_server
      .get_encoded_semantic_classifications(self.snapshot(), specifier, line_index.offset_tsc(params.range.start)?..line_index.offset_tsc(params.range.end)?)
      .await?;

    let semantic_tokens = semantic_classification.to_semantic_tokens(&asset_or_doc, line_index)?;
    let response = if !semantic_tokens.data.is_empty() { Some(SemanticTokensRangeResult::Tokens(semantic_tokens)) } else { None };
    self.performance.measure(mark);
    Ok(response)
  }
  ...
  async fn signature_help(&self, params: SignatureHelpParams) -> LspResult<Option<SignatureHelp>> {
    ...
    let mark = self.performance.mark_with_args("lsp.signature_help", &params);
    let asset_or_doc = self.get_asset_or_document(&specifier)?;
    let line_index = asset_or_doc.line_index();
  ...
  async fn will_rename_files(&self, params: RenameFilesParams) -> LspResult<Option<WorkspaceEdit>> {
    ...
    for rename in params.files {
      let old_specifier = self.url_map.normalize_url(&resolve_url(&rename.old_uri).unwrap(), LspUrlKind::File);
      let options = self.config.language_settings_for_specifier(&old_specifier).map(|s| s.update_imports_on_file_move.clone()).unwrap_or_default();
      ...
      changes.extend(
        self
          ...
          .get_edits_for_file_rename(
            self.snapshot(),
            old_specifier,
            self.url_map.normalize_url(&resolve_url(&rename.new_uri).unwrap(), LspUrlKind::File),
            (&self.fmt_options.options).into(),
            tsc::UserPreferences {
...
impl tower_lsp::LanguageServer for LanguageServer {
  async fn execute_command(&self, params: ExecuteCommandParams) -> LspResult<Option<Value>> {
    if params.command == "deno.cache" {
      ...
      let uris: Vec<Url> = serde_json::from_value(uris).map_err(|err| LspError::invalid_params(err.to_string()))?;
      let referrer = serde_json::to_value(arguments.next()).unwrap();
      let referrer: Url = serde_json::from_value(referrer).map_err(|err| LspError::invalid_params(err.to_string()))?;
      self
        .cache_request(Some(
          serde_json::to_value(lsp_custom::CacheParams {
            referrer: TextDocumentIdentifier { uri: referrer },
            uris: uris.into_iter().map(|uri| TextDocumentIdentifier { uri }).collect(),
          })
  ...
  async fn initialized(&self, _: InitializedParams) {
    ...
    let (client, http_client) = {
      ...
      if ls.config.client_capabilities.testing_api {
        let test_server = testing::TestServer::new(ls.client.clone(), ls.performance.clone(), ls.config.root_uri().cloned());
    ...
    if upgrade_check_enabled() {
      ...
      spawn(async move {
        match check_for_upgrades_for_lsp(http_client).await {
          Ok(version_info) => {
            client.send_did_upgrade_check_notification(lsp_custom::DidUpgradeCheckNotificationParams {
  ...
  async fn did_open(&self, params: DidOpenTextDocumentParams) {
    ...
    let specifier = inner.url_map.normalize_url(&params.text_document.uri, LspUrlKind::File);
  ...
  async fn did_save(&self, params: DidSaveTextDocumentParams) {
    let uri = &params.text_document.uri;
    {
      ...
      if !inner.config.workspace_settings_for_specifier(&specifier).cache_on_save || !inner.config.specifier_enabled(&specifier) || !inner.diagnostics_state.has_no_cache_diagnostics(&specifier) {
        return;
      }
  ...
  async fn did_change_configuration(&self, params: DidChangeConfigurationParams) {
    let mark = {
      let inner = self.0.read().await;
      inner.performance.mark_with_args("lsp.did_change_configuration", &params)
    };
  ...
  async fn did_change_workspace_folders(&self, params: DidChangeWorkspaceFoldersParams) {
    let (performance, mark) = {
      let mut ls = self.0.write().await;
      let mark = ls.performance.mark_with_args("lsp.did_change_workspace_folders", &params);
...
impl Inner {
  fn prepare_cache(&self, params: lsp_custom::CacheParams) -> Result<Option<PrepareCacheResult>, AnyError> {
    ...
    Ok(Some(PrepareCacheResult { cli_options, open_docs, roots, mark }))
  }
  ...
  async fn inlay_hint(&self, params: InlayHintParams) -> LspResult<Option<Vec<InlayHint>>> {
    let specifier = self.url_map.normalize_url(&params.text_document.uri, LspUrlKind::File);
    if !self.is_diagnosable(&specifier) || !self.config.specifier_enabled(&specifier) || !self.config.enabled_inlay_hints_for_specifier(&specifier) {
      return Ok(None);
    }
    ...
    let maybe_inlay_hints = self
      .ts_server
      .provide_inlay_hints(self.snapshot(), specifier.clone(), text_span, tsc::UserPreferences::from_config_for_specifier(&self.config, &self.fmt_options.options, &specifier))
      .await?;
  ...
  fn virtual_text_document(&self, params: lsp_custom::VirtualTextDocumentParams) -> LspResult<Option<String>> {
    let mark = self.performance.mark_with_args("lsp.virtual_text_document", &params);
    let specifier = self.url_map.normalize_url(&params.text_document.uri, LspUrlKind::File);
    let contents = if specifier.scheme() == "deno" && specifier.path() == "/status.md" {
      let mut contents = String::new();
      let mut documents_specifiers = self.documents.documents(DocumentsFilter::All).into_iter().map(|d| d.specifier().clone()).collect::<Vec<_>>();
...
"#,
        ...
        documents_specifiers.into_iter().map(|s| s.to_string()).collect::<Vec<String>>().join("\n    - "),
        measures.len(),
