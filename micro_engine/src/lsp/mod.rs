...
pub async fn start() -> Result<(), AnyError> {
  ...
  let builder = LspService::build(|client| language_server::LanguageServer::new(client::Client::from_tower(client), token.clone()))
  ...
  let builder = if should_send_diagnostic_batch_index_notifications() {
    builder.custom_method(lsp_custom::LATEST_DIAGNOSTIC_BATCH_INDEX, LanguageServer::latest_diagnostic_batch_index_request)
  } else {
    builder
  };
