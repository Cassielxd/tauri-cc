...
pub trait NpmSearchApi {
  ...
  async fn package_info(&self, name: &str) -> Result<Arc<NpmPackageInfo>, AnyError>;
}
...
impl CliNpmSearchApi {
  pub fn new(file_fetcher: FileFetcher, custom_base_url: Option<Url>) -> Self {
    Self {
      base_url: custom_base_url.unwrap_or_else(|| npm_registry_default_url().clone()),
      file_fetcher,
...
impl NpmSearchApi for CliNpmSearchApi {
  async fn search(&self, query: &str) -> Result<Arc<Vec<String>>, AnyError> {
    ...
    search_url.path_segments_mut().map_err(|_| anyhow!("Custom npm registry URL cannot be a base."))?.pop_if_empty().extend("-/v1/search".split('/'));
    search_url.query_pairs_mut().append_pair("text", &format!("{} boost-exact:false", query));
    let file = self.file_fetcher.fetch(&search_url, PermissionsContainer::allow_all()).await?;
    let names = Arc::new(parse_npm_search_response(&file.source)?);
    self.search_cache.lock().insert(query.to_string(), names.clone());
    Ok(names)
  }
  ...
  async fn package_info(&self, name: &str) -> Result<Arc<NpmPackageInfo>, AnyError> {
    ...
    info_url.path_segments_mut().map_err(|_| anyhow!("Custom npm registry URL cannot be a base."))?.pop_if_empty().push(name);
    let file = self.file_fetcher.fetch(&info_url, PermissionsContainer::allow_all()).await?;
    let info = Arc::new(serde_json::from_str::<NpmPackageInfo>(&file.source)?);
    self.info_cache.lock().insert(name.to_string(), info.clone());
    Ok(info)
  }
}
