...
impl Performance {
  ...
  pub fn averages(&self) -> Vec<PerformanceAverage> {
    ...
    for measure in self.measures.lock().iter() {
      averages.entry(measure.name.clone()).or_default().push(measure.duration);
    }
    averages
      .into_iter()
      .map(|(k, d)| {
  ...
  fn mark_inner<S: AsRef<str>, V: Serialize>(&self, name: S, maybe_args: Option<V>) -> PerformanceMark {
