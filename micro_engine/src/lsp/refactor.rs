...
pub static REWRITE_ARROW_BRACES: Lazy<RefactorCodeActionKind> = Lazy::new(|| RefactorCodeActionKind {
...
pub static REWRITE_PARAMETERS_TO_DESTRUCTURED: Lazy<RefactorCodeActionKind> = Lazy::new(|| RefactorCodeActionKind {
...
pub static REWRITE_PROPERTY_GENERATEACCESSORS: Lazy<RefactorCodeActionKind> = Lazy::new(|| RefactorCodeActionKind {
...
pub static ALL_KNOWN_REFACTOR_ACTION_KINDS: Lazy<Vec<&'static RefactorCodeActionKind>> = Lazy::new(|| {
  vec![
    &EXTRACT_FUNCTION,
    &EXTRACT_CONSTANT,
    &EXTRACT_TYPE,
...
pub fn prune_invalid_actions(actions: Vec<lsp::CodeAction>, number_of_invalid: usize) -> Vec<lsp::CodeAction> {
  ...
  for action in actions {
    ...
    if action_kind.starts_with(EXTRACT_CONSTANT.kind.as_str()) || action_kind.starts_with(EXTRACT_FUNCTION.kind.as_str()) {
  ...
  let top_n_invalid = prioritized_actions[0..std::cmp::min(number_of_invalid, prioritized_actions.len())].to_vec();
