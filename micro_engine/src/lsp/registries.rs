...
enum CompletionType {
  Literal(String),
  Key { key: Key, prefix: Option<String>, index: usize },
}

/// Determine if a completion at a given offset is a string literal or a key/
/// variable.
fn get_completion_type(offset: usize, tokens: &[Token], match_result: &MatchResult) -> Option<CompletionType> {
  let mut len = 0_usize;
    ...
    match token {
      ...
      Token::Key(k) => {
        ...
        if let StringOrNumber::String(name) = &k.name {
          let value = match_result.get(name).map(|s| s.to_string(Some(k), false)).unwrap_or_default();
          len += value.chars().count();
          if offset <= len {
            return Some(CompletionType::Key { key: k.clone(), prefix: None, index });
          }
        }
...
/// configuration
fn get_data(registry: &RegistryConfiguration, base: &ModuleSpecifier, variable: &Key, value: &str) -> Option<Value> {
  let url = registry.get_documentation_url_for_key(variable)?;
  get_endpoint(url, base, variable, Some(value)).ok().map(|specifier| json!({ "documentation": specifier }))
}
...
fn get_data_with_match(registry: &RegistryConfiguration, base: &ModuleSpecifier, tokens: &[Token], match_result: &MatchResult, variable: &Key, value: &str) -> Option<Value> {
  let url = registry.get_documentation_url_for_key(variable)?;
  get_endpoint_with_match(variable, url, base, tokens, match_result, Some(value)).ok().map(|specifier| json!({ "documentation": specifier }))
}

/// Convert a single variable templated string into a fully qualified URL which
/// can be fetched to provide additional data.
fn get_endpoint(url: &str, base: &Url, variable: &Key, maybe_value: Option<&str>) -> Result<ModuleSpecifier, AnyError> {
...
fn get_endpoint_with_match(variable: &Key, url: &str, base: &Url, tokens: &[Token], match_result: &MatchResult, maybe_value: Option<&str>) -> Result<ModuleSpecifier, AnyError> {
  ...
  for (key, value) in match_result.params.iter() {
    if let StringOrNumber::String(name) = key {
      ...
      url = url.replace(&format!("${{{{{name}}}}}"), &percent_encoding::percent_encode(value.to_string(maybe_key, true).as_bytes(), COMPONENT).to_string());
    }
  }
  if !has_value {
...
fn parse_replacement_variables<S: AsRef<str>>(s: S) -> Vec<String> {
  REPLACEMENT_VARIABLE_RE.captures_iter(s.as_ref()).filter_map(|c| c.get(1).map(|m| m.as_str().to_string())).collect()
}
...
fn replace_variable(url: &str, variable: &Key, maybe_value: Option<&str>) -> String {
  ...
  if let StringOrNumber::String(name) = &variable.name {
    url_str.replace(&format!("${{{name}}}"), value).replace(&format! {"${{{{{name}}}}}"}, value)
  } else {
    url_str
  }
}
...
fn validate_config(config: &RegistryConfigurationJson) -> Result<(), AnyError> {
  if config.version < 1 || config.version > 2 {
    return Err(anyhow!("Invalid registry configuration. Expected version 1 or 2 got {}.", config.version));
  }
  for registry in &config.registries {
    let (_, keys) = string_to_regex(&registry.schema, None)?;
    let key_names: Vec<String> = keys
      .map(|keys| keys.iter().filter_map(|k| if let StringOrNumber::String(s) = &k.name { Some(s.clone()) } else { None }).collect())
      .unwrap_or_default();

    for key_name in &key_names {
      if !registry.variables.iter().map(|var| var.key.to_owned()).any(|x| x == *key_name) {
...
impl RegistryConfiguration {
  fn get_url_for_key(&self, key: &Key) -> Option<&str> {
    self.variables.iter().find_map(|v| if key.name == StringOrNumber::String(v.key.clone()) { Some(v.url.as_str()) } else { None })
  }

  fn get_documentation_url_for_key(&self, key: &Key) -> Option<&str> {
    self.variables.iter().find_map(|v| if key.name == StringOrNumber::String(v.key.clone()) { v.documentation.as_deref() } else { None })
  }
}
...
impl ModuleRegistry {
  pub fn new(location: PathBuf, http_client: Arc<HttpClient>) -> Self {
    ...
    let http_cache = Arc::new(GlobalHttpCache::new(location, crate::cache::RealDenoCacheEnv));
    let mut file_fetcher = FileFetcher::new(http_cache.clone(), CacheSetting::RespectHeaders, true, http_client, Default::default(), None);
  ...
  fn complete_literal(&self, s: String, completions: &mut HashMap<String, lsp::CompletionItem>, current_specifier: &str, offset: usize, range: &lsp::Range) {
    let label = if s.starts_with('/') { s[0..].to_string() } else { s.to_string() };
    let full_text = format!("{}{}{}", &current_specifier[..offset], s, &current_specifier[offset..]);
    let text_edit = Some(lsp::CompletionTextEdit::Edit(lsp::TextEdit { range: *range, new_text: full_text.clone() }));
    ...
    completions.insert(
      ...
      lsp::CompletionItem {
        ...
        text_edit,
        commit_characters: Some(REGISTRY_IMPORT_COMMIT_CHARS.iter().map(|&c| c.into()).collect()),
        ..Default::default()
      },
    );
  }
  ...
  async fn fetch_config(&self, specifier: &ModuleSpecifier) -> Result<Vec<RegistryConfiguration>, AnyError> {
  ...
  pub async fn enable(&mut self, origin: &str) -> Result<(), AnyError> {
    ...
    if !self.origins.contains_key(&origin) {
      ...
      match self.fetch_config(&specifier).await {
        ...
        Err(err) => {
          lsp_log!("  Error fetching registry config for \"{}\": {}", origin, err.to_string());
          self.origins.remove(&origin);
        }
      }
    }
  ...
  pub async fn get_hover(&self, dependency: &Dependency) -> Option<String> {
    ...
    for registry in registries {
      ...
      if let Some(match_result) = matcher.matches(path) {
        let key = if let Some(Token::Key(key)) = tokens.iter().last() { Some(key) } else { None }?;
        let url = registry.get_documentation_url_for_key(key)?;
        let endpoint = get_endpoint_with_match(key, url, specifier, &tokens, &match_result, None).ok()?;
        let file = self.file_fetcher.fetch(&endpoint, PermissionsContainer::allow_all()).await.ok()?;
  ...
  pub async fn get_completions(&self, current_specifier: &str, offset: usize, range: &lsp::Range, specifier_exists: impl Fn(&ModuleSpecifier) -> bool) -> Option<lsp::CompletionList> {
          ...
          for registry in registries {
            ...
            loop {
              let matcher = Matcher::new(&tokens[..i], None)
                .map_err(|e| {
                  error!("Error creating matcher for schema for origin \"{}\". {}", origin, e);
                })
                .ok()?;
              if let Some(match_result) = matcher.matches(path) {
                did_match = true;
                ...
                match completion_type {
                  Some(CompletionType::Literal(s)) => self.complete_literal(s, &mut completions, current_specifier, offset, range),
                  Some(CompletionType::Key { key, prefix, index }) => {
                    let maybe_url = registry.get_url_for_key(&key);
                    if let Some(url) = maybe_url {
                      if let Some(items) = self.get_variable_items(&key, url, &specifier, &tokens, &match_result).await {
                        ...
                        for (idx, item) in items.into_iter().enumerate() {
                          let mut label = if let Some(p) = &prefix { format!("{p}{item}") } else { item.clone() };
                          if label.ends_with('/') {
                            label.pop();
                          }
                          ...
                          let command = if key.name == last_key_name && !item.ends_with('/') && !specifier_exists(&item_specifier) {
                            Some(lsp::Command {
                              title: "".to_string(),
                          ...
                          let data = get_data_with_match(registry, &specifier, &tokens, &match_result, &key, &item);
                          let commit_characters = if is_incomplete {
                            Some(REGISTRY_IMPORT_COMMIT_CHARS.iter().map(|&c| c.into()).collect())
                          } else {
  ...
  pub async fn get_documentation(&self, url: &str) -> Option<lsp::Documentation> {
    ...
    let file = self.file_fetcher.fetch(&specifier, PermissionsContainer::allow_all()).await.ok()?;
    serde_json::from_str(&file.source).ok()
  }

  pub fn get_origin_completions(&self, current_specifier: &str, range: &lsp::Range) -> Option<lsp::CompletionList> {
    let items = self
      .origins
      .keys()
      .filter_map(|k| {
        ...
        if origin.starts_with(current_specifier) {
          let text_edit = Some(lsp::CompletionTextEdit::Edit(lsp::TextEdit { range: *range, new_text: origin.clone() }));
    ...
    if !items.is_empty() {
      Some(lsp::CompletionList { items, is_incomplete: false })
    } else {
      None
    }
  }
  ...
  async fn get_variable_items(&self, variable: &Key, url: &str, base: &Url, tokens: &[Token], match_result: &MatchResult) -> Option<VariableItems> {
