...
impl ReplLanguageServer {
  pub async fn new_initialized() -> Result<ReplLanguageServer, AnyError> {
    ...
    let language_server = super::language_server::LanguageServer::new(Client::new_for_repl(), Default::default());
    ...
    language_server
      .initialize(InitializeParams {
        ...
        client_info: Some(ClientInfo { name: "Deno REPL".to_string(), version: None }),
        locale: None,
      })
      .await?;
  ...
  pub async fn completions(&mut self, line_text: &str, position: usize) -> Vec<ReplCompletionItem> {
    self.did_change(line_text).await;
    let text_info = deno_ast::SourceTextInfo::from_string(format!("{}{}", self.document_text, self.pending_text));
    ...
    let response = self
      ...
      .completion(CompletionParams {
        text_document_position: TextDocumentPositionParams {
          text_document: TextDocumentIdentifier { uri: self.get_document_specifier() },
          position: Position {
    ...
    items.sort_by_key(|item| if let Some(sort_text) = &item.sort_text { sort_text.clone() } else { item.label.clone() });
    items
      .into_iter()
      .filter_map(|item| {
...
fn get_cwd_uri() -> Result<ModuleSpecifier, AnyError> {
  ...
  ModuleSpecifier::from_directory_path(&cwd).map_err(|_| anyhow!("Could not get URI from {}", cwd.display()))
}
