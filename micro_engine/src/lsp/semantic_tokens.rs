...
impl SemanticTokensBuilder {
  ...
  pub fn push(&mut self, line: u32, char: u32, length: u32, token_type: u32, token_modifiers: u32) {
    if self.data_is_sorted_and_delta_encoded && (line < self.prev_line || (line == self.prev_line && char < self.prev_char)) {
  ...
  fn data_to_semantic_token_vec(data: &[u32], data_is_sorted_and_delta_encoded: bool) -> Vec<SemanticToken> {
  ...
  pub fn build(&self, result_id: Option<String>) -> SemanticTokens {
    SemanticTokens {
      result_id,
      data: SemanticTokensBuilder::data_to_semantic_token_vec(&self.data, self.data_is_sorted_and_delta_encoded),
    }
  }
}
