...
fn visit_arrow(arrow_expr: &ast::ArrowExpr, parent_id: &str, text_info: &SourceTextInfo, test_module: &mut TestModule) {
  if let Some((maybe_test_context, maybe_step_var)) = parse_test_context_param(arrow_expr.params.get(0)) {
    let mut collector = TestStepCollector::new(maybe_test_context, maybe_step_var, parent_id, text_info, test_module);
    arrow_expr.body.visit_with(&mut collector);
  }
}

/// Parse a function for any test steps and return them.
fn visit_fn(function: &ast::Function, parent_id: &str, text_info: &SourceTextInfo, test_module: &mut TestModule) {
  if let Some((maybe_test_context, maybe_step_var)) = parse_test_context_param(function.params.get(0).map(|p| &p.pat)) {
    let mut collector = TestStepCollector::new(maybe_test_context, maybe_step_var, parent_id, text_info, test_module);
...
fn visit_call_expr(node: &ast::CallExpr, fns: Option<&HashMap<String, ast::Function>>, range: Range, parent_id: Option<&str>, text_info: &SourceTextInfo, test_module: &mut TestModule) {
    ...
    match expr {
      ...
      ast::Expr::Ident(ident) => {
        ...
        if let Some(fn_expr) = fns.and_then(|fns| fns.get(&name)) {
          let (parent_id, _) = test_module.register(name, Some(range), false, parent_id.map(str::to_owned));
      ...
      _ => {
        if parent_id.is_none() {
          ...
          test_module.register(format!("Test {}:{}", indexes.line_number, indexes.column_number), Some(range), false, None);
        }
      }
    }
  }
...
impl<'a> TestStepCollector<'a> {
  fn new(maybe_test_context: Option<String>, maybe_step_var: Option<String>, parent_id: &'a str, text_info: &'a SourceTextInfo, test_module: &'a mut TestModule) -> Self {
      ...
      match callee_expr.as_ref() {
        ...
        ast::Expr::Ident(ident) => {
          if self.vars.contains(&ident.sym.to_string()) {
            visit_call_expr(node, None, source_range_to_lsp_range(&ident.range(), self.text_info), Some(self.parent_id), self.text_info, self.test_module);
          }
        }
              ...
              if ns_prop_ident.sym.eq("step") {
                if let ast::Expr::Ident(ident) = member_expr.obj.as_ref() {
                  if ident.sym == *test_context {
                    visit_call_expr(node, None, source_range_to_lsp_range(&ns_prop_ident.range(), self.text_info), Some(self.parent_id), self.text_info, self.test_module);
                  }
                }
              }
            }
...
impl TestCollector {
  pub fn new(specifier: ModuleSpecifier, script_version: String, text_info: SourceTextInfo) -> Self {
    Self {
...
impl Visit for TestCollector {
  fn visit_call_expr(&mut self, node: &ast::CallExpr) {
    fn visit_if_deno_test(collector: &mut TestCollector, node: &ast::CallExpr, range: &deno_ast::SourceRange, ns_prop_ident: &ast::Ident, member_expr: &ast::MemberExpr) {
      if ns_prop_ident.sym == "test" {
        ...
        visit_call_expr(node, Some(&collector.fns), source_range_to_lsp_range(range, &collector.text_info), None, &collector.text_info, &mut collector.test_module);
      }
    }
    ...
    match callee_expr.as_ref() {
      ast::Expr::Ident(ident) => {
        if self.vars.contains(&ident.sym.to_string()) {
          visit_call_expr(node, Some(&self.fns), source_range_to_lsp_range(&ident.range(), &self.text_info), None, &self.text_info, &mut self.test_module);
        }
      }
      ast::Expr::Member(member_expr) => {
        ...
        visit_if_deno_test(self, node, &ns_prop_ident.range(), ns_prop_ident, member_expr);
        ...
        if ns_prop_ident_name == "ignore" || ns_prop_ident_name == "only" {
          ...
          visit_if_deno_test(self, node, &ns_prop_ident.range(), child_ns_prop_ident, child_member_expr);
        }
      }
      _ => (),
    }
