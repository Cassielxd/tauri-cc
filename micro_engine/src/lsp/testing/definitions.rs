...
impl TestModule {
  ...
  pub fn register(&mut self, name: String, range: Option<Range>, is_dynamic: bool, parent_id: Option<String>) -> (String, bool) {
  ...
  /// Returns `(id, was_newly_registered)`.
  pub fn register_step_dynamic(&mut self, desc: &TestStepDescription, parent_static_id: &str) -> (String, bool) {
    self.register(desc.name.clone(), None, true, Some(parent_static_id.to_string()))
  }
  ...
  pub fn get_test_data(&self, id: &str) -> TestData {
    fn get_test_data_inner(tm: &TestModule, id: &str) -> TestData {
      ...
      TestData {
        ...
        steps: def.step_ids.iter().map(|id| get_test_data_inner(tm, id)).collect(),
        range: def.range,
      }
    }
  ...
  pub fn as_replace_notification(&self, maybe_root_uri: Option<&ModuleSpecifier>) -> TestingNotification {
    let label = self.label(maybe_root_uri);
    TestingNotification::Module(lsp_custom::TestModuleNotificationParams {
      text_document: lsp::TextDocumentIdentifier { uri: self.specifier.clone() },
      kind: lsp_custom::TestModuleNotificationKind::Replace,
      label,
      tests: self.defs.iter().filter(|(_, def)| def.parent_id.is_none()).map(|(id, _)| self.get_test_data(id)).collect(),
    })
  }
  ...
  pub fn label(&self, maybe_root_uri: Option<&ModuleSpecifier>) -> String {
    if let Some(root) = maybe_root_uri {
      ...
      self.specifier.path_segments().and_then(|s| s.last().map(|s| s.to_string())).unwrap_or_else(|| "<unknown>".to_string())
    }
  }
