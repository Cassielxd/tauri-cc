...
fn as_queue_and_filters(params: &lsp_custom::TestRunRequestParams, tests: &HashMap<ModuleSpecifier, TestModule>) -> (HashSet<ModuleSpecifier>, HashMap<ModuleSpecifier, LspTestFilter>) {
...
fn as_test_messages<S: AsRef<str>>(message: S, is_markdown: bool) -> Vec<lsp_custom::TestMessage> {
  let message = lsp::MarkupContent {
    kind: if is_markdown { lsp::MarkupKind::Markdown } else { lsp::MarkupKind::PlainText },
...
impl LspTestFilter {
  fn as_ids(&self, test_module: &TestModule) -> Vec<String> {
    let ids: Vec<String> = if let Some(include) = &self.include {
      ...
      test_module.defs.iter().filter(|(_, d)| d.parent_id.is_none()).map(|(k, _)| k.clone()).collect()
    };
    ids.into_iter().filter(|id| !self.exclude.contains_key(id)).collect()
  }
}
...
impl TestRun {
  pub fn new(params: &lsp_custom::TestRunRequestParams, tests: Arc<Mutex<HashMap<ModuleSpecifier, TestModule>>>, workspace_settings: config::WorkspaceSettings) -> Self {
    let (queue, filters) = {
  ...
  pub async fn exec(&self, client: &Client, maybe_root_uri: Option<&ModuleSpecifier>) -> Result<(), AnyError> {
    ...
    test::check_specifiers(
      ...
      self.queue.iter().map(|s| (s.clone(), test::TestMode::Executable)).collect(),
    )
    .await?;

    let (concurrent_jobs, fail_fast) = if let DenoSubcommand::Test(test_flags) = factory.cli_options().sub_command() {
      (test_flags.concurrent_jobs.unwrap_or_else(|| NonZeroUsize::new(1).unwrap()).into(), test_flags.fail_fast)
    } else {
    ...
    let tests: Arc<RwLock<IndexMap<usize, test::TestDescription>>> = Arc::new(RwLock::new(IndexMap::new()));
    ...
    let join_handles = queue.into_iter().map(move |specifier| {
      ...
      let filter = test::TestFilter {
        substring: None,
        regex: None,
        include: lsp_filter.and_then(|f| f.include.as_ref().map(|i| i.values().map(|t| t.name.clone()).collect())),
        exclude: lsp_filter.map(|f| f.exclude.values().map(|t| t.name.clone()).collect()).unwrap_or_default(),
      };
      ...
      spawn_blocking(move || {
        ...
        let file_result = if token.is_cancelled() {
          ...
          create_and_run_current_thread(test::test_specifier(
            ...
            sender.clone(),
            fail_fast_tracker,
            test::TestSpecifierOptions { filter, shuffle: None, trace_ops: false },
          ))
        };
        if let Err(error) = file_result {
          if error.is::<JsError>() {
            sender.send(test::TestEvent::UncaughtError(origin, Box::new(error.downcast::<JsError>().unwrap())))?;
          } else {
            return Err(error);
          }
        }
    ...
    let join_stream = stream::iter(join_handles).buffer_unordered(concurrent_jobs).collect::<Vec<Result<Result<(), AnyError>, tokio::task::JoinError>>>();

    let mut reporter = Box::new(LspTestReporter::new(self, client.clone(), maybe_root_uri, self.tests.clone()));
  ...
  fn get_args(&self) -> Vec<&str> {
    ...
    args.extend(self.workspace_settings.testing.args.iter().map(|s| s.as_str()));
    args.push("--trace-ops");
    ...
    if self.kind == lsp_custom::TestRunKind::Debug && !args.contains(&"--inspect") && !args.contains(&"--inspect-brk") {
      args.push("--inspect");
    }
    args
  }
...
impl LspTestDescription {
  ...
  fn as_test_identifier(&self, tests: &IndexMap<usize, LspTestDescription>) -> lsp_custom::TestIdentifier {
    ...
    lsp_custom::TestIdentifier {
      ...
      step_id: if static_id == root_static_id { None } else { Some(static_id.to_string()) },
    }
  }
}
...
impl LspTestReporter {
  fn new(run: &TestRun, client: Client, maybe_root_uri: Option<&ModuleSpecifier>, files: Arc<Mutex<HashMap<ModuleSpecifier, TestModule>>>) -> Self {
    Self {
      client,
      id: run.id,
  ...
  fn progress(&self, message: lsp_custom::TestRunProgressMessage) {
    self.client.send_test_notification(TestingNotification::Progress(lsp_custom::TestRunProgressParams { id: self.id, message }));
  }
  ...
  fn report_register(&mut self, desc: &test::TestDescription) {
    ...
    let test_module = files.entry(specifier.clone()).or_insert_with(|| TestModule::new(specifier, "1".to_string()));
    let (static_id, is_new) = test_module.register_dynamic(desc);
    self.tests.insert(desc.id, LspTestDescription::TestDescription(desc.clone(), static_id.clone()));
    if is_new {
      self.client.send_test_notification(TestingNotification::Module(lsp_custom::TestModuleNotificationParams {
        text_document: lsp::TextDocumentIdentifier { uri: test_module.specifier.clone() },
  ...
  fn report_output(&mut self, output: &[u8]) {
    let test = self.current_test.as_ref().map(|id| self.tests.get(id).unwrap().as_test_identifier(&self.tests));
  ...
  fn report_result(&mut self, desc: &test::TestDescription, result: &test::TestResult, elapsed: u64) {
    ...
    match result {
      ...
      test::TestResult::Ignored => {
        let desc = self.tests.get(&desc.id).unwrap();
        self.progress(lsp_custom::TestRunProgressMessage::Skipped { test: desc.as_test_identifier(&self.tests) })
      }
  ...
  fn report_step_register(&mut self, desc: &test::TestStepDescription) {
    ...
    let specifier = ModuleSpecifier::parse(&desc.location.file_name).unwrap();
    let test_module = files.entry(specifier.clone()).or_insert_with(|| TestModule::new(specifier, "1".to_string()));
    let (static_id, is_new) = test_module.register_step_dynamic(desc, self.tests.get(&desc.parent_id).unwrap().static_id());
    self.tests.insert(desc.id, LspTestDescription::TestStepDescription(desc.clone(), static_id.clone()));
    if is_new {
      self.client.send_test_notification(TestingNotification::Module(lsp_custom::TestModuleNotificationParams {
        text_document: lsp::TextDocumentIdentifier { uri: test_module.specifier.clone() },
        kind: lsp_custom::TestModuleNotificationKind::Insert,
  ...
  fn report_step_result(&mut self, desc: &test::TestStepDescription, result: &test::TestStepResult, elapsed: u64) {
    ...
    match result {
      ...
      test::TestStepResult::Ignored => self.progress(lsp_custom::TestRunProgressMessage::Skipped { test: desc.as_test_identifier(&self.tests) }),
      test::TestStepResult::Failed(failure) => self.progress(lsp_custom::TestRunProgressMessage::Failed {
        test: desc.as_test_identifier(&self.tests),
        messages: as_test_messages(failure.to_string(), false),
        duration: Some(elapsed as u32),
      }),
