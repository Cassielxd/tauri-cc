...
impl TestServer {
  pub fn new(client: Client, performance: Arc<Performance>, maybe_root_uri: Option<ModuleSpecifier>) -> Self {
    let tests: Arc<Mutex<HashMap<ModuleSpecifier, TestModule>>> = Arc::new(Mutex::new(HashMap::new()));
          ...
          match update_rx.recv().await {
            ...
            Some(snapshot) => {
              ...
              for document in snapshot.documents.documents(DocumentsFilter::AllDiagnosable) {
                ...
                let valid = if let Some(test) = tests.get(specifier) { test.script_version == script_version } else { false };
                if !valid {
                  if let Some(Ok(parsed_source)) = document.maybe_parsed_source() {
                    let was_empty = tests.remove(specifier).map(|tm| tm.is_empty()).unwrap_or(true);
                    let mut collector = TestCollector::new(specifier.clone(), script_version, parsed_source.text_info().clone());
                    parsed_source.module().visit_with(&mut collector);
                    let test_module = collector.take();
                    if !test_module.is_empty() {
                      client.send_test_notification(test_module.as_replace_notification(mru.as_ref()));
                    } else if !was_empty {
          ...
          match run_rx.recv().await {
            ...
            Some(id) => {
              ...
              if let Some(run) = maybe_run {
                ...
                client.send_test_notification(TestingNotification::Progress(lsp_custom::TestRunProgressParams {
                  id,
  ...
  pub fn run_cancel_request(&self, params: lsp_custom::TestRunCancelParams) -> LspResult<Option<Value>> {
  ...
  pub fn run_request(&self, params: lsp_custom::TestRunRequestParams, workspace_settings: config::WorkspaceSettings) -> LspResult<Option<Value>> {
