...
impl LineIndex {
  ...
  pub fn offset_tsc(&self, position: lsp::Position) -> jsonrpc::Result<u32> {
    self.offset_utf16(position).map(|ts| ts.into()).map_err(|err| jsonrpc::Error::invalid_params(err.to_string()))
  }

  fn offset_utf16(&self, position: lsp::Position) -> Result<TextSize, AnyError> {
    ...
    match chunk {
      ...
      Some(Chunk::Delete(d)) => {
        ...
        match iter.peek() {
          Some(Chunk::Insert(i)) => {
            iter.next();
            text_edits.push(TextEdit { range, new_text: i.to_string() });
          }
          _ => text_edits.push(TextEdit { range, new_text: "".to_string() }),
        }
      }
      Some(Chunk::Insert(i)) => {
