...
/// panic is undesired.
pub static INVALID_SPECIFIER: Lazy<ModuleSpecifier> = Lazy::new(|| ModuleSpecifier::parse("deno://invalid").unwrap());
...
fn from_deno_url(url: &Url) -> Option<Url> {
  ...
  string.push_str("://");
  string.push_str(&percent_encoding::percent_decode(segments.next()?.as_bytes()).decode_utf8().ok()?);
  for segment in segments {
    string.push('/');
    string.push_str(&percent_encoding::percent_decode(segment.as_bytes()).decode_utf8().ok()?);
  }
  Url::parse(&string).ok()
}
...
impl LspUrlMapInner {
  fn put(&mut self, specifier: ModuleSpecifier, url: LspClientUrl) {
    self.url_to_specifier.insert(url.as_url().clone(), specifier.clone());
...
impl LspUrlMap {
  ...
  pub fn normalize_specifier(&self, specifier: &ModuleSpecifier) -> Result<LspClientUrl, AnyError> {
    ...
    if let Some(url) = inner.get_url(specifier).cloned() {
      ...
      let url = if specifier.scheme() == "file" {
        ...
        let specifier_str = if specifier.scheme() == "asset" {
          ...
          let (media_type, _) = map_content_type(specifier, Some(&format!("{mime}")));
          let extension = if media_type == MediaType::Unknown { "" } else { media_type.as_ts_extension() };
          format!("deno:/{}/data_url{}", hash_data_specifier(specifier), extension)
        } else {
