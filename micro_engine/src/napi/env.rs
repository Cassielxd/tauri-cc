...
#[no_mangle]
pub unsafe extern "C" fn napi_fatal_error(location: *const c_char, location_len: isize, message: *const c_char, message_len: isize) -> ! {
  let location = if location.is_null() {
    ...
    Some(if location_len < 0 {
      ...
      let slice = std::slice::from_raw_parts(location as *const u8, location_len as usize);
  ...
  let message = if message_len < 0 {
    ...
    let slice = std::slice::from_raw_parts(message as *const u8, message_len as usize);
    std::str::from_utf8(slice).unwrap()
  };
...
#[napi_sym::napi_sym]
fn napi_add_env_cleanup_hook(env: *mut Env, hook: extern "C" fn(*const c_void), data: *const c_void) -> napi_status {
  let Some(env) = env.as_mut() else {
    return napi_invalid_arg;
  };

  {
    ...
    if env_cleanup_hooks.iter().any(|pair| pair.0 == hook && pair.1 == data) {
...
#[napi_sym::napi_sym]
fn napi_remove_env_cleanup_hook(env: *mut Env, hook: extern "C" fn(*const c_void), data: *const c_void) -> napi_status {
  let Some(env) = env.as_mut() else {
    return napi_invalid_arg;
  };

  {
    ...
    let maybe_index = env_cleanup_hooks.iter().rposition(|&pair| pair.0 == hook && pair.1 == data);
...
#[napi_sym::napi_sym]
fn napi_open_callback_scope(_env: *mut Env, _resource_object: napi_value, _context: napi_value, _result: *mut napi_callback_scope) -> napi_status {
  // we open scope automatically when it's needed
  napi_ok
}

#[napi_sym::napi_sym]
fn napi_close_callback_scope(_env: *mut Env, _scope: napi_callback_scope) -> napi_status {
  // we close scope automatically when it's needed
  napi_ok
}

#[napi_sym::napi_sym]
fn node_api_get_module_file_name(env: *mut Env, result: *mut *const c_char) -> napi_status {
  let Some(env) = env.as_mut() else {
    return napi_invalid_arg;
  };
...
#[napi_sym::napi_sym]
fn napi_get_uv_event_loop(_env: *mut Env, uv_loop: *mut *mut ()) -> napi_status {
...
#[napi_sym::napi_sym]
fn napi_get_node_version(env: *mut Env, result: *mut *const napi_node_version) -> napi_status {
  crate::check_env!(env);
