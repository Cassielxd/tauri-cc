...
impl TsFn {
  ...
  pub fn release(mut self) -> napi_status {
    ...
    if self.thread_counter == 0 {
      if self.tsfn_sender.unbounded_send(ThreadSafeFunctionStatus::Dead).is_err() {
  ...
  pub fn ref_(&mut self) -> napi_status {
    self.ref_counter.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
    napi_ok
  }

  pub fn unref(&mut self) -> napi_status {
    let _ = self.ref_counter.fetch_update(std::sync::atomic::Ordering::SeqCst, std::sync::atomic::Ordering::SeqCst, |x| if x == 0 { None } else { Some(x - 1) });
      ...
      let call = Box::new(move || {
        ...
        match js_func {
          Some(func) => {
            ...
            unsafe { call_js_cb(env as *mut c_void, func.into(), context, data) };
          }
          None => {
            unsafe { call_js_cb(env as *mut c_void, std::mem::zeroed(), context, data) };
          }
        }
...
fn napi_create_threadsafe_function(
  ...
  env_ref.add_threadsafe_function_ref_counter(tsfn.id, tsfn.ref_counter.clone());

  if env_ref.threadsafe_function_sender.unbounded_send(ThreadSafeFunctionStatus::Alive).is_err() {
    return napi_generic_failure;
  }
...
#[napi_sym::napi_sym]
fn napi_acquire_threadsafe_function(tsfn: napi_threadsafe_function, _mode: napi_threadsafe_function_release_mode) -> napi_status {
  let tsfn: &mut TsFn = &mut *(tsfn as *mut TsFn);
  tsfn.acquire()
}

#[napi_sym::napi_sym]
fn napi_unref_threadsafe_function(_env: &mut Env, tsfn: napi_threadsafe_function) -> napi_status {
  let tsfn: &mut TsFn = &mut *(tsfn as *mut TsFn);
  tsfn.unref()
}
...
#[napi_sym::napi_sym]
pub fn napi_get_threadsafe_function_context(func: napi_threadsafe_function, result: *mut *const c_void) -> napi_status {
...
#[napi_sym::napi_sym]
fn napi_ref_threadsafe_function(_env: &mut Env, func: napi_threadsafe_function) -> napi_status {
  let tsfn: &mut TsFn = &mut *(func as *mut TsFn);
  tsfn.ref_()
}

#[napi_sym::napi_sym]
fn napi_release_threadsafe_function(tsfn: napi_threadsafe_function, _mode: napi_threadsafe_function_release_mode) -> napi_status {
