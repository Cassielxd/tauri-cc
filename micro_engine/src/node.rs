...
impl CliCjsCodeAnalyzer {
  ...
  fn inner_cjs_analysis(&self, specifier: &ModuleSpecifier, source: &str) -> Result<CjsAnalysis, AnyError> {
    ...
    if media_type == MediaType::Json {
      return Ok(CjsAnalysis { exports: vec![], reexports: vec![] });
    }
    ...
    self.cache.set_cjs_analysis(specifier.as_str(), &source_hash, &analysis);
...
impl CjsCodeAnalyzer for CliCjsCodeAnalyzer {
  fn analyze_cjs(&self, specifier: &ModuleSpecifier, source: Option<&str>) -> Result<ExtNodeCjsAnalysis, AnyError> {
    let source = match source {
      Some(source) => Cow::Borrowed(source),
      None => Cow::Owned(self.fs.read_text_file_sync(&specifier.to_file_path().unwrap())?),
    };
