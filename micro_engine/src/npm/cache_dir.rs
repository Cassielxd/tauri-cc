...
impl NpmCacheDir {
  pub fn new(root_dir: PathBuf) -> Self {
    fn try_get_canonicalized_root_dir(root_dir: &Path) -> Result<PathBuf, AnyError> {
      if !root_dir.exists() {
        std::fs::create_dir_all(root_dir).with_context(|| format!("Error creating {}", root_dir.display()))?;
      }
  ...
  pub fn package_folder_for_id(&self, folder_id: &NpmPackageCacheFolderId, registry_url: &Url) -> PathBuf {
    if folder_id.copy_index == 0 {
      self.package_folder_for_name_and_version(&folder_id.nv, registry_url)
    } else {
      self.package_name_folder(&folder_id.nv.name, registry_url).join(format!("{}_{}", folder_id.nv.version, folder_id.copy_index))
    }
  }

  pub fn package_folder_for_name_and_version(&self, package: &PackageNv, registry_url: &Url) -> PathBuf {
    self.package_name_folder(&package.name, registry_url).join(package.version.to_string())
  }
  ...
  pub fn registry_folder(&self, registry_url: &Url) -> PathBuf {
    self.root_dir.join(root_url_to_safe_local_dirname(registry_url))
  }
  ...
  pub fn resolve_package_folder_id_from_specifier(&self, specifier: &ModuleSpecifier, registry_url: &Url) -> Option<NpmPackageCacheFolderId> {
    let registry_root_dir = self
      .root_dir_url
      .join(&format!("{}/", root_url_to_safe_local_dirname(registry_url).to_string_lossy().replace('\\', "/")))
    ...
    if let Some(end_url) = relative_url.strip_prefix('_') {
      let mut parts = end_url.split('/').map(ToOwned::to_owned).collect::<Vec<_>>();
    ...
    let mut parts = relative_url.split('/').enumerate().take(if is_scoped_package { 3 } else { 2 }).map(|(_, part)| part).collect::<Vec<_>>();
    if parts.len() < 2 {
      return None;
    }
...
pub fn mixed_case_package_name_encode(name: &str) -> String {
  ...
  base32::encode(base32::Alphabet::RFC4648 { padding: false }, name.as_bytes()).to_lowercase()
}

pub fn mixed_case_package_name_decode(name: &str) -> Option<String> {
  base32::decode(base32::Alphabet::RFC4648 { padding: false }, name).and_then(|b| String::from_utf8(b).ok())
}
