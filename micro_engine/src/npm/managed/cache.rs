...
impl NpmCache {
  pub fn new(cache_dir: NpmCacheDir, cache_setting: CacheSetting, fs: Arc<dyn deno_fs::FileSystem>, http_client: Arc<HttpClient>, progress_bar: ProgressBar) -> Self {
    Self {
      cache_dir,
      cache_setting,
      fs,
  ...
  fn should_use_global_cache_for_package(&self, package: &PackageNv) -> bool {
    self.cache_setting.should_use_for_npm_package(&package.name) || !self.previously_reloaded_packages.lock().insert(package.clone())
  }

  pub async fn ensure_package(&self, package: &PackageNv, dist: &NpmPackageVersionDistInfo, registry_url: &Url) -> Result<(), AnyError> {
    self.ensure_package_inner(package, dist, registry_url).await.with_context(|| format!("Failed caching npm package '{package}'."))
  }

  async fn ensure_package_inner(&self, package: &PackageNv, dist: &NpmPackageVersionDistInfo, registry_url: &Url) -> Result<(), AnyError> {
    let package_folder = self.cache_dir.package_folder_for_name_and_version(package, registry_url);
      ...
      && !self.fs.exists_sync(&package_folder.join(NPM_PACKAGE_SYNC_LOCK_FILENAME))
    {
      ...
      return Err(custom_error("NotCached", format!("An npm specifier not found in cache: \"{}\", --cached-only is specified.", &package.name)));
    }
  ...
  pub fn ensure_copy_package(&self, folder_id: &NpmPackageCacheFolderId, registry_url: &Url) -> Result<(), AnyError> {
    assert_ne!(folder_id.copy_index, 0);
    let package_folder = self.cache_dir.package_folder_for_id(folder_id, registry_url);
    ...
    let original_package_folder = self.cache_dir.package_folder_for_name_and_version(&folder_id.nv, registry_url);
    with_folder_sync_lock(&folder_id.nv, &package_folder, || hard_link_dir_recursive(&original_package_folder, &package_folder))?;
    Ok(())
  }

  pub fn package_folder_for_id(&self, id: &NpmPackageCacheFolderId, registry_url: &Url) -> PathBuf {
    self.cache_dir.package_folder_for_id(id, registry_url)
  }

  pub fn package_folder_for_name_and_version(&self, package: &PackageNv, registry_url: &Url) -> PathBuf {
    self.cache_dir.package_folder_for_name_and_version(package, registry_url)
  }
  ...
  pub fn resolve_package_folder_id_from_specifier(&self, specifier: &ModuleSpecifier, registry_url: &Url) -> Option<NpmPackageCacheFolderId> {
    self.cache_dir.resolve_package_folder_id_from_specifier(specifier, registry_url)
  }
}

const NPM_PACKAGE_SYNC_LOCK_FILENAME: &str = ".deno_sync_lock";

pub fn with_folder_sync_lock(package: &PackageNv, output_folder: &Path, action: impl FnOnce() -> Result<(), AnyError>) -> Result<(), AnyError> {
  fn inner(output_folder: &Path, action: impl FnOnce() -> Result<(), AnyError>) -> Result<(), AnyError> {
    fs::create_dir_all(output_folder).with_context(|| format!("Error creating '{}'.", output_folder.display()))?;
    ...
    match fs::OpenOptions::new().write(true).create(true).open(&sync_lock_path) {
      Ok(_) => {
        action()?;
