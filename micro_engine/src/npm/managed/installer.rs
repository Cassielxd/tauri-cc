...
impl PackageJsonDepsInstallerInner {
  pub fn reqs_with_info_futures<'a>(&self, reqs: &'a [&'a PackageReq]) -> FuturesOrdered<impl Future<Output = Result<(&'a PackageReq, Arc<deno_npm::registry::NpmPackageInfo>), NpmRegistryPackageInfoLoadError>>> {
...
impl PackageJsonDepsInstaller {
  pub fn new(deps_provider: Arc<PackageJsonDepsProvider>, npm_registry_api: Arc<CliNpmRegistryApi>, npm_resolution: Arc<NpmResolution>) -> Self {
  ...
  pub async fn ensure_top_level_install(&self) -> Result<(), AnyError> {
    ...
    if package_reqs.iter().all(|req| inner.npm_resolution.resolve_pkg_id_from_pkg_req(req).is_ok()) {
    ...
    while let Some(result) = reqs_with_info_futures.next().await {
      let (req, info) = result?;
      let result = inner.npm_resolution.resolve_pkg_req_as_pending_with_info(req, &info);
