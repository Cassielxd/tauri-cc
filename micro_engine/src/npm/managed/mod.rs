...
pub async fn create_managed_npm_resolver_for_lsp(options: CliNpmResolverManagedCreateOptions) -> Arc<dyn CliNpmResolver> {
...
pub async fn create_managed_npm_resolver(options: CliNpmResolverManagedCreateOptions) -> Result<Arc<dyn CliNpmResolver>, AnyError> {
  let npm_cache = create_cache(&options);
  let npm_api = create_api(&options, npm_cache.clone());
...
fn create_inner(
  ...
  let resolution = Arc::new(NpmResolution::from_serialized(npm_api.clone(), snapshot, maybe_lockfile.clone()));
  let npm_fs_resolver = create_npm_fs_resolver(fs.clone(), npm_cache.clone(), &text_only_progress_bar, npm_registry_url, resolution.clone(), node_modules_dir_path, npm_system_info.clone());
  let package_json_deps_installer = match package_json_installer {
    CliNpmResolverManagedPackageJsonInstallerOption::ConditionalInstall(provider) => Arc::new(PackageJsonDepsInstaller::new(provider, npm_api.clone(), resolution.clone())),
    CliNpmResolverManagedPackageJsonInstallerOption::NoInstall => Arc::new(PackageJsonDepsInstaller::no_op()),
  };
...
fn create_api(options: &CliNpmResolverManagedCreateOptions, npm_cache: Arc<NpmCache>) -> Arc<CliNpmRegistryApi> {
  Arc::new(CliNpmRegistryApi::new(options.npm_registry_url.clone(), npm_cache.clone(), options.http_client.clone(), options.text_only_progress_bar.clone()))
}

async fn resolve_snapshot(api: &CliNpmRegistryApi, snapshot: CliNpmResolverManagedSnapshotOption) -> Result<Option<ValidSerializedNpmResolutionSnapshot>, AnyError> {
  match snapshot {
    CliNpmResolverManagedSnapshotOption::ResolveFromLockfile(lockfile) => {
      if !lockfile.lock().overwrite {
        let snapshot = snapshot_from_lockfile(lockfile.clone(), api).await.with_context(|| format!("failed reading lockfile '{}'", lockfile.lock().filename.display()))?;
...
async fn snapshot_from_lockfile(lockfile: Arc<Mutex<Lockfile>>, api: &dyn NpmRegistryApi) -> Result<ValidSerializedNpmResolutionSnapshot, AnyError> {
...
impl std::fmt::Debug for ManagedCliNpmResolver {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    f.debug_struct("ManagedNpmResolver").field("<omitted>", &"<omitted>").finish()
  }
}
...
impl ManagedCliNpmResolver {
  ...
  pub fn resolve_pkg_folder_from_pkg_id(&self, pkg_id: &NpmPackageId) -> Result<PathBuf, AnyError> {
    let path = self.fs_resolver.package_folder(pkg_id)?;
    let path = canonicalize_path_maybe_not_exists_with_fs(&path, |path| self.fs.realpath_sync(path).map_err(|err| err.into_io_error()))?;
    log::debug!("Resolved package folder of {} to {}", pkg_id.as_serialized(), path.display());
    Ok(path)
  }

  /// Resolves the package nv from the provided specifier.
  pub fn resolve_pkg_id_from_specifier(&self, specifier: &ModuleSpecifier) -> Result<Option<NpmPackageId>, AnyError> {
    let Some(cache_folder_id) = self.fs_resolver.resolve_package_cache_folder_id_from_specifier(specifier)? else {
      return Ok(None);
    };
    Ok(Some(self.resolution.resolve_pkg_id_from_pkg_cache_folder_id(&cache_folder_id)?))
  }
  ...
  pub fn is_pkg_req_folder_cached(&self, req: &PackageReq) -> bool {
    self.resolve_pkg_id_from_pkg_req(req).ok().and_then(|id| self.fs_resolver.package_folder(&id).ok()).map(|folder| folder.exists()).unwrap_or(false)
  }
  ...
  pub fn serialized_valid_snapshot_for_system(&self, system_info: &NpmSystemInfo) -> ValidSerializedNpmResolutionSnapshot {
    self.resolution.serialized_valid_snapshot_for_system(system_info)
  }
  ...
  /// a package.json
  pub fn resolve_npm_for_deno_graph(&self, pkg_req: &PackageReq) -> NpmPackageReqResolution {
  ...
  fn resolve_pkg_id_from_pkg_req(&self, req: &PackageReq) -> Result<NpmPackageId, PackageReqNotFoundError> {
    self.resolution.resolve_pkg_id_from_pkg_req(req)
  }

  pub async fn ensure_top_level_package_json_install(&self) -> Result<(), AnyError> {
    self.package_json_deps_installer.ensure_top_level_install().await
  }

  pub async fn cache_package_info(&self, package_name: &str) -> Result<(), AnyError> {
    // this will internally cache the package information
    self.api.package_info(package_name).await.map(|_| ()).map_err(|err| err.into())
  }
  ...
  pub fn registry_folder_in_global_cache(&self, registry_url: &ModuleSpecifier) -> PathBuf {
...
impl NpmResolver for ManagedCliNpmResolver {
  ...
  fn get_npm_process_state(&self) -> String {
    serde_json::to_string(&NpmProcessState {
      kind: NpmProcessStateKind::Snapshot(self.resolution.serialized_valid_snapshot().into_serialized()),
      local_node_modules_path: self.fs_resolver.node_modules_path().map(|p| p.to_string_lossy().to_string()),
    })
    .unwrap()
  }
  ...
  fn resolve_package_folder_from_package(&self, name: &str, referrer: &ModuleSpecifier, mode: NodeResolutionMode) -> Result<PathBuf, AnyError> {
    let path = self.fs_resolver.resolve_package_folder_from_package(name, referrer, mode)?;
...
impl CliNpmResolver for ManagedCliNpmResolver {
  ...
  fn clone_snapshotted(&self) -> Arc<dyn CliNpmResolver> {
    ...
    let npm_resolution = Arc::new(NpmResolution::new(self.api.clone(), self.resolution.snapshot(), self.maybe_lockfile.clone()));
  ...
  fn resolve_pkg_folder_from_deno_module_req(&self, req: &PackageReq, _referrer: &ModuleSpecifier) -> Result<PathBuf, AnyError> {
  ...
  fn check_state_hash(&self) -> Option<u64> {
    ...
    let mut package_reqs = self.resolution.package_reqs().into_iter().collect::<Vec<_>>();
