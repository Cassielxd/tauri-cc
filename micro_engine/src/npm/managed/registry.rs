...
impl CliNpmRegistryApi {
  pub fn new(base_url: Url, cache: Arc<NpmCache>, http_client: Arc<HttpClient>, progress_bar: ProgressBar) -> Self {
...
impl NpmRegistryApi for CliNpmRegistryApi {
  async fn package_info(&self, name: &str) -> Result<Arc<NpmPackageInfo>, NpmRegistryPackageInfoLoadError> {
    match self.inner().maybe_package_info(name).await {
      Ok(Some(info)) => Ok(info),
      Ok(None) => Err(NpmRegistryPackageInfoLoadError::PackageNotExists { package_name: name.to_string() }),
  ...
  fn mark_force_reload(&self) -> bool {
    ...
    if matches!(self.inner().cache.cache_setting(), CacheSetting::Only | CacheSetting::ReloadAll) {
      return false;
    }
...
impl CliNpmRegistryApiInner {
  pub async fn maybe_package_info(self: &Arc<Self>, name: &str) -> Result<Option<Arc<NpmPackageInfo>>, AnyError> {
    let (created, future) = {
      ...
      match mem_cache.get(name) {
        ...
        None => {
          ...
          let future = {
            ...
            async move { api.load_package_info_from_registry(&name).await.map(|info| info.map(Arc::new)).map_err(Arc::new) }.boxed().shared()
          };
    ...
    if created {
      match future.await {
        Ok(maybe_info) => {
          ...
          self.mem_cache.lock().insert(name.to_string(), CacheItem::Resolved(maybe_info.clone()));
          Ok(maybe_info)
        }
        Err(err) => {
  ...
  fn load_file_cached_package_info_result(&self, name: &str) -> Result<Option<NpmPackageInfo>, AnyError> {
    ...
    match serde_json::from_str(&file_text) {
      ...
      Err(err) => {
        ...
        // rather than panic.
        log::debug!("error deserializing registry.json for '{}'. Reloading. {:?}", name, err);
        Ok(None)
      }
    }
  }
  ...
  fn save_package_info_to_file_cache_result(&self, name: &str, package_info: &NpmPackageInfo) -> Result<(), AnyError> {
    let file_cache_path = self.get_package_file_cache_path(name);
  ...
  async fn load_package_info_from_registry(&self, name: &str) -> Result<Option<NpmPackageInfo>, AnyError> {
    self
      .load_package_info_from_registry_inner(name)
      .await
      .with_context(|| format!("Error getting response at {} for package \"{}\"", self.get_package_url(name), name))
  }

  async fn load_package_info_from_registry_inner(&self, name: &str) -> Result<Option<NpmPackageInfo>, AnyError> {
    if *self.cache.cache_setting() == CacheSetting::Only {
      return Err(custom_error("NotCached", format!("An npm specifier not found in cache: \"{name}\", --cached-only is specified.")));
    }
