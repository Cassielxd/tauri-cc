...
impl RegistryReadPermissionChecker {
  pub fn new(fs: Arc<dyn FileSystem>, registry_path: PathBuf) -> Self {
    Self { fs, registry_path, cache: Default::default() }
  }

  pub fn ensure_registry_read_permission(&self, permissions: &dyn NodePermissions, path: &Path) -> Result<(), AnyError> {
    // allow reading if it's in the node_modules
    let is_path_in_node_modules = path.starts_with(&self.registry_path) && path.components().all(|c| !matches!(c, std::path::Component::ParentDir));
...
pub async fn cache_packages(packages: Vec<NpmResolutionPackage>, cache: &Arc<NpmCache>, registry_url: &Url) -> Result<(), AnyError> {
  let mut handles = Vec::with_capacity(packages.len());
  for package in packages {
    let cache = cache.clone();
    let registry_url = registry_url.clone();
    let handle = spawn(async move { cache.ensure_package(&package.id.nv, &package.dist, &registry_url).await });
    handles.push(handle);
  }
