...
impl LocalNpmPackageResolver {
  pub fn new(fs: Arc<dyn deno_fs::FileSystem>, cache: Arc<NpmCache>, progress_bar: ProgressBar, registry_url: Url, node_modules_folder: PathBuf, resolution: Arc<NpmResolution>, system_info: NpmSystemInfo) -> Self {
    Self {
      fs: fs.clone(),
      cache,
      progress_bar,
  ...
  fn resolve_folder_for_specifier(&self, specifier: &ModuleSpecifier) -> Result<Option<PathBuf>, AnyError> {
    ...
    canonicalize_path_maybe_not_exists_with_fs(&path, |path| self.fs.realpath_sync(path).map_err(|err| err.into_io_error()))
      .map(Some)
...
impl NpmPackageFsResolver for LocalNpmPackageResolver {
  ...
  fn resolve_package_folder_from_specifier(&self, specifier: &ModuleSpecifier) -> Result<Option<PathBuf>, AnyError> {
  ...
  fn resolve_package_cache_folder_id_from_specifier(&self, specifier: &ModuleSpecifier) -> Result<Option<NpmPackageCacheFolderId>, AnyError> {
  ...
  async fn cache_packages(&self) -> Result<(), AnyError> {
    sync_resolution_with_fs(&self.resolution.snapshot(), &self.cache, &self.progress_bar, &self.registry_url, &self.root_node_modules_path, &self.system_info).await
  }
...
async fn sync_resolution_with_fs(snapshot: &NpmResolutionSnapshot, cache: &Arc<NpmCache>, progress_bar: &ProgressBar, registry_url: &Url, root_node_modules_dir_path: &Path, system_info: &NpmSystemInfo) -> Result<(), AnyError> {
  ...
  for package in &package_partitions.packages {
    ...
    let package_folder_name = get_package_folder_id_folder_name(&package.get_package_cache_folder_id());
  ...
  for package in &package_partitions.copy_packages {
    ...
    if !initialized_file.exists() {
      ...
      let source_path = join_package_name(&deno_local_registry_dir.join(get_package_folder_id_folder_name(&package_cache_folder_id.with_no_count())).join("node_modules"), &package.id.nv.name);
  ...
  for package in package_partitions.iter_all() {
    let package_folder_name = get_package_folder_id_folder_name(&package.get_package_cache_folder_id());
    let sub_node_modules = deno_local_registry_dir.join(&package_folder_name).join("node_modules");
  ...
  for id in ids {
    ...
    let target_folder_name = get_package_folder_id_folder_name(&package.get_package_cache_folder_id());
    if setup_cache.insert_root_symlink(&id.nv.name, &target_folder_name) {
      let local_registry_package_path = join_package_name(&deno_local_registry_dir.join(target_folder_name).join("node_modules"), &id.nv.name);
  ...
  for package in newest_packages_by_name.values() {
    ...
    if setup_cache.insert_deno_symlink(&package.id.nv.name, &target_folder_name) {
      let local_registry_package_path = join_package_name(&deno_local_registry_dir.join(target_folder_name).join("node_modules"), &package.id.nv.name);

      symlink_package_dir(&local_registry_package_path, &join_package_name(&deno_node_modules_dir, &package.id.nv.name))?;
    }
  }
...
impl<'a> SetupCacheDep<'a> {
  pub fn insert(&mut self, name: &str, target_folder_name: &str) -> bool {
    self.current.insert(name.to_string(), target_folder_name.to_string());
...
impl SetupCache {
  pub fn load(file_path: PathBuf) -> Self {
    let previous = std::fs::read(&file_path).ok().and_then(|data| bincode::deserialize(&data).ok());
    Self {
      file_path,
      previous,
  ...
  pub fn save(&self) -> bool {
    ...
    bincode::serialize(&self.current).ok().and_then(|data| atomic_write_file(&self.file_path, data, CACHE_PERM).ok());
    true
  }
  ...
  pub fn insert_root_symlink(&mut self, name: &str, target_folder_name: &str) -> bool {
    self.current.root_symlinks.insert(name.to_string(), target_folder_name.to_string());
    if let Some(previous_target) = self.previous.as_ref().and_then(|p| p.root_symlinks.get(name)) {
      previous_target != target_folder_name
    } else {
      true
    }
  ...
  pub fn insert_deno_symlink(&mut self, name: &str, target_folder_name: &str) -> bool {
    self.current.deno_symlinks.insert(name.to_string(), target_folder_name.to_string());
    if let Some(previous_target) = self.previous.as_ref().and_then(|p| p.deno_symlinks.get(name)) {
      previous_target != target_folder_name
    } else {
      true
    }
  ...
  pub fn with_dep(&mut self, parent_name: &str) -> SetupCacheDep<'_> {
    SetupCacheDep {
      previous: self.previous.as_ref().and_then(|p| p.dep_symlinks.get(parent_name)),
      current: self.current.dep_symlinks.entry(parent_name.to_string()).or_default(),
    }
  }
}

fn get_package_folder_id_folder_name(folder_id: &NpmPackageCacheFolderId) -> String {
  let copy_str = if folder_id.copy_index == 0 { "".to_string() } else { format!("_{}", folder_id.copy_index) };
  let nv = &folder_id.nv;
...
fn get_package_folder_id_from_folder_name(folder_name: &str) -> Option<NpmPackageCacheFolderId> {
  ...
  Some(NpmPackageCacheFolderId { nv: PackageNv { name, version }, copy_index })
}
