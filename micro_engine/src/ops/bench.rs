...
#[op2]
#[serde]
fn op_register_bench<'a>(scope: &mut v8::HandleScope<'a>, state: &mut OpState, #[serde] info: BenchInfo<'a>) -> Result<BenchRegisterResult, AnyError> {
  ...
  state.borrow_mut::<BenchContainer>().0.push((description.clone(), function));
...
fn op_dispatch_bench_event(state: &mut OpState, #[serde] event: BenchEvent) {
  assert!(matches!(event, BenchEvent::Output(_)), "Only output events are expected from JS.");
  let sender = state.borrow::<UnboundedSender<BenchEvent>>().clone();
