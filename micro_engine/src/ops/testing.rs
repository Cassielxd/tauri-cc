...
#[op2]
#[serde]
pub fn op_pledge_test_permissions(state: &mut OpState, #[serde] args: ChildPermissionsArg) -> Result<Uuid, AnyError> {
...
#[op2]
pub fn op_restore_test_permissions(state: &mut OpState, #[serde] token: Uuid) -> Result<(), AnyError> {
...
fn op_register_test(
  ...
  if ret_buf.len() != 4 {
    return Err(type_error(format!("Invalid ret_buf length: {}", ret_buf.len())));
  }
...
fn op_test_event_step_result_ok(state: &mut OpState, #[smi] id: usize, #[smi] duration: u64) {
  let sender = state.borrow_mut::<TestEventSender>();
  sender.send(TestEvent::StepResult(id, TestStepResult::Ok, duration)).ok();
}

#[op2(fast)]
fn op_test_event_step_result_ignored(state: &mut OpState, #[smi] id: usize, #[smi] duration: u64) {
  let sender = state.borrow_mut::<TestEventSender>();
  sender.send(TestEvent::StepResult(id, TestStepResult::Ignored, duration)).ok();
}

#[op2]
fn op_test_event_step_result_failed(state: &mut OpState, #[smi] id: usize, #[serde] failure: TestFailure, #[smi] duration: u64) {
  let sender = state.borrow_mut::<TestEventSender>();
  sender.send(TestEvent::StepResult(id, TestStepResult::Failed(failure), duration)).ok();
}
...
fn try_collect_metrics(metrics: &OpMetricsSummaryTracker, force: bool, op_id_host_recv_msg: usize, op_id_host_recv_ctrl: usize) -> Result<std::cell::Ref<Vec<OpMetricsSummary>>, bool> {
  let metrics = metrics.per_op();
  let host_recv_msg = metrics.get(op_id_host_recv_msg).map(OpMetricsSummary::has_outstanding_ops).unwrap_or(false);
  let host_recv_ctrl = metrics.get(op_id_host_recv_ctrl).map(OpMetricsSummary::has_outstanding_ops).unwrap_or(false);
...
fn op_test_op_sanitizer_collect(state: &mut OpState, #[smi] id: u32, force: bool, #[smi] op_id_host_recv_msg: usize, #[smi] op_id_host_recv_ctrl: usize) -> Result<u8, AnyError> {
  let metrics = state.borrow::<Rc<OpMetricsSummaryTracker>>();
  let metrics = match try_collect_metrics(metrics, force, op_id_host_recv_msg, op_id_host_recv_ctrl) {
    Ok(metrics) => metrics,
    Err(false) => {
      return Ok(1);
    }
  ...
  match op_sanitizers.0.entry(id) {
    ...
    Entry::Occupied(_) => {
      return Err(generic_error(format!("Test metrics already being collected for test id {id}",)));
    }
  }
  Ok(0)
}
...
fn op_test_op_sanitizer_finish(state: &mut OpState, #[smi] id: u32, force: bool, #[smi] op_id_host_recv_msg: usize, #[smi] op_id_host_recv_ctrl: usize) -> Result<u8, AnyError> {
  ...
  let report = {
    let after_metrics = match try_collect_metrics(metrics, force, op_id_host_recv_msg, op_id_host_recv_ctrl) {
      Ok(metrics) => metrics,
      Err(false) => {
        return Ok(1);
      }
    ...
    let before_metrics = match op_sanitizers.0.get(&id) {
      ...
      _ => {
        return Err(generic_error(format!("Metrics not collected before for test id {id}",)));
      }
    };
    let mut report = vec![];

    for (id, (before, after)) in before_metrics.iter().zip(after_metrics.iter()).enumerate() {
      let async_pending_before = before.ops_dispatched_async - before.ops_completed_async;
      let async_pending_after = after.ops_dispatched_async - after.ops_completed_async;
  ...
  if report.is_empty() {
    op_sanitizers.0.remove(&id).expect("TestOpSanitizerState::Collecting");
    Ok(0)
  } else {
    op_sanitizers.0.insert(id, TestOpSanitizerState::Finished { report }).expect("TestOpSanitizerState::Collecting");
    Ok(3)
  }
}
...
#[op2]
#[serde]
fn op_test_op_sanitizer_report(state: &mut OpState, #[smi] id: u32) -> Result<Vec<TestOpSanitizerReport>, AnyError> {
  let op_sanitizers = state.borrow_mut::<TestOpSanitizers>();
  match op_sanitizers.0.remove(&id) {
    Some(TestOpSanitizerState::Finished { report }) => Ok(report),
    _ => Err(generic_error(format!("Metrics not finished collecting for test id {id}",))),
  }
}
