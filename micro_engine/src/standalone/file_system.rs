...
impl FileSystem for DenoCompileFileSystem {
  ...
  fn symlink_sync(&self, oldpath: &Path, newpath: &Path, file_type: Option<FsFileType>) -> FsResult<()> {
    self.error_if_in_vfs(oldpath)?;
    self.error_if_in_vfs(newpath)?;
    RealFs.symlink_sync(oldpath, newpath, file_type)
  }
  async fn symlink_async(&self, oldpath: PathBuf, newpath: PathBuf, file_type: Option<FsFileType>) -> FsResult<()> {
    self.error_if_in_vfs(&oldpath)?;
    self.error_if_in_vfs(&newpath)?;
  ...
  async fn utime_async(&self, path: PathBuf, atime_secs: i64, atime_nanos: u32, mtime_secs: i64, mtime_nanos: u32) -> FsResult<()> {
    self.error_if_in_vfs(&path)?;
    RealFs.utime_async(path, atime_secs, atime_nanos, mtime_secs, mtime_nanos).await
  }
}
