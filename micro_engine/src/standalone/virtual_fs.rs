...
impl VfsBuilder {
  pub fn new(root_path: PathBuf) -> Result<Self, AnyError> {
    ...
    Ok(Self {
      root_dir: VirtualDirectory {
        name: root_path.file_stem().unwrap().to_string_lossy().into_owned(),
        entries: Vec::new(),
      },
      root_path,
      files: Vec::new(),
  ...
  fn add_dir_recursive_internal(&mut self, path: &Path) -> Result<(), AnyError> {
    self.add_dir(path)?;
    let read_dir = std::fs::read_dir(path).with_context(|| format!("Reading {}", path.display()))?;
    ...
    for entry in read_dir {
      ...
      if file_type.is_dir() {
        ...
        let target = util::fs::canonicalize_path(&path).with_context(|| format!("Reading symlink {}", path.display()))?;
        if let Err(StripRootError { .. }) = self.add_symlink(&path, &target) {
          if target.is_file() {
            // this may change behavior, so warn the user about it
            log::warn!("Symlink target is outside '{}'. Inlining symlink at '{}' to '{}' as file.", self.root_path.display(), path.display(), target.display(),);
            // inline the symlink and make the target file
            let file_bytes = std::fs::read(&target).with_context(|| format!("Reading {}", path.display()))?;
            self.add_file(&path, file_bytes)?;
          } else {
            log::warn!("Symlink target is outside '{}'. Excluding symlink at '{}' with target '{}'.", self.root_path.display(), path.display(), target.display(),);
          }
        }
      }
    }
  ...
  fn add_dir(&mut self, path: &Path) -> Result<&mut VirtualDirectory, StripRootError> {
    ...
    for component in path.components() {
      ...
      let index = match current_dir.entries.binary_search_by(|e| e.name().cmp(&name)) {
        Ok(index) => index,
        Err(insert_index) => {
          current_dir.entries.insert(insert_index, VfsEntry::Dir(VirtualDirectory { name: name.to_string(), entries: Vec::new() }));
          insert_index
        }
      };
  ...
  fn add_symlink(&mut self, path: &Path, target: &Path) -> Result<(), StripRootError> {
    log::debug!("Adding symlink '{}' to '{}'", path.display(), target.display());
      ...
      Err(insert_index) => {
        dir.entries.insert(
          ...
          VfsEntry::Symlink(VirtualSymlink {
            name: name.to_string(),
            dest_parts: dest.components().map(|c| c.as_os_str().to_string_lossy().to_string()).collect::<Vec<_>>(),
          }),
        );
      }
    }
...
impl VfsRoot {
  ...
  fn find_entry_inner<'a>(&'a self, path: &Path, seen: &mut HashSet<PathBuf>) -> std::io::Result<(PathBuf, VfsEntryRef<'a>)> {
      ...
      match entry {
        VfsEntryRef::Symlink(symlink) => {
          if !seen.insert(path.to_path_buf()) {
            return Err(std::io::Error::new(std::io::ErrorKind::Other, "circular symlinks"));
          }
  ...
  fn find_entry_no_follow_inner<'a>(&'a self, path: &Path, seen: &mut HashSet<PathBuf>) -> std::io::Result<(PathBuf, VfsEntryRef<'a>)> {
    ...
    for component in relative_path.components() {
      ...
      match current_dir.entries.binary_search_by(|e| e.name().cmp(&component)) {
        Ok(index) => {
...
impl FileBackedVfsFile {
    ...
    match pos {
      ...
      SeekFrom::End(offset) => {
        if offset < 0 && -offset as u64 > self.file.len {
          ...
          *current_pos = if offset >= 0 { self.file.len - (offset as u64) } else { self.file.len + (-offset as u64) };
          Ok(*current_pos)
        }
      }
  ...
  fn read_to_buf(&self, buf: &mut [u8]) -> FsResult<usize> {
    ...
    self.vfs.read_file(&self.file, pos, buf).map_err(|err| err.into())
  }
...
impl deno_io::fs::File for FileBackedVfsFile {
  ...
  fn utime_sync(self: Rc<Self>, _atime_secs: i64, _atime_nanos: u32, _mtime_secs: i64, _mtime_nanos: u32) -> FsResult<()> {
    Err(FsError::NotSupported)
  }
  async fn utime_async(self: Rc<Self>, _atime_secs: i64, _atime_nanos: u32, _mtime_secs: i64, _mtime_nanos: u32) -> FsResult<()> {
    Err(FsError::NotSupported)
  }
...
impl FileBackedVfs {
  pub fn new(file: File, fs_root: VfsRoot) -> Self {
    Self { file: Mutex::new(file), fs_root }
  }
  ...
  pub fn read_link(&self, path: &Path) -> std::io::Result<PathBuf> {
    ...
    match entry {
      ...
      VfsEntryRef::Dir(_) | VfsEntryRef::File(_) => Err(std::io::Error::new(std::io::ErrorKind::Other, "not a symlink")),
    }
  }
  ...
  pub fn read_file(&self, file: &VirtualFile, pos: u64, buf: &mut [u8]) -> std::io::Result<usize> {
    ...
    fs_file.seek(SeekFrom::Start(self.fs_root.start_file_offset + file.offset + pos))?;
    fs_file.read(buf)
  }
  ...
  pub fn dir_entry(&self, path: &Path) -> std::io::Result<&VirtualDirectory> {
    ...
    match entry {
      ...
      VfsEntryRef::File(_) => Err(std::io::Error::new(std::io::ErrorKind::Other, "path is a file")),
    }
  }
  ...
  pub fn file_entry(&self, path: &Path) -> std::io::Result<&VirtualFile> {
    let (_, entry) = self.fs_root.find_entry(path)?;
    match entry {
      VfsEntryRef::Dir(_) => Err(std::io::Error::new(std::io::ErrorKind::Other, "path is a directory")),
      VfsEntryRef::Symlink(_) => unreachable!(),
