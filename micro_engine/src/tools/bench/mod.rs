...
async fn check_specifiers(cli_options: &CliOptions, module_load_preparer: &ModuleLoadPreparer, specifiers: Vec<ModuleSpecifier>) -> Result<(), AnyError> {
  let lib = cli_options.ts_type_lib_window();
  module_load_preparer.prepare_module_load(specifiers, false, lib, PermissionsContainer::allow_all()).await?;
  Ok(())
}

/// Run a single specifier as an executable bench module.
async fn bench_specifier(worker_factory: Arc<CliMainWorkerFactory>, permissions: Permissions, specifier: ModuleSpecifier, sender: UnboundedSender<BenchEvent>, filter: TestFilter) -> Result<(), AnyError> {
  let mut worker = worker_factory
    .create_custom_worker(specifier.clone(), PermissionsContainer::new(permissions), vec![ops::bench::deno_bench::init_ops(sender.clone())], Default::default())
    .await?;
  ...
  let mut benchmarks = benchmarks.into_iter().filter(|(d, _)| filter.includes(&d.name) && !d.ignore).collect::<Vec<_>>();
  ...
  benchmarks.sort_by(|(d1, _), (d2, _)| groups.get_index_of(&d1.group).unwrap().partial_cmp(&groups.get_index_of(&d2.group).unwrap()).unwrap());
...
async fn bench_specifiers(worker_factory: Arc<CliMainWorkerFactory>, permissions: &Permissions, specifiers: Vec<ModuleSpecifier>, options: BenchSpecifierOptions) -> Result<(), AnyError> {
  ...
  let join_handles = specifiers.into_iter().map(move |specifier| {
    ...
    spawn_blocking(move || {
      let future = bench_specifier(worker_factory, permissions, specifier, sender, options.filter);
  ...
  let handler = {
    spawn(async move {
      ...
      if used_only {
        return Err(generic_error("Bench failed because the \"only\" option was used"));
      }
...
fn is_supported_bench_path(path: &Path) -> bool {
  if let Some(name) = path.file_stem() {
    ...
    (basename.ends_with("_bench") || basename.ends_with(".bench") || basename == "bench") && is_script_ext(path)
  } else {
    false
  }
}

pub async fn run_benchmarks(flags: Flags, bench_flags: BenchFlags) -> Result<(), AnyError> {
  ...
  check_specifiers(cli_options, factory.module_load_preparer().await?, specifiers.clone()).await?;
...
pub async fn run_benchmarks_with_watch(flags: Flags, bench_flags: BenchFlags) -> Result<(), AnyError> {
  file_watcher::watch_func(
    flags,
    file_watcher::PrintConfig::new("Bench", bench_flags.watch.as_ref().map(|w| !w.no_clear_screen).unwrap_or(true)),
    move |flags, watcher_communicator, changed_paths| {
      ...
      Ok(async move {
        ...
        let bench_modules = collect_specifiers(&bench_options.files, is_supported_bench_path)?;
        ...
        let graph = module_graph_builder.create_graph(graph_kind, bench_modules.clone()).await?;
        ...
        let bench_modules_to_reload = if let Some(changed_paths) = changed_paths {
          ...
          for bench_module_specifier in bench_modules {
            if has_graph_root_local_dependent_changed(&graph, &bench_module_specifier, &changed_specifiers) {
