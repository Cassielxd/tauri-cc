...
impl BenchReporter for JsonReporter {
  ...
  fn report_result(&mut self, desc: &BenchDescription, result: &BenchResult) {
    ...
    let maybe_bench = self
      .0
      .benches
      .iter_mut()
      .find(|bench| bench.origin == desc.origin && bench.group == desc.group && bench.name == desc.name && bench.baseline == desc.baseline);
...
impl BenchReporter for ConsoleReporter {
  ...
  fn report_plan(&mut self, plan: &BenchPlan) {
    ...
    self.options = Some(mitata::reporter::Options::new(&plan.names.iter().map(|x| x.as_str()).collect::<Vec<&str>>()));
    ...
    if FIRST_PLAN.compare_exchange(true, false, Ordering::SeqCst, Ordering::SeqCst).is_ok() {
      println!("{}", colors::gray(format!("cpu: {}", mitata::cpu::name())));
      println!("{}\n", colors::gray(format!("runtime: deno {} ({})", crate::version::deno(), env!("TARGET"))));
    } else {
      println!();
    }
  ...
  fn report_wait(&mut self, desc: &BenchDescription) {
    ...
    match &desc.group {
      ...
      Some(group) => {
        if self.group.is_none() && self.has_ungrouped && self.group_measurements.is_empty() {
          println!();
        }
        ...
        if (self.group.is_none() && self.has_ungrouped) || (self.group.is_some() && self.group_measurements.is_empty()) {
          println!();
        }
  ...
  fn report_group_summary(&mut self) {
    ...
    if 2 <= self.group_measurements.len() && (self.group.is_some() || (self.group.is_none() && self.baseline)) {
      println!(
        "\n{}",
