...
pub async fn bundle(flags: Flags, bundle_flags: BundleFlags) -> Result<(), AnyError> {
  log::info!("{} \"deno bundle\" is deprecated and will be removed in the future.", colors::yellow("Warning"),);
  log::info!("Use alternative bundlers like \"deno_emit\", \"esbuild\" or \"rollup\" instead.");

  if let Some(watch_flags) = &bundle_flags.watch {
    util::file_watcher::watch_func(flags, util::file_watcher::PrintConfig::new("Bundle", !watch_flags.no_clear_screen), move |flags, watcher_communicator, _changed_paths| {
...
async fn bundle_action(factory: CliFactory, bundle_flags: &BundleFlags) -> Result<(), AnyError> {
  let cli_options = factory.cli_options();
  ...
  if let Ok(Some(import_map_path)) = cli_options.resolve_import_map_specifier().map(|ms| ms.and_then(|ref s| s.to_file_path().ok())) {
  ...
  if let Some(out_file) = out_file {
    ...
    log::info!("{} {:?} ({})", colors::green("Emit"), out_file, colors::gray(display::human_size(output_len as f64)));
    if let Some(bundle_map) = bundle_output.maybe_map {
      let map_bytes = bundle_map.as_bytes();
      let map_len = map_bytes.len();
      let ext = if let Some(curr_ext) = out_file.extension() { format!("{}.map", curr_ext.to_string_lossy()) } else { "map".to_string() };
      let map_out_file = out_file.with_extension(ext);
      util::fs::write_file(&map_out_file, map_bytes, 0o644)?;
      log::info!("{} {:?} ({})", colors::green("Emit"), map_out_file, colors::gray(display::human_size(map_len as f64)));
    }
  } else {
...
fn bundle_module_graph(graph: &deno_graph::ModuleGraph, cli_options: &CliOptions) -> Result<deno_emit::BundleEmit, AnyError> {
  ...
  deno_emit::bundle_graph(
    ...
    deno_emit::BundleOptions {
      ...
      emit_options: crate::args::ts_config_to_emit_options(ts_config_result.ts_config),
