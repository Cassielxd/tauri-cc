...
impl TypeChecker {
  pub fn new(caches: Arc<Caches>, cli_options: Arc<CliOptions>, node_resolver: Arc<NodeResolver>, npm_resolver: Arc<dyn CliNpmResolver>) -> Self {
    Self {
      caches,
      cli_options,
      node_resolver,
  ...
  pub async fn check(&self, graph: Arc<ModuleGraph>, options: CheckOptions) -> Result<(), AnyError> {
    ...
    let ts_config_result = self.cli_options.resolve_ts_config_for_emit(TsConfigType::Check { lib: options.lib })?;
    ...
    let maybe_check_hash = match self.npm_resolver.check_state_hash() {
      Some(npm_check_hash) => match get_check_hash(&graph, npm_check_hash, type_check_mode, &ts_config) {
    ...
    // case.
    let maybe_tsbuildinfo = if options.reload { None } else { cache.get_tsbuildinfo(&graph.roots[0]) };
    // to make tsc build info work, we need to consistently hash modules, so that
    // tsc can better determine if an emit is still valid or not, so we provide
    // that data here.
    let hash_data = FastInsecureHasher::new().write(&ts_config.as_bytes()).write_str(version::deno()).finish();
      ...
      response.diagnostics.filter(|d| {
        if let Some(file_name) = &d.file_name {
          if !file_name.starts_with("http") {
            if ModuleSpecifier::parse(file_name).map(|specifier| !self.node_resolver.in_npm_package(&specifier)).unwrap_or(true) {
              Some(d.clone())
            } else {
              None
            }
...
/// be used to tell
fn get_check_hash(graph: &ModuleGraph, package_reqs_hash: u64, type_check_mode: TypeCheckMode, ts_config: &TsConfig) -> CheckHashResult {
    ...
    match module {
      Module::Esm(module) => {
        ...
        match module.media_type {
          MediaType::TypeScript | MediaType::Dts | MediaType::Dmts | MediaType::Dcts | MediaType::Mts | MediaType::Cts | MediaType::Tsx => {
            has_file = true;
            has_file_to_type_check = true;
          }
          ...
          MediaType::Json | MediaType::TsBuildInfo | MediaType::SourceMap | MediaType::Wasm | MediaType::Unknown => continue,
        }
...
fn get_tsc_roots(graph: &ModuleGraph, check_js: bool) -> Vec<(ModuleSpecifier, MediaType)> {
  fn maybe_get_check_entry(module: &deno_graph::Module, check_js: bool) -> Option<(ModuleSpecifier, MediaType)> {
    match module {
      Module::Esm(module) => match module.media_type {
        MediaType::TypeScript | MediaType::Tsx | MediaType::Mts | MediaType::Cts | MediaType::Dts | MediaType::Dmts | MediaType::Dcts => Some((module.specifier.clone(), module.media_type)),
        MediaType::JavaScript | MediaType::Mjs | MediaType::Cjs | MediaType::Jsx => {
  ...
  if graph.has_node_specifier {
    ...
    result.push((ModuleSpecifier::parse("asset:///node_types.d.ts").unwrap(), MediaType::Dts));
  }

  let mut seen_roots = HashSet::with_capacity(graph.imports.len() + graph.roots.len());
    ...
    for dep in import.dependencies.values() {
      ...
      if let Some(specifier) = &specifier {
        if seen_roots.insert(*specifier) {
          let maybe_entry = graph.get(specifier).and_then(|m| maybe_get_check_entry(m, check_js));
  ...
  // now the rest
  result.extend(graph.modules().filter_map(|module| if seen_roots.contains(module.specifier()) { None } else { maybe_get_check_entry(module, check_js) }));
...
fn has_ts_check(media_type: MediaType, file_text: &str) -> bool {
  match &media_type {
    MediaType::JavaScript | MediaType::Mjs | MediaType::Cjs | MediaType::Jsx => get_leading_comments(file_text).iter().any(|text| TS_CHECK_RE.is_match(text)),
    MediaType::TypeScript | MediaType::Mts | MediaType::Cts | MediaType::Dts | MediaType::Dcts | MediaType::Dmts | MediaType::Tsx | MediaType::Json | MediaType::Wasm | MediaType::TsBuildInfo | MediaType::SourceMap | MediaType::Unknown => false,
  }
}
...
fn get_leading_comments(file_text: &str) -> Vec<String> {
  ...
  while chars.peek().is_some() {
    ...
    while chars.peek().map(|c| char::is_whitespace(*c)).unwrap_or(false) {
      chars.next();
    }
