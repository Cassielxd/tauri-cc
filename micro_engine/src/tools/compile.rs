...
pub async fn compile(flags: Flags, compile_flags: CompileFlags) -> Result<(), AnyError> {
  ...
  let output_path = resolve_compile_executable_output_path(&compile_flags, cli_options.initial_cwd()).await?;

  let graph = Arc::try_unwrap(module_graph_builder.create_graph_and_maybe_check(module_roots.clone()).await?).unwrap();
  let graph = if cli_options.type_check_mode().is_true() {
    ...
    module_graph_builder.create_graph(GraphKind::CodeOnly, module_roots).await?
  } else {
    graph
  };
  ...
  log::info!("{} {} to {}", colors::green("Compile"), module_specifier.to_string(), output_path.display(),);
...
async fn resolve_compile_executable_output_path(compile_flags: &CompileFlags, current_dir: &Path) -> Result<PathBuf, AnyError> {
  ...
  if let Some(out) = output.as_ref() {
    if path_has_trailing_slash(out) {
      if let Some(infer_file_name) = infer_name_from_url(&module_specifier).await.map(PathBuf::from) {
  ...
  if output.is_none() {
    output = infer_name_from_url(&module_specifier).await.map(PathBuf::from)
  }
