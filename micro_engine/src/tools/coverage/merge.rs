...
pub fn merge_processes(mut processes: Vec<ProcessCoverage>) -> Option<ProcessCoverage> {
  ...
  for process_cov in processes {
    for script_cov in process_cov.result {
      url_to_scripts.entry(script_cov.url.clone()).or_default().push(script_cov);
    }
  }
...
pub fn merge_scripts(mut scripts: Vec<cdp::ScriptCoverage>) -> Option<cdp::ScriptCoverage> {
  ...
  let functions: Vec<cdp::FunctionCoverage> = range_to_funcs.into_values().map(|funcs| merge_functions(funcs).unwrap()).collect();

  Some(cdp::ScriptCoverage { script_id, url, functions })
}
...
pub fn merge_functions(mut funcs: Vec<cdp::FunctionCoverage>) -> Option<cdp::FunctionCoverage> {
  ...
  Some(cdp::FunctionCoverage { function_name, ranges, is_block_coverage })
}
...
fn into_start_events<'a>(trees: Vec<&'a mut RangeTree<'a>>) -> Vec<StartEvent> {
  ...
  for (parent_index, tree) in trees.into_iter().enumerate() {
    for child in tree.children.drain(..) {
      result.entry(child.start).or_default().push((parent_index, child));
    }
  }
  result.into_iter().map(|(offset, trees)| StartEvent { offset, trees }).collect()
}
...
fn merge_range_tree_children<'a>(rta: &'a RangeTreeArena<'a>, parent_trees: Vec<&'a mut RangeTree<'a>>) -> Vec<&'a mut RangeTree<'a>> {
    ...
    match open_range {
      Some(open_range) => {
        for (parent_index, tree) in event.trees {
          ...
          parent_to_nested.entry(parent_index).or_default().push(child);
        }
      }
      None => {
        ...
        open_range = Some(CharRange { start: event.offset, end: open_range_end });
      }
    }
  }
  ...
  let child_forests: Vec<Vec<&'a mut RangeTree<'a>>> = flat_children.into_iter().zip(wrapped_children).map(|(flat, wrapped)| merge_children_lists(flat, wrapped)).collect();

  let events = get_child_events_from_forests(&child_forests);

  let mut child_forests: Vec<Peekable<::std::vec::IntoIter<&'a mut RangeTree<'a>>>> = child_forests.into_iter().map(|forest| forest.into_iter().peekable()).collect();
  ...
  for event in events.iter() {
    ...
    for (_parent_index, children) in child_forests.iter_mut().enumerate() {
      let next_tree: Option<&'a mut RangeTree<'a>> = {
        if children.peek().map(|tree| tree.start == *event).unwrap_or(false) {
          children.next()
        } else {
          None
        }
