...
impl<'rt> RangeTree<'rt> {
  pub fn new<'a>(start: usize, end: usize, delta: i64, children: Vec<&'a mut RangeTree<'a>>) -> RangeTree<'a> {
    RangeTree { start, end, delta, children }
  }
  ...
  pub fn from_sorted_ranges<'a>(rta: &'a RangeTreeArena<'a>, ranges: &[cdp::CoverageRange]) -> Option<&'a mut RangeTree<'a>> {
    Self::from_sorted_ranges_inner(rta, &mut ranges.iter().peekable(), ::std::usize::MAX, 0)
  }

  fn from_sorted_ranges_inner<'a, 'b, 'c: 'b>(rta: &'a RangeTreeArena<'a>, ranges: &'b mut Peekable<impl Iterator<Item = &'c cdp::CoverageRange>>, parent_end: usize, parent_count: i64) -> Option<&'a mut RangeTree<'a>> {
