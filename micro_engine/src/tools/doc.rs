...
async fn generate_doc_nodes_for_builtin_types(doc_flags: DocFlags, cli_options: &Arc<CliOptions>, capturing_parser: CapturingModuleParser<'_>, analyzer: &dyn ModuleAnalyzer) -> Result<IndexMap<ModuleSpecifier, Vec<doc::DocNode>>, AnyError> {
...
pub async fn doc(flags: Flags, doc_flags: DocFlags) -> Result<(), AnyError> {
  ...
  let analyzer = module_info_cache.as_module_analyzer(Some(&source_parser), &store);
  let capturing_parser = CapturingModuleParser::new(Some(&source_parser), &store);

  let doc_nodes_by_url = match doc_flags.source_files {
    DocSourceFileFlag::Builtin => generate_doc_nodes_for_builtin_types(doc_flags.clone(), cli_options, capturing_parser, &analyzer).await?,
    DocSourceFileFlag::Paths(ref source_files) => {
      let module_graph_builder = factory.module_graph_builder().await?;
      let maybe_lockfile = factory.maybe_lockfile();

      let expanded_globs = expand_globs(source_files.iter().map(PathBuf::from).collect())?;
      let module_specifiers: Result<Vec<ModuleSpecifier>, AnyError> = expanded_globs.iter().map(|source_file| Ok(resolve_url_or_path(&source_file.to_string_lossy(), cli_options.initial_cwd())?)).collect();
  ...
  if let Some(html_options) = doc_flags.html {
    generate_docs_directory(&doc_nodes_by_url, html_options).boxed_local().await
  } else {
    ...
    if doc_flags.json {
      ...
      log::info!("Checked {} file{}", modules_len, if modules_len == 1 { "" } else { "s" });
      Ok(())
    } else {
...
async fn generate_docs_directory(doc_nodes_by_url: &IndexMap<ModuleSpecifier, Vec<doc::DocNode>>, html_options: DocHtmlFlag) -> Result<(), AnyError> {
  let cwd = std::env::current_dir().context("Failed to get CWD")?;
  let output_dir_resolved = cwd.join(&html_options.output);

  let options = deno_doc::html::GenerateOptions { package_name: html_options.name };

  let files = deno_doc::html::generate(options, doc_nodes_by_url).context("Failed to generate HTML documentation")?;
  ...
  for (name, content) in files {
    ...
    let prefix = this_path.parent().with_context(|| format!("Failed to get parent path for {:?}", this_path))?;
    std::fs::create_dir_all(prefix).with_context(|| format!("Failed to create directory {:?}", prefix))?;
    std::fs::write(&this_path, content).with_context(|| format!("Failed to write file {:?}", this_path))?;
  }

  log::info!("{}", colors::green(format!("Written {} files to {:?}", no_of_files, html_options.output)));
  Ok(())
}

fn print_docs_to_stdout(doc_flags: DocFlags, mut doc_nodes: Vec<deno_doc::DocNode>) -> Result<(), AnyError> {
...
fn check_diagnostics(diagnostics: &[DocDiagnostic]) -> Result<(), AnyError> {
  ...
  bail!("Found {} documentation lint error{}.", colors::bold(diagnostics.len().to_string()), if diagnostics.len() == 1 { "" } else { "s" });
}
