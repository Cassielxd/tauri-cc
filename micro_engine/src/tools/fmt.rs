...
pub async fn format(flags: Flags, fmt_flags: FmtFlags) -> Result<(), AnyError> {
  if fmt_flags.is_stdin() {
    ...
    return format_stdin(fmt_options, cli_options.ext_flag().as_ref().map(|s| s.as_str()).unwrap_or("ts"));
  }

  if let Some(watch_flags) = &fmt_flags.watch {
    file_watcher::watch_func(flags, file_watcher::PrintConfig::new("Fmt", !watch_flags.no_clear_screen), move |flags, watcher_communicator, changed_paths| {
      let fmt_flags = fmt_flags.clone();
      Ok(async move {
        let factory = CliFactory::from_flags(flags).await?;
        let cli_options = factory.cli_options();
        let fmt_options = cli_options.resolve_fmt_options(fmt_flags)?;
        let files = collect_fmt_files(&fmt_options.files).and_then(|files| if files.is_empty() { Err(generic_error("No target files found.")) } else { Ok(files) })?;
        ...
        let refmt_files = if let Some(paths) = changed_paths {
          if fmt_options.check {
            ...
            files.iter().any(|path| paths.contains(path)).then_some(files).unwrap_or_else(|| [].to_vec())
          } else {
            files.into_iter().filter(|path| paths.contains(path)).collect::<Vec<_>>()
          }
        } else {
          files
        };
...
async fn format_files(factory: CliFactory, fmt_options: FmtOptions, paths: Vec<PathBuf>) -> Result<(), AnyError> {
  let caches = factory.caches()?;
  let check = fmt_options.check;
  let incremental_cache = Arc::new(IncrementalCache::new(caches.fmt_incremental_cache_db(), &fmt_options.options, &paths));
  if check {
...
/// (ts/tsx, js/jsx).
fn format_markdown(file_text: &str, fmt_options: &FmtOptionsConfig) -> Result<Option<String>, AnyError> {
  ...
  dprint_plugin_markdown::format_text(file_text, &markdown_config, move |tag, text, line_width| {
    ...
    if matches!(tag.as_str(), "ts" | "tsx" | "js" | "jsx" | "cjs" | "cts" | "mjs" | "mts" | "javascript" | "typescript" | "json" | "jsonc") {
...
pub fn format_json(file_path: &Path, file_text: &str, fmt_options: &FmtOptionsConfig) -> Result<Option<String>, AnyError> {
...
pub fn format_file(file_path: &Path, file_text: &str, fmt_options: &FmtOptionsConfig) -> Result<Option<String>, AnyError> {
  let ext = get_extension(file_path).unwrap_or_default();
  if matches!(ext.as_str(), "md" | "mkd" | "mkdn" | "mdwn" | "mdown" | "markdown") {
...
pub fn format_parsed_source(parsed_source: &ParsedSource, fmt_options: &FmtOptionsConfig) -> Result<Option<String>, AnyError> {
  dprint_plugin_typescript::format_parsed_source(parsed_source, &get_resolved_typescript_config(fmt_options))
}

async fn check_source_files(paths: Vec<PathBuf>, fmt_options: FmtOptionsConfig, incremental_cache: Arc<IncrementalCache>) -> Result<(), AnyError> {
  ...
  if not_formatted_files_count == 0 {
    ...
    Err(generic_error(format!("Found {not_formatted_files_count} not formatted {not_formatted_files_str} in {checked_files_str}",)))
  }
}

async fn format_source_files(paths: Vec<PathBuf>, fmt_options: FmtOptionsConfig, incremental_cache: Arc<IncrementalCache>) -> Result<(), AnyError> {
  ...
  let checked_files_count = checked_files_count.load(Ordering::Relaxed);
  info!("Checked {} {}", checked_files_count, files_str(checked_files_count));
...
fn format_ensure_stable(file_path: &Path, file_text: &str, fmt_options: &FmtOptionsConfig, fmt_func: impl Fn(&Path, &str, &FmtOptionsConfig) -> Result<Option<String>, AnyError>) -> Result<Option<String>, AnyError> {
...
fn get_resolved_typescript_config(options: &FmtOptionsConfig) -> dprint_plugin_typescript::configuration::Configuration {
  ...
  if let Some(single_quote) = options.single_quote {
    if single_quote {
      builder.quote_style(dprint_plugin_typescript::configuration::QuoteStyle::AlwaysSingle);
    }
  }
...
fn get_resolved_markdown_config(options: &FmtOptionsConfig) -> dprint_plugin_markdown::configuration::Configuration {
...
fn get_resolved_json_config(options: &FmtOptionsConfig) -> dprint_plugin_json::configuration::Configuration {
...
fn read_file_contents(file_path: &Path) -> Result<FileContents, AnyError> {
  let file_bytes = fs::read(file_path).with_context(|| format!("Error reading {}", file_path.display()))?;
  let charset = text_encoding::detect_charset(&file_bytes);
  let file_text = text_encoding::convert_to_utf8(&file_bytes, charset).map_err(|_| anyhow!("{} is not a valid UTF-8 file", file_path.display()))?;
  let had_bom = file_text.starts_with(text_encoding::BOM_CHAR);
  let text = if had_bom { text_encoding::strip_bom(&file_text).to_string() } else { file_text.to_string() };

  Ok(FileContents { text, had_bom })
}

fn write_file_contents(file_path: &Path, file_contents: FileContents) -> Result<(), AnyError> {
  ...
  F: FnOnce(PathBuf) -> Result<(), AnyError> + Send + 'static + Clone,
{
  ...
  let mut errors = join_results.into_iter().filter_map(|join_result| join_result.ok().and_then(|handle_result| handle_result.err()));
...
fn is_supported_ext_fmt(path: &Path) -> bool {
  if let Some(ext) = get_extension(path) {
    matches!(ext.as_str(), "ts" | "tsx" | "js" | "jsx" | "cjs" | "cts" | "mjs" | "mts" | "json" | "jsonc" | "md" | "mkd" | "mkdn" | "mdwn" | "mdown" | "markdown")
  } else {
    false
  }
}
