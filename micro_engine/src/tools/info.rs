...
pub async fn info(flags: Flags, info_flags: InfoFlags) -> Result<(), AnyError> {
  ...
  if let Some(specifier) = info_flags.file {
    ...
    let graph = module_graph_builder.create_graph_with_loader(GraphKind::All, vec![specifier], &mut loader).await?;
    ...
    print_cache_info(&factory, info_flags.json, cli_options.location_flag().as_ref())?;
  }
  Ok(())
}

fn print_cache_info(factory: &CliFactory, json: bool, location: Option<&deno_core::url::Url>) -> Result<(), AnyError> {
  ...
  if json {
    ...
    println!("{} {}", colors::bold("Remote modules cache:"), modules_cache.display());
    println!("{} {}", colors::bold("npm modules cache:"), npm_cache.display());
    println!("{} {}", colors::bold("Emitted modules cache:"), typescript_cache.display());
    println!("{} {}", colors::bold("Language server registries cache:"), registry_cache.display(),);
    println!("{} {}", colors::bold("Origin storage:"), origin_dir.display());
    if location.is_some() {
      println!("{} {}", colors::bold("Local Storage:"), local_storage_dir.display(),);
    }
    Ok(())
  }
}

fn add_npm_packages_to_json(json: &mut serde_json::Value, npm_resolver: &dyn CliNpmResolver) {
  ...
  if let Some(modules) = modules {
    if modules.len() == 1 && modules[0].get("kind").and_then(|k| k.as_str()) == Some("npm") {
      ...
      let maybe_package = module
        ...
        .and_then(|package_ref| snapshot.resolve_package_from_deno_module(package_ref.nv()).ok());
      ...
      for i in (0..modules.len()).rev() {
        if matches!(modules[i].get("kind").and_then(|k| k.as_str()), Some("npm") | Some("external")) {
          modules.remove(i);
        }
      }
    }
    ...
    for module in modules.iter_mut() {
      let dependencies = module.get_mut("dependencies").and_then(|d| d.as_array_mut());
  ...
  for pkg in sorted_packages {
    ...
    deps.sort();
    let deps = deps.into_iter().map(|id| serde_json::Value::String(id.as_serialized())).collect::<Vec<_>>();
...
impl TreeNode {
  pub fn from_text(text: String) -> Self {
    Self { text, children: Default::default() }
  }
}

fn print_tree_node<TWrite: Write>(tree_node: &TreeNode, writer: &mut TWrite) -> fmt::Result {
  fn print_children<TWrite: Write>(writer: &mut TWrite, prefix: &str, children: &Vec<TreeNode>) -> fmt::Result {
    ...
    for (index, child) in children.iter().enumerate() {
      ...
      let sibling_connector = if is_last { LAST_SIBLING_CONNECTOR } else { SIBLING_CONNECTOR };
      let child_connector = if child.children.is_empty() { CHILD_NO_DEPS_CONNECTOR } else { CHILD_DEPS_CONNECTOR };
      writeln!(writer, "{} {}", colors::gray(format!("{prefix}{sibling_connector}─{child_connector}")), child.text)?;
      let child_prefix = format!("{}{}{}", prefix, if is_last { EMPTY_CONNECTOR } else { VERTICAL_CONNECTOR }, EMPTY_CONNECTOR);
...
impl NpmInfo {
  pub fn build<'a>(graph: &'a ModuleGraph, npm_resolver: &'a ManagedCliNpmResolver, npm_snapshot: &'a NpmResolutionSnapshot) -> Self {
  ...
  fn fill_package_info<'a>(&mut self, package: &NpmResolutionPackage, npm_resolver: &'a ManagedCliNpmResolver, npm_snapshot: &'a NpmResolutionSnapshot) {
...
impl<'a> GraphDisplayContext<'a> {
  pub fn write<TWrite: Write>(graph: &'a ModuleGraph, npm_resolver: &'a dyn CliNpmResolver, writer: &mut TWrite) -> fmt::Result {
    ...
    Self { graph, npm_info, seen: Default::default() }.into_writer(writer)
  }

  fn into_writer<TWrite: Write>(mut self, writer: &mut TWrite) -> fmt::Result {
    if self.graph.roots.is_empty() || self.graph.roots.len() > 1 {
      return writeln!(writer, "{} displaying graphs that have multiple roots is not supported.", colors::red("error:"));
    }
    ...
    match self.graph.try_get(&root_specifier) {
      Ok(Some(root)) => {
        ...
        if let Some(cache_info) = maybe_cache_info {
          if let Some(local) = &cache_info.local {
            writeln!(writer, "{} {}", colors::bold("local:"), local.to_string_lossy())?;
          }
          if let Some(emit) = &cache_info.emit {
            writeln!(writer, "{} {}", colors::bold("emit:"), emit.to_string_lossy())?;
          }
        ...
        let total_modules_size = self
          ...
          .sum::<f64>();
        let total_npm_package_size = self.npm_info.package_sizes.values().map(|s| *s as f64).sum::<f64>();
        ...
        writeln!(writer, "{} {} unique", colors::bold("dependencies:"), dep_count,)?;
        writeln!(writer, "{} {}", colors::bold("size:"), display::human_size(total_size),)?;
        writeln!(writer)?;
      ...
      Err(err) => {
        if let ModuleError::Missing(_, _) = *err {
          writeln!(writer, "{} module could not be found", colors::red("error:"))
        } else {
          writeln!(writer, "{} {:#}", colors::red("error:"), err)
        }
      }
      Ok(None) => {
        writeln!(writer, "{} an internal error occurred", colors::red("error:"))
      }
    }
  }
  ...
  fn build_module_info(&mut self, module: &Module, type_dep: bool) -> TreeNode {
    ...
    let header_text = if was_seen {
      let specifier_str = if type_dep { colors::italic_gray(module.specifier()).to_string() } else { colors::gray(module.specifier()).to_string() };
      format!("{} {}", specifier_str, colors::gray("*"))
    } else {
      let header_text = if type_dep { colors::italic(module.specifier()).to_string() } else { module.specifier().to_string() };
  ...
  fn build_npm_deps(&mut self, package: &NpmResolutionPackage) -> Vec<TreeNode> {
  ...
  fn build_error_info(&mut self, err: &ModuleError, specifier: &ModuleSpecifier) -> TreeNode {
    self.seen.insert(specifier.to_string());
    match err {
      ModuleError::InvalidTypeAssertion { .. } => self.build_error_msg(specifier, "(invalid import attribute)"),
      ...
      ModuleError::UnsupportedImportAttributeType { .. } => self.build_error_msg(specifier, "(unsupported import attribute)"),
      ModuleError::UnsupportedMediaType { .. } => self.build_error_msg(specifier, "(unsupported)"),
      ModuleError::Missing(_, _) | ModuleError::MissingDynamic(_, _) => self.build_error_msg(specifier, "(missing)"),
      ModuleError::MissingWorkspaceMemberExports { .. } => self.build_error_msg(specifier, "(missing exports)"),
      ...
      ModuleError::UnknownPackageReq { .. } => self.build_error_msg(specifier, "(unknown package constraint)"),
    }
  }

  fn build_error_msg(&self, specifier: &ModuleSpecifier, error_msg: &str) -> TreeNode {
    TreeNode::from_text(format!("{} {}", colors::red(specifier), colors::red_bold(error_msg)))
  }

  fn build_resolved_info(&mut self, resolution: &Resolution, type_dep: bool) -> Option<TreeNode> {
    match resolution {
      Resolution::Ok(resolved) => {
        ...
        Some(match self.graph.try_get(&resolved_specifier) {
          ...
          Ok(None) => TreeNode::from_text(format!("{} {}", colors::red(specifier), colors::red_bold("(missing)"))),
        })
      }
      Resolution::Err(err) => Some(TreeNode::from_text(format!("{} {}", colors::italic(err.to_string()), colors::red_bold("(resolve error)")))),
      _ => None,
    }
  }
}
