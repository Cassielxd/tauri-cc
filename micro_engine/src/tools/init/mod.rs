...
fn create_file(dir: &Path, filename: &str, content: &str) -> Result<(), AnyError> {
  ...
  if path.exists() {
    info!("ℹ️ {}", colors::gray(format!("Skipped creating {filename} as it already exists")));
    Ok(())
  } else {
    let mut file = std::fs::OpenOptions::new().write(true).create_new(true).open(path).with_context(|| format!("Failed to create {filename} file"))?;
...
pub async fn init_project(init_flags: InitFlags) -> Result<(), AnyError> {
  ...
  let main_test_ts = include_str!("./templates/main_test.ts").replace("{CURRENT_STD_URL}", deno_std::CURRENT_STD_URL_STR);
  ...
  info!("");
  info!("  {}", colors::gray("# Run the program and watch for file changes"));
  info!("  deno task dev");
  info!("");
