...
impl<S: zeromq::Socket> Connection<S> {
  pub(crate) fn new(socket: S, key: &str) -> Self {
    let mac = if key.is_empty() { None } else { Some(hmac::Key::new(hmac::HMAC_SHA256, key.as_bytes())) };
...
impl RawMessage {
  pub(crate) async fn read<S: zeromq::SocketRecv>(connection: &mut Connection<S>) -> Result<RawMessage, AnyError> {
    Self::from_multipart(connection.socket.recv().await?, connection)
  }

  pub(crate) fn from_multipart<S>(multipart: zeromq::ZmqMessage, connection: &Connection<S>) -> Result<RawMessage, AnyError> {
    let delimiter_index = multipart.iter().position(|part| &part[..] == DELIMITER).ok_or_else(|| anyhow!("Missing delimiter"))?;
  ...
  async fn send<S: zeromq::SocketSend>(self, connection: &mut Connection<S>) -> Result<(), AnyError> {
...
impl JupyterMessage {
  pub(crate) async fn read<S: zeromq::SocketRecv>(connection: &mut Connection<S>) -> Result<JupyterMessage, AnyError> {
  ...
  fn from_raw_message(raw_message: RawMessage) -> Result<JupyterMessage, AnyError> {
    ...
    Ok(JupyterMessage {
      ...
      buffers: if raw_message.jparts.len() > 4 { raw_message.jparts[4..].to_vec() } else { vec![] },
    })
  }
  ...
  pub(crate) async fn send<S: zeromq::SocketSend>(&self, connection: &mut Connection<S>) -> Result<(), AnyError> {
    // If performance is a concern, we can probably avoid the clone and to_vec calls with a bit
    // of refactoring.
    let mut jparts: Vec<Bytes> = vec![
      serde_json::to_string(&self.header).unwrap().as_bytes().to_vec().into(),
      serde_json::to_string(&self.parent_header).unwrap().as_bytes().to_vec().into(),
      serde_json::to_string(&self.metadata).unwrap().as_bytes().to_vec().into(),
      serde_json::to_string(&self.content).unwrap().as_bytes().to_vec().into(),
    ];
...
impl fmt::Debug for JupyterMessage {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    writeln!(f, "\nHeader: {}", serde_json::to_string_pretty(&self.header).unwrap())?;
    writeln!(f, "Parent header: {}", serde_json::to_string_pretty(&self.parent_header).unwrap())?;
    writeln!(f, "Metadata: {}", serde_json::to_string_pretty(&self.metadata).unwrap())?;
