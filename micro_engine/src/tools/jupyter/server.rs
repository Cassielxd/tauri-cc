...
impl JupyterServer {
  pub async fn start(spec: ConnectionSpec, mut stdio_rx: mpsc::UnboundedReceiver<StdioMsg>, mut repl_session: repl::ReplSession) -> Result<(), AnyError> {
    ...
    let handle4 = deno_core::unsync::spawn(async move {
      while let Some(stdio_msg) = stdio_rx.recv().await {
        Self::handle_stdio_msg(iopub_socket.clone(), last_execution_request.clone(), stdio_msg).await;
      }
    });
  ...
  async fn handle_stdio_msg<S: zeromq::SocketSend>(iopub_socket: Arc<Mutex<Connection<S>>>, last_execution_request: Rc<RefCell<Option<JupyterMessage>>>, stdio_msg: StdioMsg) {
  ...
  async fn handle_heartbeat(connection: &mut Connection<zeromq::RepSocket>) -> Result<(), AnyError> {
    loop {
  ...
  async fn handle_control(mut connection: Connection<zeromq::RouterSocket>, cancel_handle: Rc<CancelHandle>) -> Result<(), AnyError> {
    loop {
      let msg = JupyterMessage::read(&mut connection).await?;
      match msg.message_type() {
        "kernel_info_request" => {
          msg.new_reply().with_content(kernel_info()).send(&mut connection).await?;
        }
        "shutdown_request" => {
  ...
  async fn handle_shell(&mut self, mut connection: Connection<zeromq::RouterSocket>) -> Result<(), AnyError> {
    loop {
      let msg = JupyterMessage::read(&mut connection).await?;
  ...
  async fn handle_shell_message(&mut self, msg: JupyterMessage, connection: &mut Connection<zeromq::RouterSocket>) -> Result<(), AnyError> {
    msg.new_message("status").with_content(json!({"execution_state": "busy"})).send(&mut *self.iopub_socket.lock().await).await?;

    match msg.message_type() {
      "kernel_info_request" => {
        msg.new_reply().with_content(kernel_info()).send(connection).await?;
      }
      "is_complete_request" => {
        msg.new_reply().with_content(json!({"status": "complete"})).send(connection).await?;
      }
      "execute_request" => {
        self.handle_execution_request(msg.clone(), connection).await?;
      }
      "comm_open" => {
        msg.comm_close_message().send(&mut *self.iopub_socket.lock().await).await?;
      }
      "complete_request" => {
        let user_code = msg.code();
        let cursor_pos = msg.cursor_pos();

        let lsp_completions = self.repl_session.language_server.completions(user_code, cursor_pos).await;

        if !lsp_completions.is_empty() {
          let matches: Vec<String> = lsp_completions.iter().map(|item| item.new_text.clone()).collect();

          let cursor_start = lsp_completions.first().map(|item| item.range.start).unwrap_or(cursor_pos);

          let cursor_end = lsp_completions.last().map(|item| item.range.end).unwrap_or(cursor_pos);
          ...
          let (completions, cursor_start) = if let Some(index) = expr.rfind('.') {
            ...
            let candidates = get_expression_property_names(&mut self.repl_session, sub_expr)
              .await
              .into_iter()
              .filter(|n| !n.starts_with("Symbol(") && n.starts_with(prop_name) && n != &*repl::REPL_INTERNALS_NAME)
              .collect();

            (candidates, cursor_pos - prop_name.len())
          } else {
            // combine results of declarations and globalThis properties
            let mut candidates = get_expression_property_names(&mut self.repl_session, "globalThis")
              .await
              .into_iter()
    ...
    msg.new_message("status").with_content(json!({"execution_state": "idle"})).send(&mut *self.iopub_socket.lock().await).await?;
    Ok(())
  }

  async fn handle_execution_request(&mut self, msg: JupyterMessage, connection: &mut Connection<zeromq::RouterSocket>) -> Result<(), AnyError> {
    ...
    *self.last_execution_request.borrow_mut() = Some(msg.clone());
    ...
    let result = self.repl_session.evaluate_line_with_object_wrapping(msg.code()).await;
    ...
    if exception_details.is_none() {
      ...
      let stack = if stack.is_empty() { format!("{}\n    at <unknown>", serde_json::to_string(&message).unwrap()) } else { stack };
      let traceback = format!("Stack trace:\n{stack}").split('\n').map(|s| s.to_owned()).collect::<Vec<_>>();

      let ename = if name.is_empty() { "Unknown error".into() } else { name };

      let evalue = if message.is_empty() { "(none)".into() } else { message };
...
async fn bind_socket<S: zeromq::Socket>(config: &ConnectionSpec, port: u32) -> Result<Connection<S>, AnyError> {
...
async fn publish_result(session: &mut repl::ReplSession, evaluate_result: &cdp::RemoteObject, execution_count: usize) -> Result<Option<HashMap<String, serde_json::Value>>, AnyError> {
...
fn get_expr_from_line_at_pos(line: &str, cursor_pos: usize) -> &str {
  ...
  let end = line[cursor_pos..].rfind(is_word_boundary).map(|i| cursor_pos + i).unwrap_or(cursor_pos);
...
// TODO(bartlomieju): dedup with repl::editor
async fn get_expression_property_names(session: &mut repl::ReplSession, expr: &str) -> Vec<String> {
  ...
  get_object_expr_properties(session, object_expr).await.unwrap_or_default()
}

// TODO(bartlomieju): dedup with repl::editor
async fn get_expression_type(session: &mut repl::ReplSession, expr: &str) -> Option<String> {
  evaluate_expression(session, expr).await.map(|res| res.result.kind)
}

// TODO(bartlomieju): dedup with repl::editor
async fn get_object_expr_properties(session: &mut repl::ReplSession, object_expr: &str) -> Option<Vec<String>> {
  ...
  let get_properties_response = session
    ...
    .await
    .ok()?;
  let get_properties_response: cdp::GetPropertiesResponse = serde_json::from_value(get_properties_response).ok()?;
  Some(get_properties_response.result.into_iter().map(|prop| prop.name).collect())
}

// TODO(bartlomieju): dedup with repl::editor
async fn evaluate_expression(session: &mut repl::ReplSession, expr: &str) -> Option<cdp::EvaluateResponse> {
  let evaluate_response = session
    .post_message_with_event_loop(
      "Runtime.evaluate",
