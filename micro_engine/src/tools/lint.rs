...
pub async fn lint(flags: Flags, lint_flags: LintFlags) -> Result<(), AnyError> {
  if let Some(watch_flags) = &lint_flags.watch {
    if lint_flags.is_stdin() {
      return Err(generic_error("Lint watch on standard input is not supported."));
    }
    file_watcher::watch_func(flags, file_watcher::PrintConfig::new("Lint", !watch_flags.no_clear_screen), move |flags, watcher_communicator, changed_paths| {
      let lint_flags = lint_flags.clone();
      Ok(async move {
        let factory = CliFactory::from_flags(flags).await?;
        let cli_options = factory.cli_options();
        let lint_options = cli_options.resolve_lint_options(lint_flags)?;
        let files = collect_lint_files(&lint_options.files).and_then(|files| if files.is_empty() { Err(generic_error("No target files found.")) } else { Ok(files) })?;
    ...
    let success = if is_stdin {
      ...
      let success = handle_lint_result(STDIN_FILE_NAME, r, reporter_lock.clone());
      reporter_lock.lock().unwrap().close(1);
      success
    } else {
      let target_files = collect_lint_files(files).and_then(|files| if files.is_empty() { Err(generic_error("No target files found.")) } else { Ok(files) })?;
...
async fn lint_files(factory: CliFactory, lint_options: LintOptions, paths: Vec<PathBuf>) -> Result<bool, AnyError> {
...
pub fn print_rules_list(json: bool, maybe_rules_tags: Option<Vec<String>>) {
  let lint_rules = if maybe_rules_tags.is_none() { rules::get_all_rules() } else { rules::get_filtered_rules(maybe_rules_tags, None, None) };
  ...
  if json {
    ...
    for rule in lint_rules.iter() {
      ...
      println!("{}", colors::gray(format!("   help: https://lint.deno.land/#{}", rule.code())));
      println!();
    }
  }
}

pub fn create_linter(media_type: MediaType, rules: Vec<&'static dyn LintRule>) -> Linter {
...
fn lint_stdin(lint_rules: Vec<&'static dyn LintRule>) -> Result<(Vec<LintDiagnostic>, String), AnyError> {
...
fn handle_lint_result(file_path: &str, result: Result<(Vec<LintDiagnostic>, String), AnyError>, reporter_lock: Arc<Mutex<Box<dyn LintReporter + Send>>>) -> bool {
...
impl LintReporter for CompactLintReporter {
  fn visit_diagnostic(&mut self, d: &LintDiagnostic, _source_lines: Vec<&str>) {
    ...
    eprintln!("{}: line {}, col {} - {} ({})", d.filename, d.range.start.line_index + 1, d.range.start.column_index + 1, d.message, d.code)
  }
...
pub fn format_diagnostic(diagnostic_code: &str, message_line: &str, source_lines: &[&str], range: &deno_lint::diagnostic::Range, maybe_hint: Option<&String>, formatted_location: &str) -> String {
  let mut lines = vec![];

  for (i, line) in source_lines.iter().enumerate().take(range.end.line_index + 1).skip(range.start.line_index) {
    lines.push(line.to_string());
    if range.start.line_index == range.end.line_index {
      lines.push(format!("{}{}", " ".repeat(range.start.column_index), colors::red(&"^".repeat(range.end.column_index - range.start.column_index))));
    } else {
      let line_len = line.len();
      if range.start.line_index == i {
        lines.push(format!("{}{}", " ".repeat(range.start.column_index), colors::red(&"^".repeat(line_len - range.start.column_index))));
  ...
  let hint = if let Some(hint) = maybe_hint { format!("    {} {}\n", colors::cyan("hint:"), hint) } else { "".to_string() };
  let help = format!("    {} for further information visit https://lint.deno.land/#{}", colors::cyan("help:"), diagnostic_code);
...
fn get_config_rules_err_empty(rules: LintRulesConfig) -> Result<Vec<&'static dyn LintRule>, AnyError> {
...
pub fn get_configured_rules(rules: LintRulesConfig) -> Vec<&'static dyn LintRule> {
