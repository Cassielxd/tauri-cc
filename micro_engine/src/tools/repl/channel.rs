...
pub fn rustyline_channel() -> (RustylineSyncMessageSender, RustylineSyncMessageHandler) {
  ...
  (
    ...
    RustylineSyncMessageHandler { response_tx, message_rx },
  )
}

pub enum RustylineSyncMessage {
  PostMessage { method: String, params: Option<Value> },
...
impl RustylineSyncMessageSender {
  pub fn post_message<T: serde::Serialize>(&self, method: &str, params: Option<T>) -> Result<Value, AnyError> {
    if let Err(err) = self.message_tx.blocking_send(RustylineSyncMessage::PostMessage {
      method: method.to_string(),
      params: params.map(|params| serde_json::to_value(params)).transpose()?,
    }) {
      Err(anyhow!("{}", err))
    } else {
  ...
  pub fn lsp_completions(&self, line_text: &str, position: usize) -> Vec<ReplCompletionItem> {
    if self.message_tx.blocking_send(RustylineSyncMessage::LspCompletions { line_text: line_text.to_string(), position }).is_err() {
      Vec::new()
    } else {
      match self.response_rx.borrow_mut().blocking_recv().unwrap() {
...
impl RustylineSyncMessageHandler {
  ...
  pub fn send(&self, response: RustylineSyncResponse) -> Result<(), AnyError> {
    self.response_tx.send(response).map_err(|err| anyhow!("{}", err))
  }
}
