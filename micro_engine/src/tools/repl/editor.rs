...
impl EditorHelper {
  pub fn get_global_lexical_scope_names(&self) -> Vec<String> {
    let evaluate_response = self
      .sync_sender
      .post_message("Runtime.globalLexicalScopeNames", Some(cdp::GlobalLexicalScopeNamesArgs { execution_context_id: Some(self.context_id) }))
      .unwrap();
  ...
  pub fn get_expression_property_names(&self, expr: &str) -> Vec<String> {
    ...
    self.get_object_expr_properties(object_expr).unwrap_or_default()
  }
...
impl Completer for EditorHelper {
  ...
  fn complete(&self, line: &str, pos: usize, _ctx: &Context<'_>) -> Result<(usize, Vec<String>), ReadlineError> {
    ...
    if !lsp_completions.is_empty() {
      ...
      return Ok((lsp_completions[0].range.start, lsp_completions.into_iter().map(|c| c.new_text).collect()));
    }
    ...
    if let Some(index) = expr.rfind('.') {
      ...
      let candidates = self
        ...
        .into_iter()
        .filter(|n| !n.starts_with("Symbol(") && n.starts_with(prop_name) && n != &*REPL_INTERNALS_NAME)
        .collect();
...
fn validate(input: &str) -> ValidationResult {
  ...
  let tokens = deno_ast::lex(input, deno_ast::MediaType::TypeScript).into_iter().filter_map(|item| match item.inner {
    ...
    match token {
      ...
      Token::RParen | Token::RBracket | Token::RBrace => {
        match (stack.pop(), token) {
          (Some(Token::LParen), Token::RParen) | (Some(Token::LBracket), Token::RBracket) | (Some(Token::LBrace), Token::RBrace) | (Some(Token::DollarLBrace), Token::RBrace) => {}
          (Some(left), _) => {
            // queue up a validation error to surface once we've finished examining the current line
            queued_validation_error = Some(ValidationResult::Invalid(Some(format!("Mismatched pairs: {left:?} is not properly closed"))));
          }
          (None, _) => {
...
impl Highlighter for EditorHelper {
  ...
  fn highlight<'l>(&self, line: &'l str, _: usize) -> Cow<'l, str> {
    ...
    let mut lexed_items = deno_ast::lex(line, deno_ast::MediaType::TypeScript).into_iter().peekable();
      ...
      out_line.replace_range(
        ...
        &match item.inner {
          deno_ast::TokenOrComment::Token(token) => match token {
            Token::Str { .. } | Token::Template { .. } | Token::BackQuote => colors::green(&line[range]).to_string(),
              ...
              Word::Ident(ident) => {
                match ident.as_ref() {
                  ...
                  _ => {
                    ...
                    } else if ident.as_ref() == "from" && matches!(next, Some(deno_ast::TokenOrComment::Token(Token::Str { .. }))) {
...
impl ReplEditor {
  pub fn new(helper: EditorHelper, history_file_path: Option<PathBuf>) -> Result<Self, AnyError> {
    ...
    editor.bind_sequence(KeyEvent(KeyCode::Char('s'), Modifiers::CTRL), EventHandler::Simple(Cmd::Newline));
    editor.bind_sequence(KeyEvent(KeyCode::Tab, Modifiers::NONE), EventHandler::Conditional(Box::new(TabEventHandler)));
    ...
    if let Some(history_file_path) = &history_file_path {
      ...
      std::fs::create_dir_all(history_file_dir).with_context(|| format!("Unable to create directory for the history file: {}", history_file_dir.display()))?;
    }
...
impl ConditionalEventHandler for TabEventHandler {
  fn handle(&self, evt: &Event, n: RepeatCount, _: bool, ctx: &EventContext) -> Option<Cmd> {
    ...
    if ctx.line().is_empty() || ctx.line()[..ctx.pos()].chars().next_back().filter(|c| c.is_whitespace()).is_some() {
