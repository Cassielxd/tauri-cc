...
async fn read_line_and_poll(repl_session: &mut ReplSession, message_handler: &mut RustylineSyncMessageHandler, editor: ReplEditor) -> Result<String, ReadlineError> {
...
async fn read_eval_file(cli_options: &CliOptions, file_fetcher: &FileFetcher, eval_file: &str) -> Result<String, AnyError> {
...
pub async fn run(flags: Flags, repl_flags: ReplFlags) -> Result<i32, AnyError> {
  ...
  let permissions = PermissionsContainer::new(Permissions::from_options(&cli_options.permissions_options())?);
  let npm_resolver = factory.npm_resolver().await?.clone();
  let resolver = factory.resolver().await?.clone();
  let file_fetcher = factory.file_fetcher()?;
  let worker_factory = factory.create_cli_main_worker_factory().await?;
  let history_file_path = factory.deno_dir().ok().and_then(|dir| dir.repl_history_file_path());
  ...
  let mut worker = worker_factory
    .create_custom_worker(main_module.clone(), permissions, vec![crate::ops::testing::deno_test::init_ops(test_event_sender.clone())], Default::default())
    .await?;
  worker.setup_repl().await?;
  let worker = worker.into_main_worker();
  let mut repl_session = ReplSession::initialize(cli_options, npm_resolver, resolver, worker, main_module, test_event_sender, test_event_receiver).await?;
  ...
  if let Some(eval_files) = repl_flags.eval_files {
    for eval_file in eval_files {
      match read_eval_file(cli_options, file_fetcher, &eval_file).await {
        Ok(eval_source) => {
          let output = repl_session.evaluate_line_and_get_output(&eval_source).await;
          // only output errors
  ...
  if !cli_options.is_quiet() {
    ...
    if repl_flags.is_default_command {
      println!("{}", colors::yellow("REPL is running with all permissions allowed."));
      println!("To specify permissions, run `deno repl` with allow flags.")
    }
  }

  loop {
    let line = read_line_and_poll(&mut repl_session, &mut rustyline_channel.1, editor.clone()).await;
    match line {
      Ok(line) => {
