...
fn comment_source_to_position_range(comment_start: SourcePos, m: &Match, text_info: &SourceTextInfo, is_jsx_import_source: bool) -> PositionRange {
...
impl ReplSession {
  pub async fn initialize(
    ...
    let mut repl_session = ReplSession {
      ...
      test_reporter_factory: Box::new(|| Box::new(PrettyTestReporter::new(false, true, false, true))),
      main_module,
      test_event_sender,
  ...
  pub async fn evaluate_line_and_get_output(&mut self, line: &str) -> EvaluationOutput {
    fn format_diagnostic(diagnostic: &deno_ast::Diagnostic) -> String {
      ...
      format!("{}: {} at {}:{}", colors::red("parse error"), diagnostic.message(), display_position.line_number, display_position.column_number,)
    }
        ...
        Err(err) => {
          ...
          match err.downcast_ref::<deno_ast::Diagnostic>() {
            ...
            None => match err.downcast_ref::<DiagnosticsError>() {
              Some(diagnostics) => Ok(EvaluationOutput::Error(diagnostics.0.iter().map(format_diagnostic).collect::<Vec<_>>().join("\n\n"))),
              None => Err(err),
            },
          }
        }
  ...
  pub async fn evaluate_line_with_object_wrapping(&mut self, line: &str) -> Result<TsEvaluateResponse, AnyError> {
    ...
    let wrapped_line = if line.trim_start().starts_with('{') && !line.trim_end().ends_with(';') { format!("({})", &line) } else { line.to_string() };

    let evaluate_response = self.evaluate_ts_expression(&wrapped_line).await;

    // If that fails, we retry it without wrapping in parens letting the error bubble up to the
    // user if it is still an error.
    let result = if wrapped_line != line && (evaluate_response.is_err() || evaluate_response.as_ref().unwrap().value.exception_details.is_some()) {
    ...
    if worker_has_tests(&mut self.worker) {
      let report_tests_handle = spawn(report_tests(self.test_event_receiver.take().unwrap(), (self.test_reporter_factory)()));
      run_tests_for_worker(&mut self.worker, &self.main_module, &Default::default(), &Default::default()).await.unwrap();
      self.test_event_sender.send(TestEvent::ForceEndReport).unwrap();
  ...
  pub async fn call_function_on_args(&mut self, function_declaration: String, args: &[cdp::RemoteObject]) -> Result<cdp::CallFunctionOnResponse, AnyError> {
    let arguments: Option<Vec<cdp::CallArgument>> = if args.is_empty() { None } else { Some(args.iter().map(|a| a.into()).collect()) };
  ...
  async fn evaluate_ts_expression(&mut self, expression: &str) -> Result<TsEvaluateResponse, AnyError> {
    ...
    self.check_for_npm_or_node_imports(&parsed_source.program()).await?;
    ...
    let value = self.evaluate_expression(&format!("'use strict'; void 0;{transpiled_src}")).await?;

    Ok(TsEvaluateResponse { ts_code: expression.to_string(), value })
  }
  ...
  async fn check_for_npm_or_node_imports(&mut self, program: &swc_ast::Program) -> Result<(), AnyError> {
    ...
    let resolved_imports = collector
      .imports
      .iter()
      .flat_map(|i| self.resolver.resolve(i, &self.referrer, ResolutionMode::Execution).ok().or_else(|| ModuleSpecifier::parse(i).ok()))
      .collect::<Vec<_>>();

    let npm_imports = resolved_imports.iter().flat_map(|url| NpmPackageReqReference::from_specifier(url).ok()).map(|r| r.into_inner().req).collect::<Vec<_>>();
...
fn parse_source_as(source: String, media_type: deno_ast::MediaType) -> Result<deno_ast::ParsedSource, AnyError> {
  let specifier = if media_type == deno_ast::MediaType::Tsx { "repl.tsx" } else { "repl.ts" };
...
static JSX_IMPORT_SOURCE_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)^[\s*]*@jsxImportSource\s+(\S+)").unwrap());
...
fn analyze_jsx_pragmas(parsed_source: &ParsedSource) -> Option<AnalyzedJsxPragmas> {
  if !matches!(parsed_source.media_type(), deno_ast::MediaType::Jsx | deno_ast::MediaType::Tsx) {
    return None;
  }
