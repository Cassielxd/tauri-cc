...
impl HmrRunner {
  pub fn new(emitter: Arc<Emitter>, session: LocalInspectorSession, watcher_communicator: Arc<WatcherCommunicator>) -> Self {
    Self {
      session,
      emitter,
  ...
  pub async fn stop(&mut self) -> Result<(), AnyError> {
    self.watcher_communicator.change_restart_mode(WatcherRestartMode::Automatic);
  ...
  async fn enable_debugger(&mut self) -> Result<(), AnyError> {
    ...
    self.session.post_message::<()>("Runtime.enable", None).await?;
    Ok(())
  }
  ...
  async fn disable_debugger(&mut self) -> Result<(), AnyError> {
    ...
    self.session.post_message::<()>("Runtime.disable", None).await?;
    Ok(())
  }

  async fn set_script_source(&mut self, script_id: &str, source: &str) -> Result<cdp::SetScriptSourceResponse, AnyError> {
    let result = self
      .session
      .post_message(
    ...
    Ok(serde_json::from_value::<cdp::SetScriptSourceResponse>(result)?)
  }

  async fn dispatch_hmr_event(&mut self, script_id: &str) -> Result<(), AnyError> {
    let expr = format!("dispatchEvent(new CustomEvent(\"hmr\", {{ detail: {{ path: \"{}\" }} }}));", script_id);
  ...
  pub async fn run(&mut self) -> Result<(), AnyError> {
    self.watcher_communicator.change_restart_mode(WatcherRestartMode::Manual);
