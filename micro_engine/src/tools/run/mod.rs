...
pub async fn run_script(flags: Flags, run_flags: RunFlags) -> Result<i32, AnyError> {
...
To grant permissions, set them before the script argument. For example:
  ...
  let mut worker = worker_factory.create_main_worker(main_module, permissions).await?;
...
// code properly.
async fn run_with_watch(flags: Flags, watch_flags: WatchFlagsWithPaths) -> Result<i32, AnyError> {
  util::file_watcher::watch_recv(
    flags,
    util::file_watcher::PrintConfig::new_with_banner(if watch_flags.hmr { "HMR" } else { "Watcher" }, "Process", !watch_flags.no_clear_screen),
    ...
    move |flags, watcher_communicator, _changed_paths| {
      Ok(async move {
        ...
        let permissions = PermissionsContainer::new(Permissions::from_options(&cli_options.permissions_options())?);
        let mut worker = factory.create_cli_main_worker_factory().await?.create_main_worker(main_module, permissions).await?;
...
pub async fn eval_command(flags: Flags, eval_flags: EvalFlags) -> Result<i32, AnyError> {
  let factory = CliFactory::from_flags(flags).await?;
  let cli_options = factory.cli_options();
  let file_fetcher = factory.file_fetcher()?;
  let main_module = cli_options.resolve_main_module()?;
  ...
  // Create a dummy source file.
  let source_code = if eval_flags.print { format!("console.log({})", eval_flags.code) } else { eval_flags.code }.into_bytes();
