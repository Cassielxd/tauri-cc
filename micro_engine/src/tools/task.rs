...
pub async fn execute_script(flags: Flags, task_flags: TaskFlags) -> Result<i32, AnyError> {
  let factory = CliFactory::from_flags(flags).await?;
  let cli_options = factory.cli_options();
  let tasks_config = cli_options.resolve_tasks_config()?;
  let maybe_package_json = cli_options.maybe_package_json();
  let package_json_scripts = maybe_package_json.as_ref().and_then(|p| p.scripts.clone()).unwrap_or_default();
  ...
  if let Some(script) = tasks_config.get(task_name) {
    ...
    let seq_list = deno_task_shell::parser::parse(&script).with_context(|| format!("Error parsing script '{task_name}'."))?;
    ...
    if let Some(package_deps) = package_json_deps_provider.deps() {
      for (key, value) in package_deps {
        if let Err(err) = value {
          log::info!("{} Ignoring dependency '{}' in package.json because its version requirement failed to parse: {:#}", colors::yellow("Warning"), key, err,);
        }
      }
    }
    ...
    let cwd = match task_flags.cwd {
      Some(path) => canonicalize_path(&PathBuf::from(path))?,
      None => maybe_package_json.as_ref().unwrap().path.parent().unwrap().to_owned(),
    };
    ...
    let task_names = vec![format!("pre{}", task_name), task_name.clone(), format!("post{}", task_name)];
    for task_name in task_names {
      if let Some(script) = package_json_scripts.get(&task_name) {
        ...
        let npx_commands = match npm_resolver.as_managed() {
          Some(npm_resolver) => resolve_npm_commands(npm_resolver, node_resolver)?,
...
fn output_task(task_name: &str, script: &str) {
  log::info!("{} {} {}", colors::green("Task"), colors::cyan(&task_name), script,);
}
...
fn collect_env_vars_with_node_modules_dir(node_modules_dir_path: &Path) -> HashMap<String, String> {
  ...
  prepend_to_path(&mut env_vars, node_modules_dir_path.join(".bin").to_string_lossy().to_string());
  env_vars
}
...
fn print_available_tasks(
  ...
  for (is_deno, (key, value)) in tasks_config.iter().map(|e| (true, e)).chain(package_json_scripts.iter().filter(|(key, _)| !tasks_config.contains_key(*key)).map(|e| (false, e))) {
    eprintln!("- {}{}", colors::cyan(key), if is_deno { "".to_string() } else { format!(" {}", colors::italic_gray("(package.json)")) });
    eprintln!("    {value}");
    had_task = true;
  }
  if !had_task {
  ...
  fn execute(&self, mut context: ShellCommandContext) -> LocalBoxFuture<'static, ExecuteResult> {
    if let Some(first_arg) = context.args.get(0).cloned() {
      if let Some(command) = context.state.resolve_command(&first_arg) {
        ...
        let _ = context.stderr.write_line(&format!("npx: could not resolve command '{first_arg}'"));
...
fn resolve_npm_commands(npm_resolver: &ManagedCliNpmResolver, node_resolver: &NodeResolver) -> Result<HashMap<String, Rc<dyn ShellCommand>>, AnyError> {
  ...
  for id in snapshot.top_level_packages() {
    ...
    for bin_command in bin_commands {
      result.insert(bin_command.to_string(), Rc::new(NpmPackageBinCommand { name: bin_command, npm_package: id.nv.clone() }) as Rc<dyn ShellCommand>);
    }
  }
