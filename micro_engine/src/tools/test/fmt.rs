...
fn abbreviate_test_error(js_error: &JsError) -> JsError {
  ...
  let should_filter = frames.iter().any(|f| if let Some(file_name) = &f.file_name { !(file_name.starts_with("[ext:") || file_name.starts_with("ext:")) } else { true });

  if should_filter {
    let mut frames = frames
      .into_iter()
      .rev()
      .skip_while(|f| if let Some(file_name) = &f.file_name { file_name.starts_with("[ext:") || file_name.starts_with("ext:") } else { false })
      .collect::<Vec<_>>();
    frames.reverse();
