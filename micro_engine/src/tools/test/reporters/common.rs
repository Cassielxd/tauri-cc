...
pub(super) fn format_test_step_ancestry(desc: &TestStepDescription, tests: &IndexMap<usize, TestDescription>, test_steps: &IndexMap<usize, TestStepDescription>) -> String {
  let root;
...
pub fn format_test_for_summary(cwd: &Url, desc: &TestDescription) -> String {
  format!(
    "{} {}",
    &desc.name,
    colors::gray(format!("=> {}:{}:{}", to_relative_path_or_remote_url(cwd, &desc.location.file_name), desc.location.line_number, desc.location.column_number))
  )
}

pub fn format_test_step_for_summary(cwd: &Url, desc: &TestStepDescription, tests: &IndexMap<usize, TestDescription>, test_steps: &IndexMap<usize, TestStepDescription>) -> String {
...
pub(super) fn report_sigint(writer: &mut dyn std::io::Write, cwd: &Url, tests_pending: &HashSet<usize>, tests: &IndexMap<usize, TestDescription>, test_steps: &IndexMap<usize, TestStepDescription>) {
  ...
  writeln!(writer, "\n{} The following tests were pending:\n", colors::intense_blue("SIGINT")).unwrap();
...
pub(super) fn report_summary(writer: &mut dyn std::io::Write, cwd: &Url, summary: &TestSummary, elapsed: &Duration) {
    ...
    for (origin, (failures, uncaught_error)) in failures_by_origin {
      for (description, failure) in failures {
        if !failure.hide_in_summary() {
          ...
          writeln!(writer, "{}: {}", colors::red_bold("error"), failure.to_string()).unwrap();
          writeln!(writer).unwrap();
          failure_titles.push(failure_title);
        }
      }
      if let Some(js_error) = uncaught_error {
        let failure_title = format!("{} (uncaught error)", to_relative_path_or_remote_url(cwd, &origin));
        writeln!(writer, "{}", &failure_title).unwrap();
        writeln!(writer, "{}: {}", colors::red_bold("error"), format_test_error(js_error)).unwrap();
  ...
  let status = if summary.has_failed() { colors::red("FAILED").to_string() } else { colors::green("ok").to_string() };
  ...
  writeln!(writer, "\n{} | {} {}", status, summary_result, colors::gray(format!("({})", display::human_elapsed(elapsed.as_millis()))),).unwrap();
}
