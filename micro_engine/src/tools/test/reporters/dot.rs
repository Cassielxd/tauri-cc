...
impl DotTestReporter {
  pub fn new() -> DotTestReporter {
    let console_width = if let Some(size) = crate::util::console::console_size() { size.cols as usize } else { 0 };
...
impl TestReporter for DotTestReporter {
  ...
  fn report_uncaught_error(&mut self, origin: &str, error: Box<JsError>) {
    ...
    println!("Uncaught error from {} {}", to_relative_path_or_remote_url(&self.cwd, origin), colors::red("FAILED"));
  }
  ...
  fn report_step_result(&mut self, desc: &TestStepDescription, result: &TestStepResult, _elapsed: u64, tests: &IndexMap<usize, TestDescription>, test_steps: &IndexMap<usize, TestStepDescription>) {
    match &result {
      TestStepResult::Ok => {
