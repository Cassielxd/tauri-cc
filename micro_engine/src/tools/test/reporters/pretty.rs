...
impl PrettyTestReporter {
  pub fn new(parallel: bool, echo_output: bool, filter: bool, repl: bool) -> PrettyTestReporter {
    PrettyTestReporter {
      parallel,
      echo_output,
      in_new_line: true,
  ...
  fn force_report_wait(&mut self, description: &TestDescription) {
    ...
    if self.parallel {
      write!(&mut self.writer, "{}", colors::gray(format!("{} => ", to_relative_path_or_remote_url(&self.cwd, &description.origin)))).unwrap();
    }
  ...
  fn force_report_step_wait(&mut self, description: &TestStepDescription) {
    ...
    write!(&mut self.writer, "{}{} ...", "  ".repeat(description.level), description.name).unwrap();
    self.in_new_line = false;
    // flush for faster feedback when line buffered
    std::io::stdout().flush().unwrap();
    self.scope_test_id = Some(description.id);
  ...
  fn force_report_step_result(&mut self, description: &TestStepDescription, result: &TestStepResult, elapsed: u64) {
    ...
    if !matches!(result, TestStepResult::Failed(TestFailure::Incomplete)) {
      write!(&mut self.writer, " {}", colors::gray(format!("({})", display::human_elapsed(elapsed.into())))).unwrap();
    }
    writeln!(&mut self.writer).unwrap();
    ...
    self.child_results_buffer.entry(description.parent_id).or_default().remove(&description.id);
  }

  fn write_output_end(&mut self) {
    if self.did_have_user_output {
      writeln!(&mut self.writer, "{}", colors::gray("----- output end -----")).unwrap();
      self.in_new_line = true;
...
impl TestReporter for PrettyTestReporter {
  ...
  fn report_plan(&mut self, plan: &TestPlan) {
    ...
    writeln!(
      &mut self.writer,
      "{}",
      colors::gray(format!("running {} {} from {}", plan.total, inflection, to_relative_path_or_remote_url(&self.cwd, &plan.origin)))
    )
    .unwrap();
    self.in_new_line = true;
  }
  ...
  fn report_output(&mut self, output: &[u8]) {
    ...
    if !self.did_have_user_output && self.started_tests {
      ...
      writeln!(&mut self.writer, "{}", colors::gray("------- output -------")).unwrap();
      self.in_new_line = true;
    }
  ...
  fn report_result(&mut self, description: &TestDescription, result: &TestResult, elapsed: u64) {
    match &result {
      ...
      TestResult::Failed(failure) => {
        ...
        self.summary.failures.push((description.clone(), failure.clone()));
      }
    ...
    writeln!(&mut self.writer, " {}", colors::gray(format!("({})", display::human_elapsed(elapsed.into())))).unwrap();
    self.in_new_line = true;
    self.scope_test_id = None;
  }
  ...
  fn report_uncaught_error(&mut self, origin: &str, error: Box<JsError>) {
    self.summary.failed += 1;
    self.summary.uncaught_errors.push((origin.to_string(), error));

    if !self.in_new_line {
      writeln!(&mut self.writer).unwrap();
    }
    writeln!(&mut self.writer, "Uncaught error from {} {}", to_relative_path_or_remote_url(&self.cwd, origin), colors::red("FAILED")).unwrap();
    self.in_new_line = true;
    self.did_have_user_output = false;
  }
  ...
  fn report_step_result(&mut self, desc: &TestStepDescription, result: &TestStepResult, elapsed: u64, tests: &IndexMap<usize, TestDescription>, test_steps: &IndexMap<usize, TestStepDescription>) {
    match &result {
      TestStepResult::Ok => {
    ...
    if self.parallel {
      ...
      write!(
        &mut self.writer,
        "{} {} ...",
        colors::gray(format!("{} =>", to_relative_path_or_remote_url(&self.cwd, &desc.origin))),
  ...
  fn report_summary(&mut self, elapsed: &Duration, _tests: &IndexMap<usize, TestDescription>, _test_steps: &IndexMap<usize, TestStepDescription>) {
  ...
  fn report_sigint(&mut self, tests_pending: &HashSet<usize>, tests: &IndexMap<usize, TestDescription>, test_steps: &IndexMap<usize, TestStepDescription>) {
    common::report_sigint(&mut self.writer, &self.cwd, tests_pending, tests, test_steps);
    self.in_new_line = true;
  }

  fn flush_report(&mut self, _elapsed: &Duration, _tests: &IndexMap<usize, TestDescription>, _test_steps: &IndexMap<usize, TestStepDescription>) -> anyhow::Result<()> {
