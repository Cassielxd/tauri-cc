...
impl RealVersionProvider {
  pub fn new(http_client: Arc<HttpClient>, check_kind: UpgradeCheckKind) -> Self {
    Self { http_client, check_kind }
  }
}
...
impl<TEnvironment: UpdateCheckerEnvironment, TVersionProvider: VersionProvider> UpdateChecker<TEnvironment, TVersionProvider> {
  pub fn new(env: TEnvironment, version_provider: TVersionProvider) -> Self {
    ...
    Self { env, version_provider, maybe_file }
  }

  pub fn should_check_for_new_version(&self) -> bool {
    match &self.maybe_file {
      Some(file) => {
        let last_check_age = self.env.current_time().signed_duration_since(file.last_checked);
  ...
  pub fn store_prompted(self) {
    if let Some(file) = self.maybe_file {
      self.env.write_check_file(&file.with_last_prompt(self.env.current_time()).serialize());
    }
  }
}
...
fn print_release_notes(current_version: &str, new_version: &str) {
  if get_minor_version(current_version) != get_minor_version(new_version) {
    log::info!("{}{}", "Release notes: https://github.com/denoland/deno/releases/tag/v", &new_version,);
    log::info!("{}{}", "Blog post: https://deno.com/blog/v", get_minor_version(new_version));
  }
}

pub fn upgrade_check_enabled() -> bool {
  matches!(env::var("DENO_NO_UPDATE_CHECK"), Err(env::VarError::NotPresent))
}
  ...
  if let Some(upgrade_version) = update_checker.should_prompt() {
    if log::log_enabled!(log::Level::Info) && std::io::stderr().is_terminal() {
      if version::is_canary() {
        eprint!("{} ", colors::green("A new canary release of Deno is available."));
        eprintln!("{}", colors::italic_gray("Run `deno upgrade --canary` to install it."));
      } else {
        eprint!("{} {} → {} ", colors::green("A new release of Deno is available:"), colors::cyan(version::deno()), colors::cyan(&upgrade_version));
        eprintln!("{}", colors::italic_gray("Run `deno upgrade` to install it."));
      }
...
pub async fn check_for_upgrades_for_lsp(http_client: Arc<HttpClient>) -> Result<Option<LspVersionUpgradeInfo>, AnyError> {
  if !upgrade_check_enabled() {
    return Ok(None);
  }
...
async fn check_for_upgrades_for_lsp_with_provider(version_provider: &impl VersionProvider) -> Result<Option<LspVersionUpgradeInfo>, AnyError> {
  ...
  if current_version == latest_version {
    ...
    Ok(Some(LspVersionUpgradeInfo { latest_version, is_canary: true }))
  } else {
    if let Ok(current) = Version::parse_standard(&current_version) {
    ...
    Ok(Some(LspVersionUpgradeInfo { latest_version, is_canary: false }))
  }
}

async fn fetch_and_store_latest_version<TEnvironment: UpdateCheckerEnvironment, TVersionProvider: VersionProvider>(env: &TEnvironment, version_provider: &TVersionProvider) {
  ...
  env.write_check_file(
    &CheckVersionFile {
      ...
      last_prompt: env.current_time().sub(chrono::Duration::hours(UPGRADE_CHECK_INTERVAL + 1)),
...
pub async fn upgrade(flags: Flags, upgrade_flags: UpgradeFlags) -> Result<(), AnyError> {
  ...
  let permissions = if let Ok(metadata) = fs::metadata(output_exe_path) {
    ...
    if permissions.readonly() {
      bail!("You do not have write permission to {}", output_exe_path.display());
    }
    #[cfg(unix)]
  ...
  let install_version = match upgrade_flags.version {
    Some(passed_version) => {
      ...
      let passed_version = passed_version.strip_prefix('v').unwrap_or(&passed_version).to_string();
    ...
    None => {
      ...
      let latest_version = get_latest_version(client, release_kind, UpgradeCheckKind::Execution).await?;
      ...
      if !upgrade_flags.force && upgrade_flags.output.is_none() && current_is_most_recent {
        log::info!("Local deno version {} is the most recent release", if upgrade_flags.canary { crate::version::GIT_COMMIT_HASH } else { crate::version::deno() });
        return Ok(());
      } else {
  ...
  let download_url = if upgrade_flags.canary {
    ...
    format!("https://dl.deno.land/canary/{}/{}", install_version, *ARCHIVE_NAME)
  } else {
    format!("{}/download/v{}/{}", RELEASE_URL, install_version, *ARCHIVE_NAME)
  };

  let archive_data = download_package(client, &download_url).await.with_context(|| format!("Failed downloading {download_url}"))?;
  ...
  if upgrade_flags.dry_run {
    ...
    let output_result = if *output_exe_path == current_exe_path {
      ...
      fs::rename(&new_exe_path, output_exe_path).or_else(|_| fs::copy(&new_exe_path, output_exe_path).map(|_| ()))
    };
...
async fn get_latest_version(client: &HttpClient, release_kind: UpgradeReleaseKind, check_kind: UpgradeCheckKind) -> Result<String, AnyError> {
...
fn normalize_version_from_server(release_kind: UpgradeReleaseKind, text: &str) -> String {
  let text = text.trim();
  match release_kind {
...
fn get_url(release_kind: UpgradeReleaseKind, check_kind: UpgradeCheckKind) -> String {
...
async fn download_package(client: &HttpClient, download_url: &str) -> Result<Vec<u8>, AnyError> {
  ...
  let maybe_bytes = {
    ...
    client.download_with_progress(download_url, &progress).await?
  };
  match maybe_bytes {
    Some(bytes) => Ok(bytes),
    None => {
...
pub fn unpack_into_dir(archive_data: Vec<u8>, is_windows: bool, temp_dir: &tempfile::TempDir) -> Result<PathBuf, AnyError> {
  ...
  let unpack_status = match archive_ext {
      ...
      Command::new("tar.exe")
        ...
        .map_err(|err| {
          if err.kind() == std::io::ErrorKind::NotFound {
            std::io::Error::new(std::io::ErrorKind::NotFound, "`tar.exe` was not found in your PATH")
          } else {
            err
          }
        })?
    ...
    "zip" => {
      ...
      Command::new("unzip")
        ...
        .map_err(|err| {
          if err.kind() == std::io::ErrorKind::NotFound {
            std::io::Error::new(std::io::ErrorKind::NotFound, "`unzip` was not found in your PATH, please install `unzip`")
          } else {
            err
          }
        })?
...
impl CheckVersionFile {
  pub fn parse(content: String) -> Option<Self> {
    ...
    let last_prompt = chrono::DateTime::parse_from_rfc3339(split_content[0]).map(|dt| dt.with_timezone(&chrono::Utc)).ok()?;
    let last_checked = chrono::DateTime::parse_from_rfc3339(split_content[1]).map(|dt| dt.with_timezone(&chrono::Utc)).ok()?;
  ...
  fn serialize(&self) -> String {
    format!("{}!{}!{}!{}", self.last_prompt.to_rfc3339(), self.last_checked.to_rfc3339(), self.latest_version, self.current_version,)
  }
