...
pub struct BuildInput<'a, TBuildGraphFn: FnOnce(Vec<ModuleSpecifier>) -> BuildGraphFuture, TEnvironment: VendorEnvironment> {
...
pub async fn build<TBuildGraphFn: FnOnce(Vec<ModuleSpecifier>) -> BuildGraphFuture, TEnvironment: VendorEnvironment>(input: BuildInput<'_, TBuildGraphFn, TEnvironment>) -> Result<BuildOutput, AnyError> {
  let BuildInput {
    mut entry_points,
    build_graph,
    parsed_source_cache,
  ...
  if let Some(jsx_import_source) = jsx_import_source {
    if let Some(specifier_text) = jsx_import_source.maybe_specifier_text() {
      if let Ok(specifier) = resolver.resolve(&specifier_text, &jsx_import_source.base_url, ResolutionMode::Execution) {
  ...
  let remote_modules = all_modules.iter().filter(|m| is_remote_specifier(m.specifier())).copied().collect::<Vec<_>>();
  ...
  Ok(BuildOutput { vendored_count: remote_modules.len(), graph })
}
...
fn validate_original_import_map(import_map: &ImportMap, output_dir: &ModuleSpecifier) -> Result<(), AnyError> {
  ...
  for scope in import_map.scopes() {
    if scope.key.starts_with(output_dir.as_str()) {
      bail!("Providing an existing import map with a scope for the output directory is not supported (\"{}\").", scope.raw_key,);
    }
...
fn build_proxy_module_source(module: &EsmModule, proxied_module: &ProxiedModule, parsed_source_cache: &ParsedSourceCache) -> Result<String, AnyError> {
