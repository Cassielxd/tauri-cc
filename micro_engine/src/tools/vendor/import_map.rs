...
impl<'a> ImportMapBuilder<'a> {
  ...
  pub fn scope(&mut self, base_specifier: &ModuleSpecifier) -> &mut ImportsBuilder<'a> {
    self
      .scopes
      .entry(self.mappings.relative_specifier_text(self.base_dir, base_specifier))
      .or_insert_with(|| ImportsBuilder::new(self.base_dir, self.mappings))
  }

  pub fn into_import_map(self, original_import_map: Option<&ImportMap>) -> ImportMap {
    fn get_local_imports(new_relative_path: &str, original_imports: &SpecifierMap) -> Vec<(String, String)> {
      ...
      for entry in original_imports.entries() {
        if let Some(raw_value) = entry.raw_value {
          if raw_value.starts_with("./") || raw_value.starts_with("../") {
            ...
            result.push((entry.raw_key.to_string(), format!("{}{}", new_relative_path, &raw_value[sub_index..])));
          }
        }
      }
      result
    ...
    fn add_local_imports<'a>(new_relative_path: &str, original_imports: &SpecifierMap, get_new_imports: impl FnOnce() -> &'a mut SpecifierMap) {
    ...
    if let Some(original_im) = original_import_map {
      let original_base_dir = ModuleSpecifier::from_directory_path(original_im.base_url().to_file_path().unwrap().parent().unwrap()).unwrap();
      let new_relative_path = self.mappings.relative_specifier_text(self.base_dir, &original_base_dir);
      // add the imports
      add_local_imports(&new_relative_path, original_im.imports(), || import_map.imports_mut());
      ...
      for scope in original_im.scopes() {
        if scope.raw_key.starts_with("./") || scope.raw_key.starts_with("../") {
          ...
          add_local_imports(&new_relative_path, scope.imports, || import_map.get_or_append_scope_mut(&new_key).unwrap());
        }
      }
    }
...
impl<'a> ImportsBuilder<'a> {
  ...
  pub fn add(&mut self, key: String, specifier: &ModuleSpecifier) {
    let value = self.mappings.relative_specifier_text(self.base_dir, specifier);
...
pub fn build_import_map(input: BuildImportMapInput<'_>) -> Result<String, AnyError> {
  ...
  for base_specifier in mappings.base_specifiers() {
    builder.imports.add(base_specifier.to_string(), base_specifier);
  }
  ...
  if let Some(jsx_import_source) = jsx_import_source {
    if let Some(specifier_text) = jsx_import_source.maybe_specifier_text() {
      if let Ok(resolved_url) = resolver.resolve(&specifier_text, &jsx_import_source.base_url, ResolutionMode::Execution) {
...
fn visit_modules(graph: &ModuleGraph, modules: &[&Module], mappings: &Mappings, import_map: &mut ImportMapBuilder, parsed_source_cache: &ParsedSourceCache) -> Result<(), AnyError> {
  for module in modules {
    ...
    for dep in module.dependencies.values() {
      visit_resolution(&dep.maybe_code, graph, import_map, &module.specifier, mappings, &text_info, source_text);
      visit_resolution(&dep.maybe_type, graph, import_map, &module.specifier, mappings, &text_info, source_text);
    }

    if let Some(types_dep) = &module.maybe_types_dependency {
      visit_resolution(&types_dep.dependency, graph, import_map, &module.specifier, mappings, &text_info, source_text);
    }
  }
...
fn visit_resolution(resolution: &Resolution, graph: &ModuleGraph, import_map: &mut ImportMapBuilder, referrer: &ModuleSpecifier, mappings: &Mappings, text_info: &SourceTextInfo, source_text: &str) {
  if let Some(resolved) = resolution.ok() {
    ...
    if !text.is_empty() {
      handle_dep_specifier(text, &resolved.specifier, graph, import_map, referrer, mappings);
    }
  }
}

fn handle_dep_specifier(text: &str, unresolved_specifier: &ModuleSpecifier, graph: &ModuleGraph, import_map: &mut ImportMapBuilder, referrer: &ModuleSpecifier, mappings: &Mappings) {
  ...
  if is_remote_specifier(&specifier) {
    handle_remote_dep_specifier(text, unresolved_specifier, &specifier, import_map, referrer, mappings)
  } else if specifier.scheme() == "file" {
    handle_local_dep_specifier(text, unresolved_specifier, &specifier, import_map, referrer, mappings);
  }
}

fn handle_remote_dep_specifier(text: &str, unresolved_specifier: &ModuleSpecifier, specifier: &ModuleSpecifier, import_map: &mut ImportMapBuilder, referrer: &ModuleSpecifier, mappings: &Mappings) {
  if is_remote_specifier_text(text) {
    ...
    if text.starts_with("./") || text.starts_with("../") {
      ...
      local_base_specifier = local_base_specifier
        ...
        .unwrap_or_else(|_| panic!("Error joining {} to {}", unresolved_specifier.path(), local_base_specifier));
      local_base_specifier.set_query(unresolved_specifier.query());

      imports.add(mappings.relative_specifier_text(&base_dir, &local_base_specifier), specifier);
      ...
      imports.add(
        {
          ...
          let mut value = ModuleSpecifier::from_directory_path(local_path.parent().unwrap()).unwrap();
          value.set_query(specifier.query());
          value.set_path(&format!("{}{}", value.path(), specifier.path_segments().unwrap().last().unwrap(),));
...
fn handle_local_dep_specifier(text: &str, unresolved_specifier: &ModuleSpecifier, specifier: &ModuleSpecifier, import_map: &mut ImportMapBuilder, referrer: &ModuleSpecifier, mappings: &Mappings) {
  if !is_remote_specifier(referrer) {
  ...
  if text.starts_with("./") || text.starts_with("../") {
    ...
    let mut specifier_local_uri = referrer_local_uri.join(text).unwrap_or_else(|_| panic!("Error joining {} to {}", unresolved_specifier.path(), referrer_local_uri));
    specifier_local_uri.set_query(unresolved_specifier.query());

    imports.add(mappings.relative_specifier_text(&base_dir, &specifier_local_uri), specifier);
  } else {
    imports.add(text.to_string(), specifier);
  }
}

fn text_from_range<'a>(text_info: &SourceTextInfo, text: &'a str, range: &Range) -> &'a str {
...
fn byte_range(text_info: &SourceTextInfo, range: &Range) -> std::ops::Range<usize> {
