...
impl Mappings {
  pub fn from_remote_modules(graph: &ModuleGraph, remote_modules: &[&Module], output_dir: &Path) -> Result<Self, AnyError> {
    ...
    for (root, specifiers) in partitioned_specifiers.into_iter() {
      ...
      for specifier in specifiers {
        ...
        let new_path = path_with_extension(&base_dir.join(if cfg!(windows) { sub_path.replace('/', "\\") } else { sub_path }), &media_type.as_ts_extension()[1..]);
    ...
    for module in remote_modules {
      if let Some(module) = module.esm() {
        if let Some(resolved) = &module.maybe_types_dependency.as_ref().and_then(|d| d.dependency.ok()) {
          let range = &resolved.range;
          // hack to tell if it's an x-typescript-types header
          let is_ts_types_header = range.start == Position::zeroed() && range.end == Position::zeroed();
          if is_ts_types_header {
            let module_path = mappings.get(&module.specifier).unwrap();
            let proxied_path = get_unique_path(path_with_stem_suffix(module_path, ".proxied"), &mut mapped_paths);
            proxies.insert(
    ...
    Ok(Self { mappings, base_specifiers, proxies })
  }
  ...
  pub fn local_path(&self, specifier: &ModuleSpecifier) -> PathBuf {
    if specifier.scheme() == "file" {
      ...
      self.mappings.get(specifier).unwrap_or_else(|| panic!("Could not find local path for {specifier}")).to_path_buf()
    }
  }

  pub fn relative_specifier_text(&self, from: &ModuleSpecifier, to: &ModuleSpecifier) -> String {
  ...
  pub fn base_specifier(&self, child_specifier: &ModuleSpecifier) -> &ModuleSpecifier {
    self
      .base_specifiers
      .iter()
  ...
  pub fn proxied_modules(&self) -> std::collections::hash_map::Iter<'_, ModuleSpecifier, ProxiedModule> {
    self.proxies.iter()
  }
}
  ...
  if let Some(file_stem) = path.file_stem().map(|f| f.to_string_lossy()) {
    if let Some(old_ext) = path.extension().map(|f| f.to_string_lossy()) {
      if file_stem.to_lowercase().ends_with(".d") {
        ...
        return path.with_file_name(format!("{}.{}", &file_stem[..file_stem.len() - ".d".len()], new_ext));
      }
      ...
      if media_type == MediaType::Unknown {
        return path.with_file_name(format!("{}.{}", path.file_name().unwrap().to_string_lossy(), new_ext));
      }
    }
  }
