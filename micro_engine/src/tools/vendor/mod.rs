...
pub async fn vendor(flags: Flags, vendor_flags: VendorFlags) -> Result<(), AnyError> {
  ...
  let output = build::build(build::BuildInput {
    entry_points,
    build_graph: move |entry_points| async move { module_graph_builder.create_graph(GraphKind::All, entry_points).await }.boxed_local(),
  ...
  let try_add_node_modules_dir = npm_package_count > 0 && cli_options.node_modules_dir_enablement().unwrap_or(true);

  log::info!(concat!("Vendored {} {} into {} directory.",), vendored_count, if vendored_count == 1 { "module" } else { "modules" }, raw_output_dir.display(),);

  let try_add_import_map = vendored_count > 0;
  let modified_result = maybe_update_config_file(&output_dir, cli_options, try_add_import_map, try_add_node_modules_dir);
  ...
  if modified_result.added_node_modules_dir {
    ...
    log::info!(
      ...
      npm_package_count,
      if npm_package_count == 1 { "package" } else { "packages" },
    );
  }
...
fn validate_output_dir(output_dir: &Path, flags: &VendorFlags) -> Result<(), AnyError> {
...
fn validate_options(options: &mut CliOptions, output_dir: &Path) -> Result<(), AnyError> {
  // check the import map
  if let Some(import_map_path) = options.resolve_import_map_specifier()?.and_then(|p| specifier_to_file_path(&p).ok()).and_then(|p| canonicalize_path(&p).ok()) {
    // make the output directory in order to canonicalize it for the check below
    std::fs::create_dir_all(output_dir)?;
    let output_dir = canonicalize_path(output_dir).with_context(|| format!("Failed to canonicalize: {}", output_dir.display()))?;
    ...
    if import_map_path.starts_with(output_dir) {
      ...
      log::warn!(
        ...
        import_map_path.strip_prefix(&cwd).unwrap_or(&import_map_path).display().to_string(),
      );
...
fn maybe_update_config_file(output_dir: &Path, options: &CliOptions, try_add_import_map: bool, try_add_node_modules_dir: bool) -> ModifiedResult {
  ...
  let fmt_config = config_file.to_fmt_config().ok().unwrap_or_default().unwrap_or_default();
  let result = update_config_file(
    config_file,
    &fmt_config.options,
    if try_add_import_map { Some(ModuleSpecifier::from_file_path(output_dir.join("import_map.json")).unwrap()) } else { None },
...
fn update_config_file(config_file: &ConfigFile, fmt_options: &FmtOptionsConfig, import_map_specifier: Option<ModuleSpecifier>, try_add_node_modules_dir: bool) -> Result<ModifiedResult, AnyError> {
  let config_path = specifier_to_file_path(&config_file.specifier)?;
  let config_text = std::fs::read_to_string(&config_path)?;
  let import_map_specifier = import_map_specifier.and_then(|import_map_specifier| relative_specifier(&config_file.specifier, &import_map_specifier));
  let modified_result = update_config_text(&config_text, fmt_options, import_map_specifier.as_deref(), try_add_node_modules_dir)?;
...
fn update_config_text(text: &str, fmt_options: &FmtOptionsConfig, import_map_specifier: Option<&str>, try_add_node_modules_dir: bool) -> Result<ModifiedResult, AnyError> {
  ...
  if let Some(import_map_specifier) = import_map_specifier {
    ...
    match obj.get("importMap") {
      Some(ObjectProp { value: Value::StringLit(lit), .. }) => {
        text_changes.push(TextChange {
  ...
  modified_result.new_text = if should_format {
    format_json(&PathBuf::from("deno.json"), &new_text, fmt_options).ok().map(|formatted_text| formatted_text.unwrap_or(new_text))
  } else {
    Some(new_text)
  };
  Ok(modified_result)
