...
pub fn partition_by_root_specifiers<'a>(specifiers: impl Iterator<Item = &'a ModuleSpecifier>) -> BTreeMap<ModuleSpecifier, Vec<ModuleSpecifier>> {
...
pub fn make_url_relative(root: &ModuleSpecifier, url: &ModuleSpecifier) -> Result<String, AnyError> {
  root.make_relative(url).ok_or_else(|| anyhow!("Error making url ({}) relative to root: {}", url.to_string(), root.to_string()))
}
