...
impl TestLoader {
  ...
  pub fn add_with_headers(&mut self, specifier: impl AsRef<str>, text: impl AsRef<str>, headers: &[(&str, &str)]) -> &mut Self {
    ...
    self.files.insert(ModuleSpecifier::parse(specifier.as_ref()).unwrap(), Ok((text.as_ref().to_string(), Some(headers))));
    self
  }

  pub fn add_redirect(&mut self, from: impl AsRef<str>, to: impl AsRef<str>) -> &mut Self {
    self.redirects.insert(ModuleSpecifier::parse(from.as_ref()).unwrap(), ModuleSpecifier::parse(to.as_ref()).unwrap());
    self
  }
}

impl Loader for TestLoader {
  fn load(&mut self, specifier: &ModuleSpecifier, _is_dynamic: bool, _cache_setting: deno_graph::source::CacheSetting) -> LoadFuture {
...
impl VendorTestBuilder {
  ...
  pub fn set_jsx_import_source_config(&mut self, jsx_import_source_config: JsxImportSourceConfig) -> &mut Self {
  ...
  pub async fn build(&mut self) -> Result<VendorOutput, AnyError> {
    ...
    let resolver = Arc::new(build_resolver(self.jsx_import_source_config.clone(), self.original_import_map.clone()));
      ...
      build_graph: {
        ...
        move |entry_points| {
          async move {
            ...
            Ok(build_test_graph(entry_points, loader, resolver.as_graph_resolver(), &analyzer).await)
          }
          .boxed_local()
        }
      },
    ...
    let mut files = files.iter().map(|(path, text)| (path_to_string(path), text.to_string())).collect::<Vec<_>>();
...
fn build_resolver(maybe_jsx_import_source_config: Option<JsxImportSourceConfig>, original_import_map: Option<ImportMap>) -> CliGraphResolver {
...
async fn build_test_graph(roots: Vec<ModuleSpecifier>, mut loader: TestLoader, resolver: &dyn deno_graph::source::Resolver, analyzer: &dyn deno_graph::ModuleAnalyzer) -> ModuleGraph {
