...
pub static COMPILER_SNAPSHOT: Lazy<Box<[u8]>> = Lazy::new(
  ...
  || {
    ...
    zstd::bulk::decompress(&COMPRESSED_COMPILER_SNAPSHOT[4..], u32::from_le_bytes(COMPRESSED_COMPILER_SNAPSHOT[0..4].try_into().unwrap()) as usize)
      .unwrap()
      .into_boxed_slice()
  },
);
...
pub fn get_types_declaration_file_text(unstable: bool) -> String {
  let mut assets = get_asset_texts_from_new_runtime().unwrap().into_iter().map(|a| (a.specifier, a.text)).collect::<HashMap<_, _>>();
...
fn get_hash(source: &str, hash_data: u64) -> String {
  FastInsecureHasher::new().write_str(source).write_u64(hash_data).finish().to_string()
}
...
#[op2]
#[serde]
fn op_load(state: &mut OpState, #[string] load_specifier: &str) -> Result<LoadResponse, AnyError> {
  let state = state.borrow_mut::<State>();
  ...
  // handle the request for that module here.
    ...
    } else if state.maybe_npm.as_ref().map(|npm| npm.node_resolver.in_npm_package(specifier)).unwrap_or(false) {
      media_type = MediaType::from_specifier(specifier);
      let file_path = specifier.to_file_path().unwrap();
      let code = std::fs::read_to_string(&file_path).with_context(|| format!("Unable to load {}", file_path.display()))?;
      Some(Cow::Owned(code))
...
#[op2]
#[serde]
fn op_resolve(state: &mut OpState, #[serde] args: ResolveArgs) -> Result<Vec<(String, String)>, AnyError> {
  let state = state.borrow_mut::<State>();
  ...
  for specifier in args.specifiers {
    ...
    let graph = &state.graph;
    let resolved_dep = graph.get(&referrer).and_then(|m| m.esm()).and_then(|m| m.dependencies.get(&specifier)).and_then(|d| d.maybe_type.ok().or_else(|| d.maybe_code.ok()));

    let maybe_result = match resolved_dep {
      Some(ResolutionResolved { specifier, .. }) => resolve_graph_specifier_types(specifier, &referrer, state)?,
...
fn resolve_graph_specifier_types(specifier: &ModuleSpecifier, referrer: &ModuleSpecifier, state: &State) -> Result<Option<(ModuleSpecifier, MediaType)>, AnyError> {
  let graph = &state.graph;
  ...
  match maybe_module {
    ...
    Some(Module::Npm(module)) => {
      if let Some(npm) = &state.maybe_npm.as_ref() {
        ...
        let maybe_resolution = npm
          .node_resolver
          .resolve_package_subpath_from_deno_module(&package_folder, module.nv_reference.sub_path(), referrer, NodeResolutionMode::Types, &PermissionsContainer::allow_all())?;
    ...
    Some(Module::External(module)) => {
      ...
      Ok(state.maybe_npm.as_ref().map(|npm| {
        let specifier = node::resolve_specifier_into_node_modules(&module.specifier);
        NodeResolution::into_specifier_and_media_type(npm.node_resolver.url_to_node_resolution(specifier).ok())
      }))
    }
...
fn resolve_non_graph_specifier_types(specifier: &str, referrer: &ModuleSpecifier, state: &State) -> Result<Option<(ModuleSpecifier, MediaType)>, AnyError> {
  ...
  if node_resolver.in_npm_package(referrer) {
    ...
    Ok(Some(NodeResolution::into_specifier_and_media_type(
      node_resolver.resolve(specifier, referrer, NodeResolutionMode::Types, &PermissionsContainer::allow_all()).ok().flatten(),
    )))
    ...
    let package_folder = npm.npm_resolver.resolve_pkg_folder_from_deno_module_req(npm_req_ref.req(), referrer)?;
    let maybe_resolution = node_resolver.resolve_package_subpath_from_deno_module(&package_folder, npm_req_ref.sub_path(), referrer, NodeResolutionMode::Types, &PermissionsContainer::allow_all())?;
    Ok(Some(NodeResolution::into_specifier_and_media_type(maybe_resolution)))
  } else {
    Ok(None)
  }
...
fn op_is_node_file(state: &mut OpState, #[string] path: &str) -> bool {
  ...
  match ModuleSpecifier::parse(path) {
    Ok(specifier) => state.maybe_npm.as_ref().map(|n| n.node_resolver.in_npm_package(&specifier)).unwrap_or(false),
    Err(_) => false,
  }
}
...
pub fn exec(request: Request) -> Result<Response, AnyError> {
  ...
  runtime.execute_script(located_script_name!(), startup_source).context("Could not properly start the compiler runtime.")?;
  ...
  if let Some(response) = state.maybe_response {
    ...
    Ok(Response { diagnostics, maybe_tsbuildinfo, stats })
  } else {
