...
impl DiffBuilder {
  pub fn build(orig_text: &str, edit_text: &str) -> String {
    let mut diff_builder = DiffBuilder {
      ...
      has_changes: false,
      line_number_width: {
        let line_count = std::cmp::max(orig_text.split('\n').count(), edit_text.split('\n').count());
  ...
  fn write_line_diff(&mut self) {
    ...
    for (i, s) in split {
      write!(self.output, "{:width$}{} ", self.orig_line + i, colors::gray(" |"), width = self.line_number_width).unwrap();
    ...
    for (i, s) in split {
      write!(self.output, "{:width$}{} ", self.edit_line + i, colors::gray(" |"), width = self.line_number_width).unwrap();
