...
static IS_TTY_WITH_CONSOLE_SIZE: Lazy<bool> = Lazy::new(|| std::io::stderr().is_terminal() && console_size().map(|s| s.cols > 0 && s.rows > 0).unwrap_or(false));
...
impl DrawThread {
  ...
  fn maybe_start_draw_thread(internal_state: &mut InternalState) {
    if internal_state.has_draw_thread || internal_state.hide || internal_state.entries.is_empty() || !DrawThread::is_supported() {
      return;
    }
