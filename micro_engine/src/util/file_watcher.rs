...
impl DebouncedReceiver {
  fn new_with_sender() -> (Arc<mpsc::UnboundedSender<Vec<PathBuf>>>, Self) {
    ...
    (Arc::new(sender), Self { receiver, received_items: HashSet::new() })
  }

  async fn recv(&mut self) -> Option<Vec<PathBuf>> {
    if self.received_items.is_empty() {
      self.received_items.extend(self.receiver.recv().await?.into_iter());
    }
...
fn create_print_after_restart_fn(banner: &'static str, clear_screen: bool) -> impl Fn() {
  move || {
    ...
    info!("{} File change detected! Restarting!", colors::intense_blue(banner),);
  }
}
...
pub async fn watch_func<O, F>(flags: Flags, print_config: PrintConfig, operation: O) -> Result<(), AnyError>
where
  O: FnMut(Flags, Arc<WatcherCommunicator>, Option<Vec<PathBuf>>) -> Result<F, AnyError>,
  F: Future<Output = Result<(), AnyError>>,
{
  let fut = watch_recv(flags, print_config, WatcherRestartMode::Automatic, operation).boxed_local();
...
/// changes. For example, in the case where we would like to bundle, then `operation` would
/// have the logic for it like bundling the code.
pub async fn watch_recv<O, F>(mut flags: Flags, print_config: PrintConfig, restart_mode: WatcherRestartMode, mut operation: O) -> Result<(), AnyError>
where
  O: FnMut(Flags, Arc<WatcherCommunicator>, Option<Vec<PathBuf>>) -> Result<F, AnyError>,
  F: Future<Output = Result<(), AnyError>>,
{
  ...
  let PrintConfig { banner, job_name, clear_screen } = print_config;
  ...
  loop {
    ...
    let operation_future = error_handler(operation(flags.clone(), watcher_communicator.clone(), changed_paths.borrow_mut().take())?);
...
fn new_watcher(sender: Arc<mpsc::UnboundedSender<Vec<PathBuf>>>) -> Result<RecommendedWatcher, AnyError> {
  Ok(Watcher::new(
    move |res: Result<NotifyEvent, NotifyError>| {
      ...
      let paths = event.paths.iter().filter_map(|path| canonicalize_path(path).ok()).collect();
...
fn consume_paths_to_watch(watcher: &mut RecommendedWatcher, receiver: &mut UnboundedReceiver<Vec<PathBuf>>) {
  loop {
