...
impl ImportMapUnfurler {
  ...
  pub fn unfurl(&self, url: &ModuleSpecifier, data: Vec<u8>) -> Result<Vec<u8>, AnyError> {
    let media_type = MediaType::from_specifier(url);

    match media_type {
      MediaType::JavaScript | MediaType::Jsx | MediaType::Mjs | MediaType::Cjs | MediaType::TypeScript | MediaType::Mts | MediaType::Cts | MediaType::Dts | MediaType::Dmts | MediaType::Dcts | MediaType::Tsx => {
        // continue
      }
      MediaType::SourceMap | MediaType::Unknown | MediaType::Json | MediaType::Wasm | MediaType::TsBuildInfo => {
        // not unfurlable data
        return Ok(data);
      }
    }
    ...
    let mut analyze_specifier = |specifier: &str, range: &deno_graph::PositionRange| {
      let resolved = self.import_map.resolve(specifier, url);
      if let Ok(resolved) = resolved {
        let new_text = if resolved.scheme() == "file" { format!("./{}", url.make_relative(&resolved).unwrap()) } else { resolved.to_string() };
    ...
    for ts_ref in &module_info.ts_references {
      ...
      analyze_specifier(&specifier_with_range.text, &specifier_with_range.range);
    }
...
fn to_range(parsed_source: &ParsedSource, range: &deno_graph::PositionRange) -> std::ops::Range<usize> {
  let mut range = range.as_source_range(parsed_source.text_info()).as_byte_range(parsed_source.text_info().range().start);
