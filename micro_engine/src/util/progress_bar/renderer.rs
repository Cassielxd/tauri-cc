  ...
  fn render(&self, data: ProgressData) -> String {
    let (bytes_text, bytes_text_max_width) = {
      ...
      if total_size == 0 {
        ...
        (format!(" {}/{}", human_download_size(pos, total_size), total_size_str,), 2 + total_size_str.len() * 2)
      }
    };
    let (total_text, total_text_max_width) = if data.total_entries <= 1 {
      ...
      (format!(" ({}/{})", data.total_entries - data.pending_entries, data.total_entries), 4 + total_entries_str.len() * 2)
    };

    let elapsed_text = get_elapsed_text(data.duration);
    let mut text = String::new();
    if !data.display_entry.message.is_empty() {
      text.push_str(&format!("{} {}{}\n", colors::green("Download"), data.display_entry.message, bytes_text,));
    }
    ...
    let total_bars = if same_line_text_width > max_width { 1 } else { max_width - same_line_text_width };
  ...
  fn render(&self, data: ProgressData) -> String {
    let bytes_text = {
      ...
      if total_size == 0 {
        ...
        format!(" {}/{}", human_download_size(pos, total_size), human_download_size(total_size, total_size))
      }
    };
    ...
    format!("{} {}{}{}", data.display_entry.prompt.as_text(), data.display_entry.message, colors::gray(bytes_text), colors::gray(total_text),)
  }
}
