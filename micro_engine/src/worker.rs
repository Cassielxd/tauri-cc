...
impl CliMainWorker {
  ...
  pub async fn run(&mut self) -> Result<i32, AnyError> {
    ...
    if self.is_main_cjs {
      deno_node::load_cjs_module(&mut self.worker.js_runtime, &self.main_module.to_file_path().unwrap().to_string_lossy(), true, self.shared.options.inspect_brk)?;
    } else {
  ...
  pub async fn run_for_watcher(self) -> Result<(), AnyError> {
    ...
    impl FileWatcherModuleExecutor {
      pub fn new(worker: CliMainWorker) -> FileWatcherModuleExecutor {
        FileWatcherModuleExecutor { inner: worker, pending_unload: false }
      }
  ...
  pub async fn maybe_setup_hmr_runner(&mut self) -> Result<Option<HmrRunner>, AnyError> {
  ...
  pub fn execute_script_static(&mut self, name: &'static str, source_code: &'static str) -> Result<v8::Global<v8::Value>, AnyError> {
    self.worker.js_runtime.execute_script_static(name, source_code)
  }
}
...
impl CliMainWorkerFactory {
  ...
  pub async fn create_custom_worker(&self, main_module: ModuleSpecifier, permissions: PermissionsContainer, mut custom_extensions: Vec<Extension>, stdio: deno_runtime::deno_io::Stdio) -> Result<CliMainWorker, AnyError> {
    ...
    let (main_module, is_main_cjs) = if let Ok(package_ref) = NpmPackageReqReference::from_specifier(&main_module) {
      let package_ref = if package_ref.req().version_req.version_text() == "*" {
        ...
        shared
          ...
          .as_ref()
          .and_then(|deps| {
            deps.values().filter_map(|v| v.as_ref().ok()).find(|dep| dep.name == package_ref.req().name).map(|dep| {
      ...
      if let Some(npm_resolver) = shared.npm_resolver.as_managed() {
        npm_resolver.add_package_reqs(&[package_ref.req().clone()]).await?;
      }

      // use a fake referrer that can be used to discover the package.json if necessary
      let referrer = ModuleSpecifier::from_directory_path(self.shared.fs.cwd()?).unwrap().join("package.json")?;
      let package_folder = shared.npm_resolver.resolve_pkg_folder_from_deno_module_req(package_ref.req(), &referrer)?;
      let node_resolution = self.resolve_binary_entrypoint(&package_folder, package_ref.sub_path(), &permissions)?;
    ...
    let options = WorkerOptions {
      bootstrap: BootstrapOptions {
        ...
        maybe_binary_npm_command_name: shared.options.maybe_binary_npm_command_name.clone(),
      },
      extensions,
    ...
    if self.shared.subcommand.is_test_or_jupyter() {
      worker.js_runtime.execute_script_static("40_jupyter.js", include_str!("js/40_jupyter.js"))?;
    }
  ...
  fn resolve_binary_entrypoint(&self, package_folder: &Path, sub_path: Option<&str>, permissions: &PermissionsContainer) -> Result<NodeResolution, AnyError> {
    match self.shared.node_resolver.resolve_binary_export(package_folder, sub_path) {
  ...
  fn resolve_binary_entrypoint_fallback(&self, package_folder: &Path, sub_path: Option<&str>, permissions: &PermissionsContainer) -> Result<Option<NodeResolution>, AnyError> {
    ...
    let Some(resolution) = self.shared.node_resolver.resolve_package_subpath_from_deno_module(package_folder, sub_path, &referrer, NodeResolutionMode::Execution, permissions)? else {
      return Ok(None);
    };
    match &resolution {
      NodeResolution::BuiltIn(_) => Ok(None),
      NodeResolution::CommonJs(specifier) | NodeResolution::Esm(specifier) => {
        if specifier.to_file_path().map(|p| p.exists()).unwrap_or(false) {
          Ok(Some(resolution))
        } else {
...
fn create_web_worker_callback(shared: Arc<SharedWorkerState>, stdio: deno_runtime::deno_io::Stdio) -> Arc<CreateWebWorkerCb> {
  Arc::new(move |args| {
    ...
    let module_loader = shared.module_loader_factory.create_for_worker(args.parent_permissions.clone(), args.permissions.clone());
    let maybe_source_map_getter = shared.module_loader_factory.create_source_map_getter();
    let create_web_worker_cb = create_web_worker_callback(shared.clone(), stdio.clone());
