...
export class Database {
   ...
   *
   * Current Usage:
   *     const orm = new Database(new SQLite3Connector({
   *       filepath: "./orm.sqlite"
   *     }));
   *
   ...
   *
   * Dialect usage:
   *     const orm = new Database("sqlite3", {
   *       filepath: "./orm.sqlite"
   *     });
   *
   *     const orm = new Database({
   ...
   *
   *     await orm.query("SELECT * FROM `flights`");
   */
